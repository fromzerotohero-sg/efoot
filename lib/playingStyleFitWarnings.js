...
export function buildPlayingStyleFitWarnings(roster, styleRows, lang = 'it') {
  ...
  for (const p of roster || []) {
    ...
    if (lang === 'en') {
      ...
      warnings.push(`${p.player_name || '?'}: posición ${pos} pero el estilo "${styleName}" solo funciona en ${allowed}`)
    } else {
...
export function formatFitWarningsBlock(warnings, lang = 'it') {
  ...
  const title =
    lang === 'en'
      ...
      : lang === 'es'
      ? 'POSICIÓN VS ESTILO DE JUGADOR (base de datos — corrige formación o cartas)'
