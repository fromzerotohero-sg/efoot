import { formationNameFromVerticalLayers } from './formationLayerName.js'

/**
 * Valida limitazioni ruolo secondo memoria Attila
 * 
 * Limitazioni:
 * - Attacco (A): 1-5 giocatori (max 2 P; max 1 ESA e max 1 EDA in zona attacco)
 * - Centrocampo (C): 1-6 giocatori (max 1 CLD/CLS)
 * - Difesa (D): 2-5 giocatori (max 3 DC, max 1 TD, max 1 TS; il 4° difensore deve essere TD/TS)
...
export function validateFormationLimits(slotPositions) {
  ...
  const positionCounts = {
    ...
    ESA: 0,
    EDA: 0,
    EDE: 0, // retrocompatibilità: normalizziamo EDE -> EDA
    CLD: 0,
    CLS: 0,
    P: 0,
    SP: 0,
  ...
  const defendersByY = [] // Difensori (y: 63-80, allineato al builder campo)
  const midfieldersByY = [] // Centrocampisti (y: 40-62)
  ...
  positions.forEach((pos, index) => {
    ...
    if (y !== null) {
      if (y > 80) {
        ...
        // Attacco (y < 40)
        // Nota: ESA/EDA possono essere usati come esterni offensivi nel builder (y < 40)
  ...
  // Conta solo posizioni in difesa (y: 63-80)
  ...
  // Conta solo posizioni in centrocampo (y: 40-62)
  ...
  // CLD/CLS in centrocampo (y: 40-62)
  ...
  // Validazione Attacco (1-5 giocatori, max 2 P; max 1 ESA e max 1 EDA)
  ...
  // ESA/EDA in attacco (y < 40): consentite entrambe le fasce (1 ESA + 1 EDA), non due sullo stesso lato
...
/** Fallback legacy: 3 fasce Y fisse (difesa / metà / attacco). */
function formationNameFromLegacyYBands(slotPositions) {
...
/**
 * Deriva il nome modulo Konami dalle coordinate Y (layer verticali).
 * Fallback: conteggio legacy a 3 fasce se Y insufficienti.
 * @param {Record<string, { position?: string, x?: number, y?: number }>} slotPositions
 * @returns {string} es. "3-2-3-2", "4-2-1-3", "4-3-3"; "1-1-1" se dati insufficienti
 */
export function getFormationNameFromSlotPositions(slotPositions) {
  const layerName = formationNameFromVerticalLayers(slotPositions)
  if (layerName) return layerName
  return formationNameFromLegacyYBands(slotPositions)
}
