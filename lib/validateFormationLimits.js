...
export function validateFormationLimits(slotPositions) {
  ...
  positions.forEach((pos, index) => {
    ...
    if (y !== null) {
      if (y > 80) {
        ...
        // Attacco (y < 40)
        // Nota: ESA/EDE possono essere usati come esterni offensivi nel builder (y < 40)
        if (['P', 'SP', 'CF', 'TRQ', 'CLD', 'CLS', 'ESA', 'EDE'].includes(position)) {
  ...
  // Validazione Attacco (1-5 giocatori, max 2 P e max 1 esterno offensivo ESA/EDE)
  ...
  // ESA/EDE in attacco (y < 40) - esterni offensivi
  const esaInAttack = attackersByY.filter(p => p.position === 'ESA').length
  const edeInAttack = attackersByY.filter(p => p.position === 'EDE').length
  if (esaInAttack + edeInAttack > 1) {
    errors.push(`Attacco: massimo 1 esterno offensivo (ESA/EDE) (attualmente: ${esaInAttack + edeInAttack})`)
  }
