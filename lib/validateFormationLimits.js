import { resolveKonamiFormationName } from './formationRules.js'
...
/**
 * Deriva il nome formazione dalle posizioni negli slot.
 * 1) Match preset Konami (linee ruolo-slot)
 * 2) Fallback: conteggio D-M-F da coordinate Y
 * @param {Record<string, { position?: string, x?: number, y?: number }>} slotPositions
 * @returns {string} es. "3-2-3-2", "4-3-3"; "1-1-1" se dati insufficienti
 */
export function getFormationNameFromSlotPositions(slotPositions) {
  const konami = resolveKonamiFormationName(slotPositions)
  if (konami) return konami

  if (!slotPositions || typeof slotPositions !== 'object') return '1-1-1'
  let def = 0, mid = 0, fwd = 0
  // Usa le coordinate y (se presenti) come fonte per la "linea" (D/M/F).
  // Questo evita mismatch quando ruoli come ESA/EDE vengono usati in zona offensiva.
