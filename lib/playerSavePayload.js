...
} from '@/lib/playingStyleResolve'
import { normalizePlayerSkillsArray } from '@/lib/playerSkillLabels'
import { isComAiPlaystyle } from '@/lib/efootballTruthLayer'
...
/** Nome stile grezzo da payload save-player (catalogo / foto). */
...
function resolveCatalogSkills(payload, card) {
  ...
  if (merged.length > 0) {
    return normalizePlayerSkillsArray(merged).filter((skill) => isComAiPlaystyle(skill))
  }
  return []
}
...
export function buildCatalogPlayerSavePayload(card, options = {}) {
  ...
  const rawPlayingStyle =
    payload.role || payload.playing_style || card.playing_style || null
  ...
  const rawStyleContract = getPlayingStylesContract(card)
  const attackingStyle = resolvePlayingStyleDbName(rawStyleContract.attack || rawPlayingStyle)
  const defendingStyle = resolvePlayingStyleDbName(rawStyleContract.defense)
  const playingStyles = {
    format: defendingStyle ? 'dual' : 'single',
    attack: attackingStyle || rawStyleContract.attack || rawPlayingStyle || null,
    defense: defendingStyle || null,
    primary: attackingStyle || rawStyleContract.attack || rawPlayingStyle || null
  }
  ...
  const metadata = {
    ...
    ...(attackingStyle ? { attacking_playing_style: attackingStyle } : {}),
    ...(defendingStyle ? { defensive_playing_style: defendingStyle } : {}),
...
export function buildPhotoPlayerSavePayload(extracted, options = {}) {
  ...
  const italianPlayingStyle = resolvePlayingStyleDbName(rawPlayingStyle)
  const attackingStyle = resolvePlayingStyleDbName(base.attacking_playing_style)
  const defendingStyle = resolvePlayingStyleDbName(base.defensive_playing_style)
  const additionalSkills = normalizePlayerSkillsArray(base.additional_skills)
  const additionalKeys = new Set(additionalSkills.map((skill) => skill.toLowerCase()))
  const nativeSkills = normalizePlayerSkillsArray(
    Array.isArray(base.native_skills)
      ? base.native_skills
      : (Array.isArray(base.skills)
        ? base.skills.filter((skill) => !additionalKeys.has(String(skill || '').toLowerCase()))
        : [])
  )
  const allSkills = normalizePlayerSkillsArray([...nativeSkills, ...additionalSkills])
  const playingStyles = {
    format: defendingStyle ? 'dual' : 'single',
    attack: attackingStyle || italianPlayingStyle || rawPlayingStyle || null,
    defense: defendingStyle || null,
    primary: attackingStyle || italianPlayingStyle || rawPlayingStyle || null
  }
  ...
  return {
    ...
    native_skills: nativeSkills,
    additional_skills: additionalSkills,
    com_skills: normalizePlayerSkillsArray([
      ...(Array.isArray(base.com_skills) ? base.com_skills : []),
      ...(Array.isArray(base.ai_playstyles) ? base.ai_playstyles : [])
    ]).filter((skill) => isComAiPlaystyle(skill)),
    metadata,
    extracted_data: {
      ...base,
      original_positions: originalPositionsOut,
