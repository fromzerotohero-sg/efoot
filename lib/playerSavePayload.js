...
function resolveCatalogSkills(payload, card) {
  if (Array.isArray(payload?.skills) && payload.skills.length > 0) {
    return normalizePlayerSkillsArray(payload.skills)
  }
  if (Array.isArray(card?.player_skills) && card.player_skills.length > 0) {
    return normalizePlayerSkillsArray(card.player_skills)
  }
  return []
}

/** Abilità COM / AI Playstyles da catalogo. */
function resolveCatalogComSkills(payload, card) {
  if (Array.isArray(payload?.com_skills) && payload.com_skills.length > 0) {
    return normalizePlayerSkillsArray(payload.com_skills)
  }
  if (Array.isArray(card?.ai_playstyles) && card.ai_playstyles.length > 0) {
    return normalizePlayerSkillsArray(card.ai_playstyles)
  }
  return []
}
