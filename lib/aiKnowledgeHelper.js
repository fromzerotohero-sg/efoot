...
import { isReservePlayer, isStarterPlayer } from './rosterSlotUtils.js'
...
function calculateRosterScore(players) {
  ...
  const starters = players.filter((p) => isStarterPlayer(p))
  const reserves = players.filter((p) => isReservePlayer(p))
...
function calculateUsageScore({ playersCount, matchesCount, completedGoalsCount, chatMessagesCount = 0 }) {
  const interactions = (playersCount || 0) + (matchesCount || 0) + (completedGoalsCount || 0)
  const chatMessages = Math.max(0, Number(chatMessagesCount) || 0)
  let score = 0
...
export async function calculateAIKnowledgeScore(userId, supabaseUrl, serviceKey) {
  ...
  const [
    ...
    ,
    admin.from('hero_chat_messages').select('id', { count: 'exact', head: true })
