...
function calculateProfileScore(profile) {
  ...
  baseFields.forEach(field => {
    if (isFilled(profile[field])) baseFilled++
  })

  let aiPrefFilled = 0
  aiPreferenceFields.forEach(field => {
    if (isFilled(profile[field])) aiPrefFilled++
  })

  // common_problems contribuisce come segnale aggiuntivo di profiling IA
  if (Array.isArray(profile.common_problems) && profile.common_problems.length > 0) {
    aiPrefFilled += 1
  }

  // Breakdown profilo: base anagrafico/gioco (12) + preferenze IA (4) = 16
  const baseScore = (baseFilled / baseFields.length) * 12
  const aiPrefScore = (Math.min(aiPreferenceFields.length, aiPrefFilled) / aiPreferenceFields.length) * 4
  const total = baseScore + aiPrefScore

  return Math.min(16, Math.round(total * 10) / 10)
}
...
function calculateRosterScore(players, formation) {
  ...
  // Base: 11 titolari = 12%
  const titolari = players.filter((p) => {
  ...
  // Bonus: Riserve = 4%
  const riserve = players.filter((p) => {
    const slot = Number(p?.slot_index)
    return !Number.isFinite(slot) || slot < 0 || slot > 10
  })
  ...
  players.forEach((player) => {
    const hasOverall = player?.overall_rating !== null && player?.overall_rating !== undefined
    const pos = player?.original_positions
    const hasPositions =
      (Array.isArray(pos) && pos.length > 0) ||
      (typeof pos === 'string' && pos.trim() !== '' && pos.trim() !== '[]') ||
      (pos && typeof pos === 'object' && !Array.isArray(pos) && Object.keys(pos).length > 0)

    if (hasOverall && hasPositions) {
      completeDataCount++
    }
  })
...
function calculateCoachScore(coach) {
  ...
  if (coach.is_active === true || coach.id || coach.coach_name || coach.name) {
    return 8
  }
...
export async function calculateAIKnowledgeScore(userId, supabaseUrl, serviceKey) {
  ...
  try {
    ...
    const { data: coachRows, error: coachError } = await admin
      .from('coaches')
      .select('id, coach_name, user_id, is_active')
      .eq('user_id', userId)
      .eq('is_active', true)
      .limit(1)
    ...
    const coachScore = calculateCoachScore(activeCoach || {})
