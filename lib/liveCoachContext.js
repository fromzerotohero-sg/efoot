...
function buildCompactCoachInstructions({ lang = 'it', profile = {}, formation = {}, tactics = {}, coach = {}, players = [], opponentContext = null, efootballKnowledge = '' }) {
  const isEn = lang === 'en'
  ...
  const engineBlockEn = `ENGINE (MANDATORY - USE THIS FRAMEWORK TO DECIDE):
INPUT: ROSTER (player names, OVR, roles, skills), TACTICS (team style: ${tacticalStyle || 'unknown'}), COACH (style competence), USER WEAK POINT: ${weakPoint || 'unknown'}.
DECISION FRAMEWORK:
- Check user question topic (shooting/defense/passing/speed/specific player)
- Check available players with relevant skills from roster: ${playerLines.slice(0, 8).join(', ') || 'unknown'}
- Pick 1 specific action with named players only
FORBIDDEN PHRASES (NEVER USE):
- "tighten fullbacks", "drop the line", "slide tackle", "more compact", "push forward", "pressure", "close them down"
- Any generic football clichés without specific player names
REQUIRED FORMAT:
- First sentence: SPECIFIC action with [PlayerName] and exact role/instruction
- Second sentence: ONE question or timing
- Max 12 words per sentence
- Use ONLY player names from roster above`

  const engineBlockIt = `ENGINE (OBBLIGATORIO - USA QUESTO PER DECIDERE):
INPUT: ROSA (nomi giocatori, OVR, ruoli, abilità), TATTICA (stile squadra: ${tacticalStyle || 'sconosciuto'}), ALLENATORE (competenza stili), PUNTO DEBOLE: ${weakPoint || 'sconosciuto'}.
FRAMEWORK DECISIONE:
- Controlla argomento domanda (tiro/difesa/passaggio/velocità/giocatore specifico)
- Controlla giocatori disponibili con abilità rilevanti dalla rosa: ${playerLines.slice(0, 8).join(', ') || 'sconosciuto'}
- Scegli 1 azione specifica con nomi giocatore citati
FRASI VIETATE (NON USARE MAI):
- "stringere i terzini", "abbassare la linea", "far scivolare", "più compatti", "spingere", "pressione", "chiudere"
- Qualsiasi cliché generico da calcio senza nome giocatore specifico
FORMATO RICHIESTO:
- Prima frase: azione SPECIFICA con [NomeGiocatore] e ruolo/istruzione esatta
- Seconda frase: UNA domanda o timing
- Max 12 parole per frase
- Usa SOLO nomi giocatore dalla rosa sopra`

  // Blocco KNOWLEDGE BASE eFootball (RAG) per consigli concreti
  const ragBlockEn = efootballKnowledge
    ? `EFOOTBALL KNOWLEDGE BASE (USE THIS FOR CONCRETE ADVICE):\n${efootballKnowledge.slice(0, 8000)}`
    : ''
  const ragBlockIt = efootballKnowledge
    ? `KNOWLEDGE BASE eFootball (USA QUESTO PER CONSIGLI CONCRETI):\n${efootballKnowledge.slice(0, 8000)}`
    : ''
...
export async function buildLiveCoachContext({ userId, lang = 'it', opponentContext = null }) {
  ...
  // Carica KNOWLEDGE BASE eFootball (RAG) come fa la chat testuale
