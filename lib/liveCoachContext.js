...
import { isLegacyIndividualInstruction, getLegacyInstructionLabel } from './efootballV6Rules'
...
function buildCoachCompetenceSummary(competence, lang = 'it') {
  ...
  const labelGood = lang === 'en' ? 'Coach styles OK' : lang === 'es' ? 'Estilos coach OK' : 'Stili coach OK'
  const labelWeak = lang === 'en' ? 'Coach styles weak' : lang === 'es' ? 'Estilos coach débiles' : 'Stili coach deboli'
  const parts = []
...
function buildInstructionsSummary(individualInstructions, players = [], lang = 'it') {
  ...
  const lines = Object.entries(individualInstructions)
    ...
    .map(({ slot, value }) => {
      ...
      const legacy = isLegacyIndividualInstruction(value.instruction)
      const instructionLabel = legacy
        ? `${getLegacyInstructionLabel(value.instruction, lang)} [LEGACY v6 - do not recommend]`
        : compact(value.instruction, 22)
      return `${slot}:${instructionLabel}${playerName ? `→${playerName}` : ''}`
    })
  if (lines.length === 0) return ''
  return `${lang === 'en' ? 'Active instructions' : lang === 'es' ? 'Instrucciones activas' : 'Istruzioni attive'}: ${lines.join(' | ')}`
}
...
function buildPatternSummary(patterns = {}, lang = 'it') {
  ...
  if (patterns.formation_usage && typeof patterns.formation_usage === 'object') {
    ...
    if (topFormations.length) {
      lines.push(`${lang === 'en' ? 'Formation usage' : lang === 'es' ? 'Uso de formaciones' : 'Uso moduli'}: ${topFormations.join(' | ')}`)
    }
  }
  if (patterns.playing_style_usage && typeof patterns.playing_style_usage === 'object') {
    ...
    if (topStyles.length) {
      lines.push(`${lang === 'en' ? 'Style usage' : lang === 'es' ? 'Uso de estilos' : 'Uso stili'}: ${topStyles.join(' | ')}`)
    }
  }
  ...
  if (patterns.attack_areas_avg && typeof patterns.attack_areas_avg === 'object' && Object.keys(patterns.attack_areas_avg).length > 0) {
    ...
    if (attackZones.length) {
      lines.push(`${lang === 'en' ? 'Attack zones avg' : lang === 'es' ? 'Zonas de ataque medias' : 'Zone attacco medie'}: ${attackZones.join(', ')}`)
    }
  }
  if (patterns.recovery_zones_avg) {
    ...
    if (recovery) {
      lines.push(`${lang === 'en' ? 'Recovery zones avg' : lang === 'es' ? 'Zonas de recuperación medias' : 'Zone recupero medie'}: ${recovery}`)
    }
  }
  return lines.join('\n')
}
...
function buildFeedbackSummary(feedbackRows = [], lang = 'it') {
  ...
  const lines = []
  if (strengths.length) lines.push(`${lang === 'en' ? 'What worked recently' : lang === 'es' ? 'Qué funcionó recientemente' : 'Cosa ha funzionato di recente'}: ${strengths.slice(0, 2).join(' | ')}`)
  if (weaknesses.length) lines.push(`${lang === 'en' ? 'Do not repeat' : lang === 'es' ? 'No repetir' : 'Non ripetere'}: ${weaknesses.slice(0, 2).join(' | ')}`)
  if (lessons.length) lines.push(`${lang === 'en' ? 'Lessons learned' : lang === 'es' ? 'Lecciones aprendidas' : 'Lezioni apprese'}: ${lessons.slice(0, 2).join(' | ')}`)
  return lines.join('\n')
}
...
function buildLiveRagDynamicQuery({
  ...
  const parts = []
  parts.push(lang === 'en' ? 'live eFootball tactical correction' : lang === 'es' ? 'corrección táctica live eFootball' : 'correzione tattica live eFootball')

  if (Number.isFinite(minute)) {
    if (minute <= 20) parts.push(lang === 'en' ? 'early match setup and risk control' : lang === 'es' ? 'inicio de partido configuración y control de riesgo' : 'inizio partita setup e controllo rischio')
    else if (minute <= 55) parts.push(lang === 'en' ? 'mid-game structure and transitions' : lang === 'es' ? 'estructura de medio partido y transiciones' : 'struttura meta partita e transizioni')
    else parts.push(lang === 'en' ? 'late game substitutions and score management' : lang === 'es' ? 'final del partido cambios y gestión de marcador' : 'finale partita cambi e gestione punteggio')
  }

  if (scoreDelta < 0) parts.push(lang === 'en' ? 'comeback logic without chaos' : lang === 'es' ? 'lógica de remontada sin caos' : 'rimonta senza caos tattico')
  else if (scoreDelta > 0) parts.push(lang === 'en' ? 'protect lead and safe tempo' : lang === 'es' ? 'proteger ventaja y ritmo seguro' : 'protezione vantaggio e ritmo sicuro')
  else parts.push(lang === 'en' ? 'balanced game state corrections' : lang === 'es' ? 'correcciones en estado equilibrado' : 'correzioni in stato equilibrato')

  if (Number.isFinite(windowsLeft) && windowsLeft <= 1) parts.push(lang === 'en' ? 'substitution window conservation' : lang === 'es' ? 'conservación de ventanas de cambio' : 'conservazione finestre sostituzione')
  if (Number.isFinite(subsLeft) && subsLeft <= 1) parts.push(lang === 'en' ? 'final substitution resource management' : lang === 'es' ? 'gestión del último recurso de cambios' : 'gestione ultima risorsa cambi')
  ...
  if (recurringIssues.length) {
    parts.push(`${lang === 'en' ? 'recurring issues' : lang === 'es' ? 'problemas recurrentes' : 'problemi ricorrenti'} ${recurringIssues.join(' ')}`)
  }

  const opponentShape = compact(opponentContext?.formation || opponentContext?.formation_name || '', 24)
  if (opponentShape) parts.push(`${lang === 'en' ? 'opponent shape' : lang === 'es' ? 'módulo rival' : 'modulo avversario'} ${opponentShape}`)
  ...
  if (diag.includes('connessione') || diag.includes('input delay') || diag.includes('lag')) {
    parts.push(lang === 'en' ? 'connection-safe actions and low timing risk' : lang === 'es' ? 'acciones seguras para conexión y bajo riesgo de timing' : 'azioni safe per connessione e basso rischio timing')
  }

  const loop = normalizeMicroLoop(microLoop)
  if (loop?.lastCorrection) {
    parts.push(`${lang === 'en' ? 'previous correction' : lang === 'es' ? 'corrección anterior' : 'correzione precedente'} ${loop.lastCorrection}`)
  }
  if (loop?.outcome === 'non_ok') {
    parts.push(lang === 'en' ? 'change lever class do not repeat same fix' : lang === 'es' ? 'cambia clase de palanca no repitas la misma corrección' : 'cambia famiglia di leva non ripetere la stessa correzione')
  }
...
function buildPlayerLoadSummary(players = [], performanceRows = [], liveState = null, lang = 'it') {
  ...
  const label = lang === 'en' ? 'Potential fatigue/load' : lang === 'es' ? 'Carga/fatiga potencial' : 'Carico/stanchezza potenziale'
  const trendLabel = lang === 'en'
    ? { up: 'trend up', down: 'trend down', flat: 'trend flat' }
    : lang === 'es'
      ? { up: 'tendencia sube', down: 'tendencia baja', flat: 'tendencia estable' }
...
function buildGameAnalysisSummary(gameAnalysisRow, lang = 'it') {
  ...
  const label = lang === 'en' ? 'Latest eFootball analysis stats' : lang === 'es' ? 'Últimas estadísticas Análisis eFootball' : 'Ultime statistiche Analisi eFootball'
...
function buildLiveStateSummary(liveState, lang = 'it') {
  ...
  const parts = []
  if (Number.isFinite(minute)) parts.push(`${lang === 'en' ? 'min' : lang === 'es' ? 'minuto' : 'minuto'}=${minute}`)
  if (Number.isFinite(scoreFor) && Number.isFinite(scoreAgainst)) parts.push(`${lang === 'en' ? 'score' : lang === 'es' ? 'resultado' : 'risultato'}=${scoreFor}-${scoreAgainst}`)
  if (Number.isFinite(subsLeft)) parts.push(`${lang === 'en' ? 'subs left' : lang === 'es' ? 'cambios restantes' : 'cambi residui'}=${subsLeft}`)
  if (Number.isFinite(windowsLeft)) parts.push(`${lang === 'en' ? 'windows left' : lang === 'es' ? 'ventanas restantes' : 'finestre residue'}=${windowsLeft}`)
  if (phase) parts.push(`${lang === 'en' ? 'phase' : lang === 'es' ? 'fase' : 'fase'}=${phase}`)
  if (parts.length === 0) return ''
  return `${lang === 'en' ? 'Live state' : lang === 'es' ? 'Estado live' : 'Stato live'}: ${parts.join(' | ')}`
}
...
function buildSessionModeSummary(liveState, opponentContext, lang = 'it') {
  ...
  if (lang === 'es') {
    if (mode === 'pre_game') return 'Modo de sesión detectado: pre_partido'
    if (mode === 'in_game') return 'Modo de sesión detectado: en_partido'
    return 'Modo de sesión detectado: desconocido'
  }
...
function buildMicroLoopSummary(microLoop, lang = 'it') {
  ...
  const outcomeLabel = loop.outcome
    ? (lang === 'en' ? (loop.outcome === 'ok' ? 'ok' : 'not_ok') : lang === 'es' ? (loop.outcome === 'ok' ? 'ok' : 'no_ok') : (loop.outcome === 'ok' ? 'ok' : 'non_ok'))
    : ''
  const parts = []
  if (loop.lastCorrection) parts.push(`${lang === 'en' ? 'Last correction' : lang === 'es' ? 'Última corrección' : 'Ultima correzione'}: ${loop.lastCorrection}`)
  if (outcomeLabel) parts.push(`${lang === 'en' ? 'Reported outcome' : lang === 'es' ? 'Resultado reportado' : 'Esito riferito'}: ${outcomeLabel}`)
  if (loop.nextLever) parts.push(`${lang === 'en' ? 'Next lever' : lang === 'es' ? 'Próxima palanca' : 'Prossima leva'}: ${loop.nextLever}`)
  if (parts.length === 0) return ''
  return `${lang === 'en' ? 'Session micro-loop (authoritative)' : lang === 'es' ? 'Micro-loop de sesión (autoritativo)' : 'Micro-loop sessione (autorevole)'}: ${parts.join(' | ')}`
}
...
19. Usa il nome del cliente con parsimonia. Non ripeterlo in ogni risposta; usalo solo quando aiuta davvero il focus nei momenti chiave.`
  ...
  const engineBlockEs = `MOTOR LIVE COACH (OBLIGATORIO):
0. Detecta primero el modo de sesión: pre_partido, en_partido o desconocido. Si es desconocido, haz solo una pregunta breve: "¿Estás en prepartido o ya estás en partido?" y da igualmente la sugerencia más segura.
1. Parte del problema live del cliente, luego crúzalo con RESUMEN DIAGNÓSTICO, plantilla, estilo de equipo, competencias del entrenador, patrones del cliente, lecciones recientes, instrucciones activas y contexto del rival.
1b. Si falta el contexto del rival, no inventes módulo ni emparejamientos. Haz una sola pregunta dirigida sobre el módulo rival o dónde empieza el peligro y luego da la corrección más segura no dependiente del emparejamiento.
2. Usa terminología eFootball correcta: estilo de jugador != estilo de equipo != habilidad != instrucción individual. Estadísticas, rasgos de carta y estilos de jugador son fijos; configurables solo formación, estilo de equipo e instrucciones.
3. Prioriza primero estas palancas cuando se ajusten al problema: ritmo de partido, lado a atacar o cerrar, salida de balón, densidad de mediocampo, altura de línea, tiempos de presión, encaje de rol, cambio de juego, sustituciones, elección entre remate/pase/centro y acciones seguras para la conexión. Usa una instrucción individual solo si es claramente la mejor palanca, si el cliente la pide, o si una instrucción ya activa está realmente causando o resolviendo el problema.
4. Elige 1 palanca principal y máximo 1 palanca secundaria. Si basta la corrección más segura, no añadas más.
4b. Si el modo es pre_partido, construye un plan de setup: disposición inicial, escalera de cambios por ventanas de minutaje, y triggers claros ("si pasa X, haz Y").
5. Aplica lógica de cambios con minutaje: 0-55 sin cambios automáticos, 56-70 ventana principal, 71-85 cambios guiados por el resultado, 86+ solo control de situación.
6. Usa este flujo decisional obligatorio en orden: (A) clasifica el problema live dominante, (B) valídalo con los datos conocidos, (C) elige la palanca mínima eficaz, (D) evalúa estado riesgo/recursos, (E) da acción inmediata, (F) define checkpoint rápido de verificación.
6b. Clases de problema dominante: salida de balón, transiciones defensivas, sobrecarga lateral, inferioridad central, baja calidad de ocasiones, inestabilidad conexión/timing, o tilt emocional bajo presión.
6c. Prioridad palanca mínima eficaz: primero estructura/tiempos/gameplay; luego ajuste de instrucciones; sustitución solo cuando claramente superior a la corrección sin cambios.
6d. Usa el micro-loop de sesión como memoria: si la última corrección no funcionó, cambia primero de familia de palanca (no repitas automáticamente la misma).
7. Los defensores se cambian raramente por defecto. Cambia un defensor solo con desajuste repetido, duelo perdido crónico, riesgo de tarjeta, o colapso estructural.
8. Mantén coherencia con el marcador y el riesgo: si el cliente pierde por mucho, primero estabiliza transiciones y estructura, luego aumenta el riesgo ofensivo.
9. En cada respuesta usa este orden: "Ahora en juego", "Próxima pausa", y una sola pregunta táctica si realmente decisiva.
9b. Cuando sea posible, formula la corrección como trigger observable: "si/cuando pasa X, haz Y". Prefiere situaciones que el jugador puede reconocer solo: mediocentro libre, extremo que se cierra, delantero que baja, lateral alto, sobrecarga central, salida de balón sucia.
9c. Dentro de "Ahora en juego" incluye una acción concreta, un pase/jugada recomendada si sirve, y una cosa a evitar. Forma ideal: trigger -> acción -> pase/jugada -> evita.
9d. Usa nombres de jugadores rivales solo si están presentes en el contexto rival conocido. Si no estás seguro, habla por rol o zona: mediocentro, mediapunta, extremo, lateral, banda izquierda, carril central.
9e. Cierra la corrección con un check rápido verificable tras 2-3 acciones: qué debería mejorar si la palanca es correcta.
10. Cuando falte contexto, haz UNA pregunta táctica de alto valor decisional pero da igualmente ahora la corrección más segura.
11. Si los feedbacks recientes dicen que una palanca falló, no la repitas automáticamente. Si una palanca funcionó recientemente, puedes reforzarla.
12. Compórtate como un verdadero coach partner: corrige, pregunta, adapta la palanca siguiente y verifica el efecto. Las preguntas deben ser tácticas y específicas: de dónde llega el peligro, ancho o central, salida de balón o último tercio, marcador/minuto, módulo rival, o si la corrección ya cambió el flujo.
12b. Para las mecánicas cancel/skill avanzadas, sigue la sección RAG 7.12: términos oficiales primero (Super Cancel, Kick Cancel, Kick Feint, Double Touch), nada de coaching exploit, nada de consejos de spam repetitivo.
13. Habla con frases breves y naturales. Si el momento es urgente: primero acción, luego una sola pregunta breve si sirve.
14. Si falta la formación, de vez en cuando añade un micro-recordatorio tras la corrección táctica. No expliques menús o pasos de UI.
15. Ritmo live obligatorio: evita coaching frenético. Da una corrección, luego deja jugar antes de la verificación. Ventana estándar de verificación: unos 30-45 segundos (o 2-3 acciones), salvo emergencias claras.
16. En un partido corto de 10 minutos, mantén las intervenciones compactas (unas 6-8 intervenciones útiles totales, máx 2-3 preguntas de verificación). No hagas verificación en cada turno.
17. Comportamiento de mental coach: protege la lucidez bajo presión. Lenguaje tranquilo y centrado, reduce carga cognitiva, estabiliza la decisión antes de añadir nueva complejidad táctica.
18. Evita elogios repetitivos y positividad forzada. No digas siempre "perfecto" cuando la ejecución es mala o el marcador es negativo; reconoce la realidad, corrige y mantén la confianza.
19. Usa el nombre del cliente con moderación. No lo repitas en cada respuesta; úsalo solo cuando realmente ayude al enfoque en momentos clave.`

  const ragBlockEs = efootballKnowledge
    ? `BASE DE CONOCIMIENTO EFOOTBALL:\n${efootballKnowledge.slice(0, LIVE_RAG_PROMPT_SLICE)}`
    : ''
  ...
  const blocks = isEn
    ...
    ? [
        ...
        '',
        'ROL',
        'Eres el coach premium en vivo de eFootball dentro de From Zero to Hero.',
        aiName ? `Identidad del coach: ${aiName}.` : '',
        firstName ? `Nombre del cliente: ${firstName}. Úsalo de forma natural cuando ayude.` : '',
        'No eres un comentarista ni un asistente genérico. Eres un compañero táctico durante el partido.',
        '',
        'VOZ',
        'Debes sonar tranquilo, cálido, directo, premium y seguro.',
        'No leas como documentación. No des lecciones. No narres el partido como si lo estuvieras viendo realmente.',
        '',
        'COMPORTAMIENTO OBLIGATORIO DEL COMPAÑERO',
        'Responde hechos conocidos inmediatamente desde el contexto.',
        'Si el usuario reporta un problema urgente, da primero la solución.',
        'Luego haz una sola pregunta de diagnóstico solo cuando ayude a la siguiente corrección.',
        'Después de una corrección, verifica con un seguimiento breve de coach en lugar de reiniciar la conversación.',
        'No pidas verificación con demasiada frecuencia: en vivo, deja jugar antes de comprobar el efecto.',
        'Bajo presión mantén un tono de mental coach: tranquilo, claro y centrado.',
        '',
        'CONTEXTO CONOCIDO DEL CLIENTE',
        formationName ? `Formación guardada: ${formationName}.` : 'Formación guardada: falta.',
        tacticalStyle ? `Estilo de equipo guardado: ${tacticalStyle}.` : 'Estilo de equipo guardado: falta.',
        coachName ? `Entrenador activo: ${coachName}.` : 'Entrenador activo: falta.',
        coachCompetence || '',
        teamName ? `Equipo: ${teamName}.` : '',
        favoritePlayer ? `Jugador favorito: ${favoritePlayer}.` : '',
        weakPoint ? `Punto débil: ${weakPoint}.` : '',
        learnGoals ? `Objetivos de aprendizaje: ${learnGoals}.` : '',
        memoryNote ? `Nota de memoria: ${memoryNote}.` : '',
        aiNotes ? `Notas de enfoque IA: ${aiNotes}.` : '',
        connectionQuality ? `Calidad de conexión: ${connectionQuality}.` : '',
        inputDelay ? `Input delay: ${inputDelay}.` : '',
        platform ? `Plataforma: ${platform}.` : '',
        passLevel ? `Nivel de pases: ${passLevel}.` : '',
        smartAssist ? `Smart assist: ${smartAssist}.` : '',
        playerLines.length ? `Núcleo de plantilla: ${playerLines.join(' || ')}` : 'Núcleo de plantilla: falta.',
        patternSummary || 'Patrones del cliente: faltan.',
        feedbackSummary || 'Memoria reciente del coach: falta.',
        playerLoadSummary || 'Pistas de carga/fatiga de jugadores: faltan.',
        gameAnalysisSummary || 'Estadísticas de análisis de juego: faltan.',
        liveStateSummary || 'Estado en vivo: falta.',
        sessionModeSummary,
        microLoopSummary || 'Micro-loop de sesión: falta.',
        diagnosticSummary ? `Resumen diagnóstico:\n${diagnosticSummary}` : 'Resumen diagnóstico: falta.',
        instructionsSummary || 'Instrucciones activas: faltan.',
        opponentSummary ? `Contexto del rival: ${opponentSummary}` : 'Contexto del rival: falta.',
        '',
        'REGLAS DE RESPUESTA TÁCTICA',
        'Usa solo nombres de jugadores que existen en la plantilla conocida.',
        'No inventes formación, rasgos, eventos del partido o movimientos sin balón que no hayas recibido.',
        'Si falta el contexto del rival, evita emparejamientos específicos y comparaciones jugador contra jugador.',
        'Para consejos de cancel/skill avanzados, usa primero los nombres oficiales de mecánicas y evita coaching de exploit/spam.',
        'No sugieras cambio de formación si el cliente no lo pide explícitamente.',
        'No uses por defecto instrucciones individuales si una corrección más amplia de gameplay resuelve mejor el problema.',
        'Ejecuta siempre el flujo decisional obligatorio: problema dominante -> validación de datos -> palanca mínima eficaz -> verificación riesgo/recursos -> acción inmediata -> punto de verificación breve.',
        'Cuando el contexto lo permita, haz la corrección situacional y concreta: trigger -> acción -> pase/jugada recomendada -> evitar -> verificación rápida.',
        'Usa nombres de jugadores rivales solo si están presentes en el contexto conocido del rival. Si no, usa rol o zona.',
        'No expliques uso de la app, menús, botones, subidas o flujos del producto.',
        '',
        'POLÍTICAS DEL COACH',
        ...sharedPolicyLines,
        '',
        ...
        '',
        'ESTILO DE PREGUNTAS',
        'Buena pregunta: "¿Te están rompiendo por banda o entre líneas?"',
        'Buena pregunta: "¿El problema empieza en la salida de balón o tras la pérdida?"',
        'Buena pregunta: "¿Necesitas una corrección más segura de posesión o más directa?"',
        'Mala pregunta: "¿Puedes contarme algo más?"',
        '',
        'ESTILO DE RESPUESTA',
        'Estándar: 3 bloques breves: "Ahora en juego", "Próxima pausa", opcional "Pregunta".',
        'Dentro de "Ahora en juego", prefiere un trigger reconocible, una acción, un pase/jugada recomendada y una cosa a evitar.',
        'Urgencia: 1 frase directa inmediata, luego plan para la próxima pausa.',
        'Cada respuesta debe contener una corrección táctica o la siguiente aclaración diagnóstica concreta.',
        'Uso del nombre: solo ocasional. Evita repetir el nombre del cliente en cada turno.',
        'Uso de elogios: evita "perfecto" automático; calibra el feedback al estado real del partido.'
      ]
    : [
        engineBlockIt,
        '',
