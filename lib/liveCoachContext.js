...
function buildCoachCompetenceSummary(competence, lang = 'it') {
  ...
  const labelGood = (lang === 'en' || lang === 'es') ? 'Coach styles OK' : 'Stili coach OK'
  const labelWeak = (lang === 'en' || lang === 'es') ? 'Coach styles weak' : 'Stili coach deboli'
  const parts = []
...
function buildInstructionsSummary(individualInstructions, players = [], lang = 'it') {
  ...
  return `${(lang === 'en' || lang === 'es') ? 'Active instructions' : 'Istruzioni attive'}: ${lines.join(' | ')}`
}
...
function buildPatternSummary(patterns = {}, lang = 'it') {
  ...
  if (patterns.formation_usage && typeof patterns.formation_usage === 'object') {
    ...
    if (topFormations.length) {
      lines.push(`${(lang === 'en' || lang === 'es') ? 'Formation usage' : 'Uso moduli'}: ${topFormations.join(' | ')}`)
    }
  }
  if (patterns.playing_style_usage && typeof patterns.playing_style_usage === 'object') {
    ...
    if (topStyles.length) {
      lines.push(`${(lang === 'en' || lang === 'es') ? 'Style usage' : 'Uso stili'}: ${topStyles.join(' | ')}`)
    }
  }
  if (Array.isArray(patterns.recurring_issues) && patterns.recurring_issues.length > 0) {
    const issues = patterns.recurring_issues
      ...
      .filter(Boolean)
    if (issues.length) {
      lines.push(`${(lang === 'en' || lang === 'es') ? 'Recurring issues' : 'Problemi ricorrenti'}: ${issues.join(' | ')}`)
    }
  }
  if (patterns.attack_areas_avg && typeof patterns.attack_areas_avg === 'object' && Object.keys(patterns.attack_areas_avg).length > 0) {
    ...
    if (attackZones.length) {
      lines.push(`${(lang === 'en' || lang === 'es') ? 'Attack zones avg' : 'Zone attacco medie'}: ${attackZones.join(', ')}`)
    }
  }
  if (patterns.recovery_zones_avg) {
    ...
    if (recovery) {
      lines.push(`${(lang === 'en' || lang === 'es') ? 'Recovery zones avg' : 'Zone recupero medie'}: ${recovery}`)
    }
  }
  return lines.join('\n')
}
...
function buildFeedbackSummary(feedbackRows = [], lang = 'it') {
  ...
  const lines = []
  if (strengths.length) lines.push(`${(lang === 'en' || lang === 'es') ? 'What worked recently' : 'Cosa ha funzionato di recente'}: ${strengths.slice(0, 2).join(' | ')}`)
  if (weaknesses.length) lines.push(`${(lang === 'en' || lang === 'es') ? 'Do not repeat' : 'Non ripetere'}: ${weaknesses.slice(0, 2).join(' | ')}`)
  if (lessons.length) lines.push(`${(lang === 'en' || lang === 'es') ? 'Lessons learned' : 'Lezioni apprese'}: ${lessons.slice(0, 2).join(' | ')}`)
  return lines.join('\n')
}
...
function buildLiveRagDynamicQuery({
  ...
  const parts = []
  parts.push((lang === 'en' || lang === 'es') ? 'live eFootball tactical correction' : 'correzione tattica live eFootball')

  if (Number.isFinite(minute)) {
    if (minute <= 20) parts.push((lang === 'en' || lang === 'es') ? 'early match setup and risk control' : 'inizio partita setup e controllo rischio')
    else if (minute <= 55) parts.push((lang === 'en' || lang === 'es') ? 'mid-game structure and transitions' : 'struttura meta partita e transizioni')
    else parts.push((lang === 'en' || lang === 'es') ? 'late game substitutions and score management' : 'finale partita cambi e gestione punteggio')
  }

  if (scoreDelta < 0) parts.push((lang === 'en' || lang === 'es') ? 'comeback logic without chaos' : 'rimonta senza caos tattico')
  else if (scoreDelta > 0) parts.push((lang === 'en' || lang === 'es') ? 'protect lead and safe tempo' : 'protezione vantaggio e ritmo sicuro')
  else parts.push((lang === 'en' || lang === 'es') ? 'balanced game state corrections' : 'correzioni in stato equilibrato')

  if (Number.isFinite(windowsLeft) && windowsLeft <= 1) parts.push((lang === 'en' || lang === 'es') ? 'substitution window conservation' : 'conservazione finestre sostituzione')
  if (Number.isFinite(subsLeft) && subsLeft <= 1) parts.push((lang === 'en' || lang === 'es') ? 'final substitution resource management' : 'gestione ultima risorsa cambi')
  ...
  if (recurringIssues.length) {
    parts.push(`${(lang === 'en' || lang === 'es') ? 'recurring issues' : 'problemi ricorrenti'} ${recurringIssues.join(' ')}`)
  }

  const opponentShape = compact(opponentContext?.formation || opponentContext?.formation_name || '', 24)
  if (opponentShape) parts.push(`${(lang === 'en' || lang === 'es') ? 'opponent shape' : 'modulo avversario'} ${opponentShape}`)
  ...
  if (diag.includes('connessione') || diag.includes('input delay') || diag.includes('lag')) {
    parts.push((lang === 'en' || lang === 'es') ? 'connection-safe actions and low timing risk' : 'azioni safe per connessione e basso rischio timing')
  }

  const loop = normalizeMicroLoop(microLoop)
  if (loop?.lastCorrection) {
    parts.push(`${(lang === 'en' || lang === 'es') ? 'previous correction' : 'correzione precedente'} ${loop.lastCorrection}`)
  }
  if (loop?.outcome === 'non_ok') {
    parts.push((lang === 'en' || lang === 'es') ? 'change lever class do not repeat same fix' : 'cambia famiglia di leva non ripetere la stessa correzione')
  }
...
function buildPlayerLoadSummary(players = [], performanceRows = [], liveState = null, lang = 'it') {
  ...
  const label = (lang === 'en' || lang === 'es') ? 'Potential fatigue/load' : 'Carico/stanchezza potenziale'
...
function buildGameAnalysisSummary(gameAnalysisRow, lang = 'it') {
  ...
  const label = (lang === 'en' || lang === 'es') ? 'Latest eFootball analysis stats' : 'Ultime statistiche Analisi eFootball'
...
function buildLiveStateSummary(liveState, lang = 'it') {
  ...
  const parts = []
  if (Number.isFinite(minute)) parts.push(`${(lang === 'en' || lang === 'es') ? 'min' : 'minuto'}=${minute}`)
  if (Number.isFinite(scoreFor) && Number.isFinite(scoreAgainst)) parts.push(`${(lang === 'en' || lang === 'es') ? 'score' : 'risultato'}=${scoreFor}-${scoreAgainst}`)
  if (Number.isFinite(subsLeft)) parts.push(`${(lang === 'en' || lang === 'es') ? 'subs left' : 'cambi residui'}=${subsLeft}`)
  if (Number.isFinite(windowsLeft)) parts.push(`${(lang === 'en' || lang === 'es') ? 'windows left' : 'finestre residue'}=${windowsLeft}`)
  if (phase) parts.push(`${(lang === 'en' || lang === 'es') ? 'phase' : 'fase'}=${phase}`)
  if (parts.length === 0) return ''
  return `${(lang === 'en' || lang === 'es') ? 'Live state' : 'Stato live'}: ${parts.join(' | ')}`
}
...
function buildMicroLoopSummary(microLoop, lang = 'it') {
  ...
  const outcomeLabel = loop.outcome
    ? ((lang === 'en' || lang === 'es') ? (loop.outcome === 'ok' ? 'ok' : 'not_ok') : (loop.outcome === 'ok' ? 'ok' : 'non_ok'))
    : ''
  const parts = []
  if (loop.lastCorrection) parts.push(`${(lang === 'en' || lang === 'es') ? 'Last correction' : 'Ultima correzione'}: ${loop.lastCorrection}`)
  if (outcomeLabel) parts.push(`${(lang === 'en' || lang === 'es') ? 'Reported outcome' : 'Esito riferito'}: ${outcomeLabel}`)
  if (loop.nextLever) parts.push(`${(lang === 'en' || lang === 'es') ? 'Next lever' : 'Prossima leva'}: ${loop.nextLever}`)
  if (parts.length === 0) return ''
  return `${(lang === 'en' || lang === 'es') ? 'Session micro-loop (authoritative)' : 'Micro-loop sessione (autorevole)'}: ${parts.join(' | ')}`
}
