...
export function buildPurchaseFactsBlock({
  ...
  if (['same_name', 'same_field_role', 'same_family', 'same_wide_flank', 'natural_competence_only'].includes(anchor.type)) {
    ...
    if (anchor.type === 'same_wide_flank') {
      lines.push(
        isEn
          ? 'Wide attacker rule: if the pack card brings 1v1, pace, special dribbling/curler tools, side-foot value or a different way to attack the lane, frame it as roster weapon or premium rotation. Do not downgrade only because the current wide starter is strong.'
          : 'Regola fascia offensiva: se la carta pack porta 1v1, velocità, dribbling/tiro speciale, valore lato-piede o modo diverso di attaccare la corsia, leggila come arma di rosa o rotazione premium. Non abbassare solo perché il titolare largo è forte.'
      )
    }
