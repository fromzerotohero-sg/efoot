...
export function isPremiumCatalogCard(card = {}, catalogCard = null) {
  const cat = toAscii(
    `${card?.category || ''} ${catalogCard?.category || ''} ${catalogCard?.card_type || ''}`
  )
...
export function buildPurchaseFactsBlock({
  ...
  if (rotationPool.crowdedLane) {
    lines.push(
      isEn
        ? `Rotation pool: ${rotationPool.countOnRole} starters already in ${cardRole} on field (${rotationPool.names.join(', ') || 'n/a'}).`
        : `Pool rotazione: ${rotationPool.countOnRole} titolari già in ${cardRole} in campo (${rotationPool.names.join(', ') || 'n/d'}).`
    )
  }
