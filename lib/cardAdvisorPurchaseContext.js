/**
 * Fatti vincolanti per Card Advisor Pro: ruoli in campo, anchor confronto, gap modulo.
 * Confronto skill: stesso ruolo sul modulo salvato, poi fascia larga, poi reparto.
 * Sinergie con compagni = sezione separata nel report, non paragoni difensori/attaccanti.
 */

import { getSlotPosition } from './buildCoachServerUtils.js'
import { isStarterPlayer, parseSlotIndex } from './rosterSlotUtils.js'

const VALID_PURCHASE_FIT = new Set([
  'fits_current_setup',
  'fits_with_rotation',
  'fits_if_formation_change',
  'skill_only_no_slot',
  'not_your_playstyle',
  'skip_duplicate',
  'insufficient_data'
])

const FAMILY_POSITIONS = {
  gk: ['PT'],
  def: ['DC', 'TD', 'TS'],
  mid: ['MED', 'CC', 'TRQ', 'CLS', 'CLD'],
  att: ['P', 'SP', 'CF', 'ESA', 'EDA']
}

/** Fascia offensiva: solo per carte att/mid larghe — mai TD/TS con ESA */
const WIDE_LEFT_ATTACK = new Set(['CLS', 'ESA'])
const WIDE_RIGHT_ATTACK = new Set(['CLD', 'EDA'])

function toAscii(value = '') {
  return String(value)
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, ' ')
    .trim()
}

export function roleFamily(position = '') {
  const pos = String(position || '').trim().toUpperCase()
  if (pos === 'PT') return 'gk'
  if (FAMILY_POSITIONS.def.includes(pos)) return 'def'
  if (FAMILY_POSITIONS.mid.includes(pos)) return 'mid'
  return 'att'
}

function isStarter(player) {
  return isStarterPlayer(player)
}

/** Ruolo scheda rosa (DB `players.position`). */
function rosterRole(player) {
  return String(player?.position || '').trim().toUpperCase()
}

/**
 * Ruolo effettivo in campo: `formation_layout.slot_positions` per lo slot, altrimenti scheda.
 * @param {object|null} formation — riga `formation_layout`
 */
export function effectiveFieldRole(player, formation = null) {
  const fromLayout = getSlotPosition(player, formation)
  if (fromLayout) return String(fromLayout).trim().toUpperCase()
  return rosterRole(player)
}

function playerInFamilyOnPitch(player, family, formation = null) {
  const role = effectiveFieldRole(player, formation)
  return (FAMILY_POSITIONS[family] || []).includes(role)
}

function formatAnchorLabel(player, formation = null) {
  const onPitch = effectiveFieldRole(player, formation)
  const roster = rosterRole(player)
  if (!onPitch) return String(player?.player_name || '').trim()
  if (roster && roster !== onPitch) {
    return `${player.player_name} (${onPitch} in campo, scheda ${roster})`
  }
  return `${player.player_name} (${onPitch})`
}

function naturalHighPositions(player) {
  const list = Array.isArray(player?.original_positions) ? player.original_positions : []
  return list
    .map((entry) => {
      const position = typeof entry === 'string' ? entry : entry?.position
      const competence = typeof entry === 'object' ? String(entry?.competence || '').toLowerCase() : ''
      if (!position) return null
      if (competence && !competence.includes('alta') && !competence.includes('high')) return null
      return String(position).trim().toUpperCase()
    })
    .filter(Boolean)
}

function playerHasNaturalAt(player, targetPosition) {
  return naturalHighPositions(player).includes(String(targetPosition || '').trim().toUpperCase())
}

function findSameNameStarter(starters, cardName) {
  const key = toAscii(cardName)
  if (!key) return null
  return (
    starters.find((p) => toAscii(p.player_name) === key) ||
    starters.find((p) => key.length >= 4 && toAscii(p.player_name).includes(key)) ||
    null
  )
}

function allowsWideFlankComparison(cardRole) {
  const family = roleFamily(cardRole)
  if (family === 'def' || family === 'gk') return false
  return WIDE_LEFT_ATTACK.has(cardRole) || WIDE_RIGHT_ATTACK.has(cardRole)
}

function wideFlankZone(cardRole) {
  if (!allowsWideFlankComparison(cardRole)) return null
  if (WIDE_LEFT_ATTACK.has(cardRole)) return 'left_wide'
  if (WIDE_RIGHT_ATTACK.has(cardRole)) return 'right_wide'
  return null
}

function playerInWideFlankOnPitch(player, zone, formation = null) {
  const role = effectiveFieldRole(player, formation)
  if (zone === 'left_wide') return WIDE_LEFT_ATTACK.has(role)
  if (zone === 'right_wide') return WIDE_RIGHT_ATTACK.has(role)
  return false
}

function sortStarters(candidates, cardPosition, cardFamily, formation = null) {
  return [...candidates].sort((a, b) => {
    const fieldA =
      effectiveFieldRole(a, formation) === cardPosition ? 2 : playerInFamilyOnPitch(a, cardFamily, formation) ? 1 : 0
    const fieldB =
      effectiveFieldRole(b, formation) === cardPosition ? 2 : playerInFamilyOnPitch(b, cardFamily, formation) ? 1 : 0
    if (fieldA !== fieldB) return fieldB - fieldA
    const slotA = Number(a?.slot_index)
    const slotB = Number(b?.slot_index)
    if (Number.isFinite(slotA) && Number.isFinite(slotB)) return slotA - slotB
    return String(a.player_name || '').localeCompare(String(b.player_name || ''))
  })
}

function pickStat(stats = {}, keys = []) {
  const entries = Object.entries(stats || {})
  const values = []
  for (const [key, value] of entries) {
    const normalized = toAscii(key)
    if (!keys.some((k) => normalized.includes(k))) continue
    const numeric = Number(value)
    if (Number.isFinite(numeric) && numeric > 0) values.push(numeric)
  }
  if (!values.length) return null
  return Math.round(values.reduce((sum, value) => sum + value, 0) / values.length)
}

function roleFitScore(player, targetRole, targetFamily, formation = null) {
  const fieldRole = effectiveFieldRole(player, formation)
  const cardRole = rosterRole(player)
  let score = 0
  if (fieldRole === targetRole) score += 50
  else if (playerInFamilyOnPitch(player, targetFamily, formation)) score += 25
  if (cardRole && cardRole === fieldRole) score += 12
  else if (cardRole && cardRole !== fieldRole) score -= 10

  const stats = player?.base_stats || {}
  if (targetFamily === 'def') {
    score += pickStat(stats, ['defen', 'tackl', 'intercept', 'aggression']) || 0
    score += (pickStat(stats, ['phys', 'contact', 'strength']) || 0) * 0.35
    score += (pickStat(stats, ['speed', 'accel', 'pace']) || 0) * 0.25
  } else if (targetFamily === 'gk') {
    score += pickStat(stats, ['gk', 'catch', 'parry', 'reflex', 'reach']) || 0
  } else if (targetFamily === 'mid') {
    score += pickStat(stats, ['pass', 'control', 'dribbl']) || 0
    score += (pickStat(stats, ['defen', 'tackl', 'intercept']) || 0) * 0.35
  } else {
    score += pickStat(stats, ['offens', 'finish', 'shot', 'dribbl']) || 0
    score += (pickStat(stats, ['speed', 'accel', 'pace']) || 0) * 0.3
  }

  const skillCount = [
    ...(Array.isArray(player?.skills) ? player.skills : []),
    ...(Array.isArray(player?.com_skills) ? player.com_skills : [])
  ].filter(Boolean).length
  score += Math.min(skillCount, 10) * 1.5
  return score
}

function buildRolePoolContext(starters = [], cardRole = '', cardFamily = '', formation = null, stylesLookup = {}, lang = 'it') {
  const sameRole = starters.filter((p) => effectiveFieldRole(p, formation) === cardRole)
  const sameLine = starters.filter((p) => playerInFamilyOnPitch(p, cardFamily, formation))
  const candidates = sameRole.length ? sameRole : sameLine
  const ranked = candidates
    .map((player) => {
      const fieldRole = effectiveFieldRole(player, formation)
      const cardSheetRole = rosterRole(player)
      return {
        player,
        label: formatAnchorLabel(player, formation),
        fieldRole,
        cardSheetRole,
        style: playerStyleName(player, stylesLookup),
        outOfPosition: Boolean(cardSheetRole && fieldRole && cardSheetRole !== fieldRole),
        score: roleFitScore(player, cardRole, cardFamily, formation)
      }
    })
    .sort((a, b) => a.score - b.score)

  const replacementCandidate = ranked[0] || null
  const poolLabels = ranked
    .map((entry) => {
      const style = entry.style ? `, stile ${entry.style}` : ''
      const mismatch = entry.outOfPosition
        ? lang === 'en'
          ? `, card role ${entry.cardSheetRole}`
          : lang === 'es'
            ? `, rol ficha ${entry.cardSheetRole}`
            : `, scheda ${entry.cardSheetRole}`
        : ''
      return `${entry.label}${style}${mismatch}`
    })
    .filter(Boolean)

  return {
    labels: poolLabels,
    replacementCandidate,
    count: ranked.length,
    basis: sameRole.length ? 'same_role' : sameLine.length ? 'same_line' : 'none'
  }
}

function lineupMismatchNotes(starters, formation, lang) {
  if (!formation?.slot_positions) return ''
  const isEn = lang === 'en'
  const mismatches = starters
    .map((p) => {
      const onPitch = effectiveFieldRole(p, formation)
      const roster = rosterRole(p)
      if (!onPitch || !roster || onPitch === roster) return null
      return `${p.player_name}: ${roster} scheda → ${onPitch} in campo`
    })
    .filter(Boolean)
  if (!mismatches.length) return ''
  return isEn
    ? `Roster vs saved layout: ${mismatches.join('; ')}.`
    : lang === 'es'
      ? `Ficha vs formación guardada: ${mismatches.join('; ')}.`
      : `Scheda vs modulo salvato: ${mismatches.join('; ')}.`
}

/**
 * @returns {{
 *   type: 'same_name'|'same_field_role'|'same_family'|'same_wide_flank'|'natural_competence_only'|'none',
 *   player: object|null,
 *   cardRole: string,
 *   fieldRole: string|null,
 *   displayLabel: string,
 *   namingRule: string,
 *   compareFamily: string
 * }}
 */
export function pickComparisonAnchor(players = [], cardPosition = '', cardName = '', formation = null) {
  const cardRole = String(cardPosition || '').trim().toUpperCase()
  const cardFamily = roleFamily(cardRole)
  const starters = (players || []).filter((p) => p?.player_name && isStarter(p))

  if (!cardRole || starters.length === 0) {
    return {
      type: 'none',
      player: null,
      cardRole,
      fieldRole: null,
      displayLabel: '',
      namingRule: '',
      compareFamily: cardFamily
    }
  }

  const sameName = findSameNameStarter(starters, cardName)
  if (sameName && playerInFamilyOnPitch(sameName, cardFamily, formation)) {
    const role = effectiveFieldRole(sameName, formation)
    return {
      type: 'same_name',
      player: sameName,
      cardRole,
      fieldRole: role,
      displayLabel: formatAnchorLabel(sameName, formation),
      namingRule: `Same player already in starting XI (${role} on saved layout). Use ${formatAnchorLabel(sameName, formation)} as the technical skill anchor; purchase decision still depends on concrete new tools, movement, body type and rotation value.`,
      compareFamily: cardFamily
    }
  }

  const sameField = starters.filter((p) => effectiveFieldRole(p, formation) === cardRole)
  if (sameField.length) {
    const player = sortStarters(sameField, cardRole, cardFamily, formation)[0]
    const role = effectiveFieldRole(player, formation)
    return {
      type: 'same_field_role',
      player,
      cardRole,
      fieldRole: role,
      displayLabel: formatAnchorLabel(player, formation),
      namingRule: `Technical skill anchor: "${formatAnchorLabel(player, formation)}" — same role on the saved formation as the pack card (${cardRole}). Do not use this alone as the purchase verdict; evaluate the full role pool and the most replaceable slot.`,
      compareFamily: cardFamily
    }
  }

  const zone = wideFlankZone(cardRole)
  if (zone) {
    const inZone = starters.filter((p) => playerInWideFlankOnPitch(p, zone, formation))
    if (inZone.length) {
      const player = sortStarters(inZone, cardRole, cardFamily, formation)[0]
      const role = effectiveFieldRole(player, formation)
      return {
        type: 'same_wide_flank',
        player,
        cardRole,
        fieldRole: role,
        displayLabel: formatAnchorLabel(player, formation),
        namingRule:
          `Wide-band technical reference from saved layout: ${formatAnchorLabel(player, formation)}. Compare pack ${cardRole} to this flank starter for skills — not a central striker from another role. Decide purchase from hierarchy and role pool.`,
        compareFamily: cardFamily
      }
    }
  }

  const naturalOnly = starters.filter(
    (p) =>
      playerHasNaturalAt(p, cardRole) &&
      effectiveFieldRole(p, formation) !== cardRole &&
      playerInFamilyOnPitch(p, cardFamily, formation)
  )
  if (naturalOnly.length) {
    const player = sortStarters(naturalOnly, cardRole, cardFamily, formation)[0]
    const role = effectiveFieldRole(player, formation)
    return {
      type: 'natural_competence_only',
      player,
      cardRole,
      fieldRole: role,
      displayLabel: formatAnchorLabel(player, formation),
      namingRule:
        `NEVER write "${player.player_name} ${cardRole}" as field starter. On layout: ${role}; natural competence includes ${cardRole} on the card only. Use this as a technical fallback, not as the only purchase comparison.`,
      compareFamily: cardFamily
    }
  }

  const sameFamilyStarters = starters.filter((p) => playerInFamilyOnPitch(p, cardFamily, formation))
  if (sameFamilyStarters.length) {
    const player = sortStarters(sameFamilyStarters, cardRole, cardFamily, formation)[0]
    const role = effectiveFieldRole(player, formation)
    const familyLabel =
      cardFamily === 'def'
        ? 'difensore in rosa'
        : cardFamily === 'mid'
          ? 'centrocampista in rosa'
          : cardFamily === 'att'
            ? 'attaccante in rosa'
            : 'portiere in rosa'
    return {
      type: 'same_family',
      player,
      cardRole,
      fieldRole: role,
      displayLabel: formatAnchorLabel(player, formation),
      namingRule:
        `Fallback technical anchor — no starter in pack role ${cardRole} on saved layout: compare within the same line to ${formatAnchorLabel(player, formation)} (${familyLabel}). Do not compare to another line (def vs att). Decide purchase from the line pool and match plan.`,
      compareFamily: cardFamily
    }
  }

  return {
    type: 'none',
    player: null,
    cardRole,
    fieldRole: null,
    displayLabel: '',
    namingRule: `No starter in the same line (${cardFamily}) for role ${cardRole} on saved layout. Do not compare to players from another line (def vs att).`,
    compareFamily: cardFamily
  }
}

function summarizePassingPattern(gameAnalysis, lang) {
  const passing = gameAnalysis?.stats?.passing
  if (!passing || typeof passing !== 'object') return ''
  const entries = Object.entries(passing)
    .map(([key, value]) => ({ key: toAscii(key), value: Number(value) || 0 }))
    .filter((e) => e.value > 0)
  if (!entries.length) return ''
  const crossKeys = ['cross', 'corner']
  const crossTotal = entries.filter((e) => crossKeys.some((k) => e.key.includes(k))).reduce((s, e) => s + e.value, 0)
  const throughTotal = entries
    .filter((e) => e.key.includes('filtrant') || e.key.includes('through'))
    .reduce((s, e) => s + e.value, 0)
  if (lang === 'en') {
    if (crossTotal > 0 && crossTotal <= throughTotal) return 'Game stats: few crosses relative to through/ground passes.'
    if (throughTotal > crossTotal) return 'Game stats: through balls and ground passes dominate over crosses.'
  } else if (lang === 'es') {
    if (crossTotal > 0 && crossTotal <= throughTotal) return 'Estadísticas de juego: pocos centros respecto a pases filtrados/rasos.'
    if (throughTotal > crossTotal) return 'Estadísticas de juego: dominan los pases filtrados y rasos sobre los centros.'
  } else {
    if (crossTotal > 0 && crossTotal <= throughTotal) return 'Statistiche gioco: pochi cross rispetto a filtranti/rasoterra.'
    if (throughTotal > crossTotal) return 'Statistiche gioco: prevalgono filtranti e passaggi rasoterra sui cross.'
  }
  return ''
}

function buildDispositionLine(players, formation, lang) {
  const starters = (players || [])
    .filter(isStarter)
    .sort((a, b) => (parseSlotIndex(a.slot_index) ?? 99) - (parseSlotIndex(b.slot_index) ?? 99))
  const positions = starters.map((p) => effectiveFieldRole(p, formation) || '?').join(', ')
  const mod = formation?.formation ? String(formation.formation).trim() : ''
  const layoutNote = formation?.slot_positions
    ? ''
    : lang === 'en'
      ? ' (layout positions missing — using roster card roles)'
      : lang === 'es'
        ? ' (formación sin slot_positions — roles de ficha)'
        : ' (modulo senza slot_positions — ruoli da scheda)'
  if (lang === 'en') {
    return mod
      ? `Saved formation: ${mod}. On field: ${positions || 'not set'}.${layoutNote}`
      : `On field: ${positions || 'not set'}.${layoutNote}`
  }
  if (lang === 'es') {
    return mod
      ? `Formación guardada: ${mod}. En campo: ${positions || 'no establecida'}.${layoutNote}`
      : `En campo: ${positions || 'no establecida'}.${layoutNote}`
  }
  return mod
    ? `Modulo salvato: ${mod}. In campo: ${positions || 'non impostata'}.${layoutNote}`
    : `In campo: ${positions || 'non impostata'}.${layoutNote}`
}

function familyLabelIt(family) {
  if (family === 'def') return 'difensori'
  if (family === 'mid') return 'centrocampo'
  if (family === 'att') return 'attacco'
  if (family === 'gk') return 'portieri'
  return 'rosa'
}

export function isPremiumCatalogCard(card = {}, catalogCard = null) {
  const cat = toAscii(
    [
      card?.category,
      card?.card_type,
      card?.card_category,
      catalogCard?.category,
      catalogCard?.card_type,
      catalogCard?.card_category,
      catalogCard?.players_payload?.category,
      catalogCard?.players_payload?.card_type
    ].filter(Boolean).join(' ')
  )
  return /(epic|legendary|leggendar|show time|showtime|big time)/.test(cat)
}

function playerStyleName(player, stylesLookup = {}) {
  return (
    (player?.playing_style_id && stylesLookup[player.playing_style_id])
    || player?.role
    || ''
  ).trim()
}

/** Titolari nello stesso ruolo in campo (es. 2 CF in 3-3-4). */
export function summarizeRotationPool(starters = [], cardRole = '', formation = null, stylesLookup = {}, cardStyle = '', lang = 'it') {
  const isEn = lang === 'en'
  const role = String(cardRole || '').trim().toUpperCase()
  const onRole = starters.filter((p) => effectiveFieldRole(p, formation) === role)
  const cardStyleKey = toAscii(cardStyle)
  const sameStyle = onRole.filter((p) => {
    const ps = toAscii(playerStyleName(p, stylesLookup))
    return cardStyleKey && ps && (ps === cardStyleKey || ps.includes(cardStyleKey) || cardStyleKey.includes(ps))
  })
  const names = onRole.map((p) => formatAnchorLabel(p, formation)).filter(Boolean)
  return {
    countOnRole: onRole.length,
    sameStyleCount: sameStyle.length,
    names,
    crowdedLane: onRole.length >= 2
  }
}

/**
 * Blocco testuale vincolante per il prompt (mantiene il JSON contesto esistente).
 */
export function buildPurchaseFactsBlock({
  card,
  catalogCard = null,
  players = [],
  formation = null,
  profile = {},
  gameAnalysis = null,
  patterns = {},
  stylesLookup = {},
  lang = 'it'
} = {}) {
  const isEn = lang === 'en'
  const isEs = lang === 'es'
  const cardRole = String(card?.position || '').trim().toUpperCase()
  const cardFamily = roleFamily(cardRole)
  const cardStyle = String(card?.style || catalogCard?.playing_style || '').trim()
  const premiumCard = isPremiumCatalogCard(card, catalogCard)
  const anchor = pickComparisonAnchor(players, cardRole, card?.name, formation)
  const starters = (players || []).filter(isStarter)
  const hasStarterAtCardRole = starters.some((p) => effectiveFieldRole(p, formation) === cardRole)
  const rotationPool = summarizeRotationPool(starters, cardRole, formation, stylesLookup, cardStyle, lang)
  const rolePool = buildRolePoolContext(starters, cardRole, cardFamily, formation, stylesLookup, lang)
  const disposition = buildDispositionLine(players, formation, lang)
  const mismatchNote = lineupMismatchNotes(starters, formation, lang)
  const passPattern = summarizePassingPattern(gameAnalysis, lang)
  const weakPoint = profile?.ai_weak_point ? String(profile.ai_weak_point).trim() : ''
  const matchCount = Number(patterns?.last_50_matches_count) || 0

  const lines = []
  lines.push(isEn ? '--- PURCHASE FACTS (binding — do not contradict) ---' : isEs ? '--- HECHOS DE COMPRA (vinculantes — no contradecir) ---' : '--- FATTI ACQUISTO (vincolanti — non contraddire) ---')
  lines.push(disposition)
  if (mismatchNote) lines.push(mismatchNote)
  lines.push(
    isEn
      ? `Pack card: ${card?.name || '?'}, role ${cardRole || '?'}, style ${card?.style || 'n/a'}, line ${cardFamily}.`
      : isEs
        ? `Carta pack: ${card?.name || '?'}, rol ${cardRole || '?'}, estilo ${card?.style || 'n/d'}, línea ${familyLabelIt(cardFamily)}.`
        : `Carta pack: ${card?.name || '?'}, ruolo ${cardRole || '?'}, stile ${card?.style || 'n/d'}, reparto ${familyLabelIt(cardFamily)}.`
  )
  lines.push(
    isEn
      ? `Skill comparison rule: ONLY same line (${cardFamily}) — defenders vs defenders, midfield vs midfield, attack vs attack. Teammate synergies go in "synergies", NOT by comparing a defender to an attacker.`
      : isEs
        ? `Regla comparación skills: SOLO misma línea (${familyLabelIt(cardFamily)}) — defensas vs defensas, mediocampo vs mediocampo, ataque vs ataque. Sinergias con compañeros en "synergies", NO comparando un defensa con un atacante.`
        : `Regola confronto skill: SOLO stesso reparto (${familyLabelIt(cardFamily)}) — difensori vs difensori, centrocampo vs centrocampo, attacco vs attacco. Sinergie con compagni in "synergies", NON paragonando un difensore a un attaccante.`
  )
  lines.push(
    isEn
      ? 'Decision principle: compare this pack card against the customer roster first. Decide the role hierarchy and the most replaceable player before using the technical skill anchor. A favorable read is allowed only when it changes role coverage, rotation, movement, special tools, body type or TODAY match plan — never raw rating.'
      : isEs
        ? 'Principio de decisión: compara primero esta carta pack con la plantilla del cliente. Decide jerarquía de rol y jugador más reemplazable antes de usar el anchor técnico de skills. Lectura favorable solo si cambia cobertura de rol, rotación, movimiento, herramientas especiales, body type o plan de partido DE HOY — nunca rating bruto.'
        : 'Principio decisionale: confronta prima questa carta pack con la rosa del cliente. Decidi gerarchia ruolo e giocatore più sostituibile prima di usare l\u2019anchor tecnico skill. Lettura favorevole solo se cambia copertura ruolo, rotazione, movimento, tool speciali, body type o piano partita DI OGGI — mai rating grezzo.'
  )
  lines.push(
    hasStarterAtCardRole
      ? isEn
        ? `Starter in same field role ${cardRole}: yes.`
        : isEs
          ? `Titular con mismo rol en campo (${cardRole}): sí.`
          : `Titolare con stesso ruolo in campo (${cardRole}): sì.`
      : isEn
        ? `Starter in same field role ${cardRole}: none.`
        : isEs
          ? `Titular con mismo rol en campo (${cardRole}): ninguno.`
          : `Titolare con stesso ruolo in campo (${cardRole}): nessuno.`
  )

  if (['same_name', 'same_field_role', 'same_family', 'same_wide_flank', 'natural_competence_only'].includes(anchor.type)) {
    const typeNote =
      anchor.type === 'same_family'
        ? isEn
          ? ' (fallback — same line, no pack role on saved layout)'
          : isEs
            ? ' (fallback — misma línea, sin titular en el rol pack en la formación)'
            : ' (fallback — stesso reparto, nessun titolare nel ruolo pack sul modulo)'
        : anchor.type === 'same_wide_flank'
          ? isEn
            ? ' (wide attack/mid only)'
            : isEs
              ? ' (solo banda ofensiva)'
              : ' (solo fascia offensiva)'
          : anchor.type === 'natural_competence_only'
            ? isEn
              ? ' (natural competence — different field role)'
              : isEs
                ? ' (competencia natural — rol en campo diferente)'
                : ' (competenza naturale — ruolo in campo diverso)'
            : ''
    lines.push(
      isEn
        ? `Official technical skill anchor: ${anchor.displayLabel}${typeNote}. This is not automatically the purchase blocker or the player to replace.`
        : isEs
          ? `Anchor técnico oficial de skills: ${anchor.displayLabel}${typeNote}. No es automáticamente el bloqueo de compra ni el jugador a reemplazar.`
          : `Anchor tecnico skill ufficiale: ${anchor.displayLabel}${typeNote}. Non è automaticamente il blocco acquisto né il giocatore da sostituire.`
    )
    lines.push(anchor.namingRule)
    if (anchor.type !== 'same_name') {
      lines.push(
        isEn
          ? 'Roster anchor rule: this player is a comparison reference, not a purchase blocker. Use the anchor to explain what the pack card adds to the squad: rotation, different movement, side/foot, card-only skills, body type or match plan.'
          : isEs
            ? 'Regla anchor plantilla: este jugador es referencia de comparación, no bloqueo de compra. Usa el anchor para explicar qué aporta la carta pack a la plantilla: rotación, movimiento diferente, lado/pie, skills solo carta, body type o plan de partido.'
            : 'Regola anchor rosa: questo giocatore è riferimento di confronto, non blocco acquisto. Usa l\u2019anchor per spiegare cosa la carta pack aggiunge alla rosa: rotazione, movimento diverso, lato/piede, skill solo carta, body type o piano partita.'
      )
    }
    if (anchor.type === 'same_name') {
      lines.push(
        isEn
          ? 'eFootball squad rule: same player name/version family already in the XI cannot be used together as rotation. This is a replace-or-keep decision: say the new card replaces the current version if the upgrade is concrete, otherwise use skip_duplicate/not worth changing. Never recommend keeping both in the same squad or rotating both Yamals/Messis/etc.'
          : isEs
            ? 'Regla plantilla eFootball: mismo nombre/familia de jugador ya en los 11 no puede usarse junto como rotación. Es una decisión reemplazar-o-mantener: escribe que la nueva carta reemplaza la versión actual si la mejora es concreta, si no usa skip_duplicate/no vale cambiarla. Nunca recomiendes mantener ambas o rotar dos Yamal/Messi/etc.'
            : 'Regola rosa eFootball: stesso nome/famiglia giocatore già negli 11 non può essere usato insieme come rotazione. È una decisione sostituisci-o-tieni: scrivi che la nuova carta sostituisce la versione attuale se l\u2019upgrade è concreto, altrimenti usa skip_duplicate/non vale cambiarla. Mai consigliare di tenere entrambe o ruotare due Yamal/Messi/ecc.'
      )
    }
    if (anchor.type === 'same_wide_flank') {
      lines.push(
        isEn
          ? 'Wide attacker rule: if the pack card brings 1v1, pace, special dribbling/curler tools, side-foot value or a different way to attack the lane, decide hierarchy first. If it is the better Plan A, say it starts over the current wide starter; use premium rotation only when it is truly a bench/match-plan option.'
          : isEs
            ? 'Regla banda ofensiva: si la carta pack aporta 1v1, velocidad, regate/tiro especial, valor lado-pie o forma diferente de atacar el carril, decide primero la jerarquía. Si es el plan A mejor, escribe que es titular sobre el extremo actual; usa rotación premium solo si es realmente opción banquillo/plan partido.'
            : 'Regola fascia offensiva: se la carta pack porta 1v1, velocità, dribbling/tiro speciale, valore lato-piede o modo diverso di attaccare la corsia, decidi prima la gerarchia. Se è il piano A migliore, scrivi che parte sopra il titolare largo attuale; usa rotazione premium solo se è davvero opzione panchina/piano partita.'
      )
    }
    if (anchor.type === 'natural_competence_only') {
      lines.push(
        isEn
          ? 'Purchase rule: pack role differs from field role — for premium cards, prefer fits_if_formation_change or fits_with_rotation with a clear setup condition when the card has concrete tools. Do not default to a cold verdict only because the saved layout uses a different role.'
          : isEs
            ? 'Regla compra: rol pack distinto del rol en campo — para cartas premium, prefiere fits_if_formation_change o fits_with_rotation con condición clara cuando la carta tiene herramientas concretas. No enfríes el veredicto solo porque la formación guardada usa un rol diferente.'
            : 'Regola acquisto: ruolo pack diverso dal ruolo in campo — per carte premium preferisci fits_if_formation_change o fits_with_rotation con condizione chiara quando la carta ha tool concreti. Non raffreddare il verdetto solo perché il modulo salvato usa un ruolo diverso.'
      )
    }
  } else {
    lines.push(
      isEn
        ? `Official skill comparison anchor: none in line ${cardFamily} for role ${cardRole}.`
        : isEs
          ? `Referencia comparación skills: ninguno en la línea ${familyLabelIt(cardFamily)} para rol ${cardRole}.`
          : `Riferimento confronto skill: nessuno nel reparto ${familyLabelIt(cardFamily)} per ruolo ${cardRole}.`
    )
    lines.push(
      isEn
        ? 'Do not compare to players from another line (e.g. no Ronaldinho for a defender card).'
        : isEs
          ? 'No compares con jugadores de otra línea (ej. nada de Ronaldinho para una carta defensiva).'
          : 'Non confrontare con giocatori di altro reparto (es. niente Ronaldinho per una carta difensiva).'
    )
  }

  if (rolePool.count > 0) {
    lines.push(
      isEn
        ? `Role pool to evaluate before verdict (${rolePool.basis}): ${rolePool.labels.join('; ')}.`
        : isEs
          ? `Pool de roles a evaluar antes del veredicto (${rolePool.basis}): ${rolePool.labels.join('; ')}.`
          : `Pool ruolo da valutare prima del verdetto (${rolePool.basis}): ${rolePool.labels.join('; ')}.`
    )
    if (rolePool.replacementCandidate) {
      lines.push(
        isEn
          ? `Most replaceable current option by role fit/profile, not raw rating: ${rolePool.replacementCandidate.label}. If the pack card offers a better movement, style fit, defensive/offensive tool or match-plan role than this player, frame the verdict around replacing/upgrading this slot — not around beating the best player in the line.`
          : isEs
            ? `Opción actual más reemplazable por encaje rol/perfil, no rating bruto: ${rolePool.replacementCandidate.label}. Si la carta pack ofrece movimiento, estilo, herramienta defensiva/ofensiva o plan de partido mejor que este jugador, centra el veredicto en reemplazar/mejorar este slot — no en superar al mejor de la línea.`
            : `Opzione attuale più sostituibile per fit ruolo/profilo, non rating grezzo: ${rolePool.replacementCandidate.label}. Se la carta pack offre movimento, stile, tool difensivo/offensivo o piano partita migliore di questo giocatore, centra il verdetto su upgrade/sostituzione di questo slot — non sul battere il migliore del reparto.`
      )
    }
  }

  if (!hasStarterAtCardRole && cardRole) {
    lines.push(
      isEn
        ? `Formation gap: no current ${cardRole} starter, but this is not a negative for premium cards. If the card has concrete tools, frame it as premium rotation with formation change, dedicated slot, or bench use.`
        : isEs
          ? `Vacío de formación: ningún titular ${cardRole} actual, pero no es negativo para cartas premium. Si la carta tiene herramientas concretas, léela como rotación premium con cambio de formación, slot dedicado o uso de banquillo.`
          : `Gap modulo: nessun titolare ${cardRole} attuale, ma non è negativo per carte premium. Se la carta ha tool concreti, leggila come rotazione premium con cambio modulo, slot dedicato o uso dalla panchina.`
    )
  }

  if (rotationPool.crowdedLane) {
    lines.push(
      isEn
        ? `Rotation pool context, not a penalty: ${rotationPool.countOnRole} starters already in ${cardRole} on field (${rotationPool.names.join(', ') || 'n/a'}). Judge whether this card adds a different match plan or bench rotation.`
        : isEs
          ? `Contexto pool rotación, no penalización: ${rotationPool.countOnRole} titulares ya en ${cardRole} en campo (${rotationPool.names.join(', ') || 'n/d'}). Valora si esta carta añade plan de partido diferente o rotación de banquillo.`
          : `Contesto pool rotazione, non penalità: ${rotationPool.countOnRole} titolari già in ${cardRole} in campo (${rotationPool.names.join(', ') || 'n/d'}). Valuta se questa carta aggiunge piano partita diverso o rotazione dalla panchina.`
    )
  }

  if (premiumCard && hasStarterAtCardRole && anchor.type !== 'same_name') {
    lines.push(
      isEn
        ? 'Premium card in a covered role: default verdict premium_rotation or fits_with_rotation when pros list distinct card-only skills, body type or match-plan use. Forbidden: final_decision "not today" / "do not buy" only because another starter shares the same playing style — same Goal Poacher ≠ automatic duplicate; compare dribbling, finishing tools, connection lag and rotation value.'
        : isEs
          ? 'Carta premium en rol ya cubierto: veredicto por defecto premium_rotation o fits_with_rotation si los pros citan skills solo carta, body type o plan de partido. Prohibido: final_decision "hoy no" / "no comprar" solo porque otro titular comparte el mismo estilo — mismo Opportunista ≠ duplicado automático; compara regate, herramientas de finalización, lag y valor de rotación.'
          : 'Carta premium in ruolo già coperto: verdetto di default premium_rotation o fits_with_rotation se i pro citano skill solo carta, body type o piano partita. Vietato: final_decision "Oggi no" / "non comprare" solo perché un altro titolare ha lo stesso stile — stesso Opportunista ≠ doppione automatico; confronta dribbling, strumenti in finalizzazione, lag e valore rotazione.'
    )
    if (rotationPool.crowdedLane && rotationPool.sameStyleCount >= 1) {
      lines.push(
        isEn
          ? `Same movement style as ${rotationPool.sameStyleCount} current ${cardRole} starter(s): still valid as elite rotation — alternate by match plan (depth vs box presence), not as a forced weekly starter.`
          : isEs
            ? `Mismo estilo de movimiento que ${rotationPool.sameStyleCount} titular(es) ${cardRole}: sigue siendo válido como rotación de élite — alterna por plan de partido (profundidad vs presencia en área), no como titular fijo obligatorio.`
            : `Stesso stile di movimento di ${rotationPool.sameStyleCount} titolare/i ${cardRole}: resta valida come rotazione d'élite — alterna per piano partita (profondità vs presenza in area), non come titolare fisso obbligatorio.`
      )
    }
  }

  if (premiumCard && cardFamily === 'gk') {
    lines.push(
      isEn
        ? 'Premium goalkeeper rule: do not call the card unnecessary just because the customer already has a goalkeeper. A new premium GK is at least a premium_rotation case when it offers reliability, reach/body, close saves, rebound control, distribution or form coverage.'
        : isEs
          ? 'Regla portero premium: no digas que la carta no es necesaria solo porque el cliente ya tiene un portero. Un nuevo PT premium es al menos premium_rotation cuando ofrece fiabilidad, alcance/cuerpo, paradas cercanas, control de rebotes, distribución o cobertura de forma.'
          : 'Regola portiere premium: non dire che la carta non è necessaria solo perché il cliente ha già un portiere. Un nuovo PT premium è almeno premium_rotation quando offre affidabilità, reach/body, parate ravvicinate, controllo rimbalzi, distribuzione o copertura forma.'
    )
  }

  if (passPattern) lines.push(passPattern)
  if (weakPoint) {
    lines.push(isEn ? `Profile weak point: ${weakPoint}.` : isEs ? `Punto débil del perfil: ${weakPoint}.` : `Punto debole profilo: ${weakPoint}.`)
  }
  if (matchCount < 3) {
    lines.push(
      isEn
        ? 'Low match sample: keep wording honest, but do not downgrade a premium card with concrete tools; explain the rotation/match-plan case.'
        : isEs
          ? 'Muestra baja de partidos: sé honesto en el texto, pero no rebajes una premium con herramientas concretas; explica el caso rotación/plan partido.'
          : 'Poche partite in archivio: resta onesto nel testo, ma non abbassare una premium con tool concreti; spiega il caso rotazione/piano partita.'
    )
  }
  if (!(players || []).length) {
    lines.push(
      isEn
        ? 'No roster: card-only review, purchase_fit should be insufficient_data.'
        : isEs
          ? 'Sin plantilla: solo review de carta, purchase_fit = insufficient_data.'
          : 'Rosa assente: solo review carta, purchase_fit = insufficient_data.'
    )
  }
  lines.push(isEn ? '--- END PURCHASE FACTS ---' : isEs ? '--- FIN HECHOS DE COMPRA ---' : '--- FINE FATTI ACQUISTO ---')

  return {
    text: lines.join('\n'),
    anchor,
    hasStarterAtCardRole,
    premiumCard,
    rotationPool,
    rolePool
  }
}

export function normalizePurchaseFit(value) {
  const key = String(value || '').trim()
  return VALID_PURCHASE_FIT.has(key) ? key : ''
}
