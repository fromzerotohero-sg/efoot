...
function buildProfileSummary(profile, lang) {
  ...
  const lines = []
  if (profile.first_name) lines.push(`${(lang === 'en' || lang === 'es') ? 'Player' : 'Cliente'}: ${sanitize(profile.first_name, 40)}`)
  if (profile.team_name) lines.push(`${(lang === 'en' || lang === 'es') ? 'Team name' : 'Nome squadra'}: ${sanitize(profile.team_name, 60)}`)
  if (profile.current_division) lines.push(`${(lang === 'en' || lang === 'es') ? 'Division' : 'Divisione'}: ${sanitize(profile.current_division, 40)}`)
  if (profile.ai_weak_point) lines.push(`${(lang === 'en' || lang === 'es') ? 'Known weak point' : 'Punto debole noto'}: ${sanitize(profile.ai_weak_point, 80)}`)
  if (profile.ai_learn_goals) lines.push(`${(lang === 'en' || lang === 'es') ? 'Learning goal' : 'Cosa vuole imparare'}: ${sanitize(profile.ai_learn_goals, 140)}`)
  if (profile.platform) lines.push(`${(lang === 'en' || lang === 'es') ? 'Platform' : 'Piattaforma'}: ${sanitize(profile.platform, 30)}`)
...
function buildMatchesSummary(matches, lang) {
  if (!Array.isArray(matches) || matches.length === 0) {
    return (lang === 'en' || lang === 'es') ? 'No match history available.' : 'Nessuno storico partite disponibile.'
  }
...
function buildPatternSummary(patterns, lang) {
  if (!patterns || typeof patterns !== 'object') {
    return (lang === 'en' || lang === 'es') ? 'No tactical pattern data available.' : 'Nessun pattern tattico disponibile.'
  }
  ...
  if (patterns.formation_usage && typeof patterns.formation_usage === 'object') {
    ...
    if (topFormations.length > 0) {
      lines.push(`${(lang === 'en' || lang === 'es') ? 'Used formations' : 'Formazioni usate'}: ${topFormations.join(', ')}`)
    }
  }

  return lines.length > 0
    ? lines.join('\n')
    : ((lang === 'en' || lang === 'es') ? 'No tactical pattern data available.' : 'Nessun pattern tattico disponibile.')
}
...
function buildGameAnalysisSummary(gameAnalysis, lang) {
  ...
  return `${(lang === 'en' || lang === 'es') ? 'Gameplay analysis' : 'Analisi di gioco'}:\n${parts.join('\n')}`
}
...
function buildSmartRosterSummary(context, lang) {
  ...
  const header = [
    `${(lang === 'en' || lang === 'es') ? 'Formation' : 'Modulo'}: ${sanitize(context?.formation || ((lang === 'en' || lang === 'es') ? 'not detected' : 'non rilevato'), 30)}`,
    `${(lang === 'en' || lang === 'es') ? 'Detected starters' : 'Titolari rilevati'}: ${players.length}`,
    `${(lang === 'en' || lang === 'es') ? 'Coach detected' : 'Allenatore rilevato'}: ${coach?.coach_name ? sanitize(coach.coach_name, 60) : ((lang === 'en' || lang === 'es') ? 'no' : 'no')}`
  ]

  const playerLines = players.map((player, index) => {
    const role = sanitize(player.position || '?', 12)
    const rating = player.overall_rating != null ? `OVR ${player.overall_rating}` : ((lang === 'en' || lang === 'es') ? 'OVR n/a' : 'OVR n/d')
