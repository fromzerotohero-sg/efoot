...
export async function repairSinglePlayerBuildCoachBaseline(admin, player, rosterContext, { dryRun = true } = {}) {
  ...
  let baselineNested = catalogBaselineNested
  ...
  const playProfileOverall = clampOverallRating(
    preview.progressionOverall ?? preview.playProfileOverall ?? preview.afterOverall,
    cap
  )
  const boostedProfileOverall = clampOverallRating(preview.playProfileOverall ?? preview.afterOverall, cap)
  const boostedFieldOverall = clampOverallRating(preview.fieldOverall ?? playProfileOverall, cap)
  const fieldOverall = playProfileOverall
  ...
  const patch = {
    ...
    overall_rating: playProfileOverall,
    position_ratings: {
      ...(player.position_ratings && typeof player.position_ratings === 'object' ? player.position_ratings : {}),
      [resolvedPosition]: fieldOverall,
      ...(appPosition ? { [appPosition]: fieldOverall } : {})
    },
    metadata: {
      ...
      build_coach: {
        ...
        after: {
          ...
          play_profile_overall: playProfileOverall,
          booster_profile_overall: boostedProfileOverall,
          field_overall: fieldOverall,
          boosted_field_overall: boostedFieldOverall,
          overall_rating: playProfileOverall,
          overall_decimal: afterOverallDecimal,
          effective_base_stats: effectiveNested,
          field_effective_base_stats: fieldNested,
          overall_cap: cap
        }
      },
