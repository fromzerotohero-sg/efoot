...
export async function repairSinglePlayerBuildCoachBaseline(admin, player, rosterContext, { dryRun = true } = {}) {
  ...
  const playProfileOverall = clampOverallRating(preview.playProfileOverall ?? preview.afterOverall, cap)
  const fieldOverall = clampOverallRating(preview.fieldOverall ?? playProfileOverall, cap)
