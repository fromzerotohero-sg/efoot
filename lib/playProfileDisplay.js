...
/**
 * Profilo Play da copiare/confrontare: L1 + PT.
 * Booster e coach possono alterare stats separate, ma non devono gonfiare l'OVR
 * principale che l'utente confronta nella schermata build/campo Play.
 */
export function resolvePlayProfileForPlayer(player, { slotPosition = null, catalogCard = null, coach = null, teamStyle = null } = {}) {
  ...
  if (preview && Number.isFinite(preview.progressionOverall ?? preview.playProfileOverall ?? preview.afterOverall)) {
    const stats =
      preview.finalBaseStats ||
      nestedBaselineStatsFromGameplayPreview({ baseStats: preview.finalEfhubStats }) ||
      {}
    const overall = preview.progressionOverall ?? preview.playProfileOverall ?? preview.afterOverall
...
/**
 * In campo nella UI Play: stesso OVR build/progressione. Coach/boosters sono
 * mantenuti separati per analisi, non per gonfiare l'OVR mostrato.
 */
export function resolveFieldProfileForPlayer(
  ...
  const preview = buildPreview(player, { slotPosition, catalogCard, coach, teamStyle })
  if (preview && Number.isFinite(preview.progressionOverall ?? preview.playProfileOverall ?? preview.fieldOverall)) {
    const stats =
      preview.finalBaseStats ||
      nestedBaselineStatsFromGameplayPreview({ baseStats: preview.finalEfhubStats }) ||
      {}
    return {
      stats,
      overall: preview.progressionOverall ?? preview.playProfileOverall ?? preview.fieldOverall,
      fromField: true,
      targetPosition: preview.targetPosition || null
    }
  }

  const storedPlay = player.metadata?.build_coach?.after?.play_profile_overall
  if (Number.isFinite(Number(storedPlay))) {
    return {
      stats: player.base_stats || {},
      overall: Number(storedPlay),
      fromField: false
    }
  }
