...
function preferFieldContext({ coach, teamStyle, player } = {}) {
  if (coach || teamStyle) return true
  const boosters = Array.isArray(player?.available_boosters) ? player.available_boosters : []
  return boosters.filter((entry) => String(entry?.name || '').trim()).length > 1
}
...
export function resolvePlayProfileForPlayer(player, { slotPosition = null, catalogCard = null, coach = null, teamStyle = null } = {}) {
  ...
  const useField = preferFieldContext({ coach, teamStyle, player })
  if (preview && Number.isFinite(preview.playProfileOverall ?? preview.afterOverall)) {
    const stats = useField
      ? (preview.fieldStatsNested ||
        nestedFieldStatsFromGameplayPreview(preview) ||
        {})
      : (preview.playProfileStatsNested ||
        nestedEffectiveStatsFromGameplayPreview({ finalInGameStats: preview.finalInGameStats }) ||
        {})
    const overall = useField && Number.isFinite(preview.fieldOverall)
      ? preview.fieldOverall
      : (preview.playProfileOverall ?? preview.afterOverall)
