...
/**
 * When pesdb (or catalog) stores max_level=1 but the card progresses, read initialLevelCap from EFHub live.
 */
export async function enrichCatalogCardForBuildCoach(player, catalogCard) {
  if (!catalogCard || !needsEfhubLevelCapEnrichment(player, catalogCard)) return catalogCard
  ...
  const next = { ...catalogCard, level_cap_source: 'efhub_live' }
