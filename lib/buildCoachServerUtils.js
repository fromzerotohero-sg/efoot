...
import { needsEfhubLevelCapEnrichment, resolveProgressionLevelCap } from './gameplayBuildCoach.js'
...
export async function enrichCatalogCardForBuildCoach(player, catalogCard) {
  ...
  if (!Number.isFinite(efhubCap) || efhubCap <= 1) return catalogCard

  return {
    ...catalogCard,
    max_level: Math.floor(efhubCap),
    overall_level_1: detail.overall_level_1 ?? catalogCard.overall_level_1,
    overall_max_level: detail.overall_max_level ?? catalogCard.overall_max_level,
    level_cap_source: 'efhub_live'
  }
}
