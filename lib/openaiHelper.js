...
function logOpenAIErrorDiagnostics(response, errorData, requestBody, operationType) {
  const error = errorData?.error || {}
  console.error(`[${operationType}] OpenAI response error`, {
...
export async function callOpenAIWithRetry(apiKey, requestBody, operationType = 'openai-call') {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      logOpenAIErrorDiagnostics(response, errorData, requestBody, operationType)
      ...
      if (statusCode === 429 || errorCode === 'rate_limit_exceeded') {
        if (attempt < MAX_RETRIES) {
          if (process.env.NODE_ENV !== 'production') console.log(`[${operationType}] Rate limit hit, retrying in ${RETRY_DELAY_MS.rate_limit}ms (attempt ${attempt + 1}/${MAX_RETRIES + 1})`)
