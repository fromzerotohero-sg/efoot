...
export const COM_AI_PLAYSTYLE_KEYS = new Set([
  ...
  'earlycrosser', 'earlycross', 'longranger', 'funambolo', 'serpentina',
...
export const GAME_FACTS = [
  ...
  fact('skill_training', {
    status: 'confirmed',
    source: SRC.dreamTeam,
    textIt: `Skill Training aggiunge al massimo ${MAX_ADDITIONAL_SKILLS} Additional Skills. Native e aggiuntive sono distinte. COM/AI playstyle non occupano questi slot.`,
    textEn: `Skill Training adds a maximum of ${MAX_ADDITIONAL_SKILLS} Additional Skills. Native and additional skills are distinct. COM/AI playstyles do not occupy these slots.`
  }),
  ...
  fact('fluid_formation', {
    status: 'confirmed',
    source: SRC.v600,
    textIt: 'Formazione fluida: due schemi nel Game Plan, uno in possesso e uno senza. Il passaggio non è istantaneo: i giocatori devono coprire la distanza. Consigliare, non applicare in app.',
    textEn: 'Fluid Formation: two Game Plan shapes, one in possession and one out of possession. Transition is not instant. Advise only; the app does not apply it.'
  }),
...
export function isComAiPlaystyle(raw) {
  ...
  return /funambolo|serpentina|trenoincorsa|mazingrun|speedingbullet|longranger|longballexpert|earlycross|incisiverun/.test(key)
}
...
export function collectCoachLinkUps(coachRow) {
  ...
  pushLinkUp(collected, coachRow?.connection_2)
