...
/**
 * Distingue un elenco confermato vuoto da dati mai importati.
 * `connection: null` nel catalogo non prova che la carta non possieda Link-up.
 */
export function getCoachLinkUpDataState(source) {
  if (!source || typeof source !== 'object') return 'unavailable'
  if (normalizeLinkUpPlays(source).length > 0) return 'present'

  const extracted = source.extracted_data && typeof source.extracted_data === 'object'
    ? source.extracted_data
    : {}
  const explicitlyRecorded = (
    Array.isArray(extracted.link_up_plays) ||
    Array.isArray(extracted.connections) ||
    Array.isArray(source.link_up_plays) ||
    Array.isArray(source.connection)
  )
  return explicitlyRecorded ? 'confirmed_none' : 'unavailable'
}
...
/**
 * Blocco Hero/diagnostico: Collegamenti allenatore (max 2).
 * Distingue Link-up da competenze stile. Se manca il dato, lo dice esplicitamente.
 */
export function formatCoachLinkUpsForHeroPrompt({ coach, starters = [], stylesLookup = {}, lang = 'it' } = {}) {
  if (!coach) return ''
  const dataState = getCoachLinkUpDataState(coach)
  const plays = normalizeLinkUpPlays(coach)
    .map((play) => evaluateLinkUpPlay(play, starters, stylesLookup))
    .filter(Boolean)

  const copy = lang === 'en'
    ? {
      header: 'COACH LINK-UP PLAYS (max 2, independent). These are NOT playing-style competence numbers.',
      none: 'COACH LINK-UP PLAYS: confirmed none for this manager card.',
      unavailable: 'COACH LINK-UP DATA: unavailable for this imported manager card. This does NOT mean the manager has no Link-up. Do NOT infer Link-ups from style competence numbers and do NOT invent requirements. Say that Link-ups cannot be verified from the imported data.',
      activatable: 'requirements met by current starters; verify the in-game Game Plan indicator',
      insufficient: 'insufficient data to verify',
      notActivatable: 'not activatable with these starters',
      focal: 'Focal Point',
      key: 'Key Man',
      noStarter: 'no compatible starter',
      unverifiable: 'not verifiable'
    }
    : lang === 'es'
      ? {
        header: 'LINK-UP DEL ENTRENADOR (máx. 2, independientes). NO son los números de competencia de estilo.',
        none: 'LINK-UP DEL ENTRENADOR: confirmado que esta carta no tiene ninguno.',
        unavailable: 'DATOS LINK-UP DEL ENTRENADOR: no disponibles para esta carta importada. Esto NO significa que el entrenador no tenga Link-up. NO los deduzcas de los números de competencia y no inventes requisitos. Di que no se pueden verificar con los datos importados.',
        activatable: 'requisitos cumplidos por los titulares; verifica el indicador en el Game Plan',
        insufficient: 'datos insuficientes para verificarlo',
        notActivatable: 'no activable con estos titulares',
        focal: 'Punto focal',
        key: 'Hombre clave',
        noStarter: 'ningún titular compatible',
        unverifiable: 'no verificable'
      }
      : {
        header: 'COLLEGAMENTI ALLENATORE (Link-up Play, max 2, indipendenti). NON sono i numeri di competenza stile.',
        none: 'COLLEGAMENTI ALLENATORE: confermato che questa carta non ne possiede.',
        unavailable: 'DATI COLLEGAMENTI ALLENATORE: non disponibili per questa carta importata. Questo NON significa che l\'allenatore non possieda Link-up. NON dedurli dai numeri di competenza stile e non inventare requisiti. Di\' che non sono verificabili dai dati importati.',
        activatable: 'requisiti soddisfatti dai titolari; verifica l’indicatore nel Game Plan',
        insufficient: 'dati insufficienti per verificarlo',
        notActivatable: 'non attivabile con questi titolari',
        focal: 'Punto focale',
        key: 'Uomo chiave',
        noStarter: 'nessun titolare compatibile',
        unverifiable: 'non verificabile'
      }

  if (!plays.length) return dataState === 'confirmed_none' ? copy.none : copy.unavailable
  ...
  plays.forEach((play, index) => {
    ...
    const statusLabel = status === 'activatable'
      ...
      : status === 'insufficient_data'
        ? copy.insufficient
        : copy.notActivatable
    lines.push(`- ${index + 1}. ${play.name || 'Link-up'}: ${statusLabel}`)
    if (play.focal_point) {
      const names = (play.focal_candidates || []).map((item) => item.name).filter(Boolean).join(', ')
      const match = status === 'insufficient_data' ? copy.unverifiable : (names || copy.noStarter)
      lines.push(`  ${copy.focal}: ${play.focal_point.playing_style || '-'} (${play.focal_point.position || '-'}) → ${match}`)
    }
    if (play.key_man) {
      const names = (play.key_man_candidates || []).map((item) => item.name).filter(Boolean).join(', ')
      const match = status === 'insufficient_data' ? copy.unverifiable : (names || copy.noStarter)
      lines.push(`  ${copy.key}: ${play.key_man.playing_style || '-'} (${play.key_man.position || '-'}) → ${match}`)
    }
  })
  return lines.join('\n')
}

export function prependLiveLinkUpOverride(cachedContext, liveLinkUpText, lang = 'it') {
  const live = String(liveLinkUpText || '').trim()
  const cached = String(cachedContext || '')
  if (!live) return cached
  const notice = lang === 'en'
    ? 'This LIVE Link-up block overrides any Connection/Link-up wording in the summary below.'
    : lang === 'es'
      ? 'Este bloque LIVE de Link-up prevalece sobre cualquier Connection/Link-up del resumen siguiente.'
      : 'Questo blocco LIVE dei Collegamenti prevale su qualsiasi Connection/Link-up nel riassunto sotto.'
