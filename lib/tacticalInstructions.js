...
import { getSnapshotPlayerRole } from './clientFormationSnapshot.js'
...
export function validateIndividualInstruction(category, playerId, instruction, titolari, formationSnapshot = null) {
  const config = INDIVIDUAL_INSTRUCTIONS_CONFIG[category]
  ...
  const phase = category.startsWith('difesa_') ? 'defense' : 'attack'
  const phasePlayers = (titolari || []).map((item) => ({
    ...item,
    position: getSnapshotPlayerRole(formationSnapshot, item, phase) || item.position
  }))
  const compatiblePlayers = config.filterPlayers(phasePlayers)
  if (!compatiblePlayers.find((p) => p.id === playerId)) {
    return {
      valid: false,
      error: `Player position ${getSnapshotPlayerRole(formationSnapshot, player, phase) || player.position} not compatible with ${category}`
    }
  }
