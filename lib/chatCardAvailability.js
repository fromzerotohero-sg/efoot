 ...
 * filtrata per is_active e per il gate pack valutabili.
 */
import { isEvaluableCardAdvisorRelease } from './cardAdvisorReleaseGate.js'
...
async function findActiveCardsByTokens(admin, tokens) {
  ...
  try {
    ...
    return Array.isArray(data)
      ? data.filter((row) => isEvaluableCardAdvisorRelease(row?.card_advisor_releases?.release_name))
      : []
  } catch (err) {
...
export async function buildCardAvailabilityBlock({ admin, message, lang = 'it' }) {
  try {
    ...
    if (lang === 'en') {
      ...
      out.push('DISPONIBILIDAD CARD ADVISOR (solo releases activas):')
    } else {
    ...
    if (groups.size > 0) {
      for (const [name, cards] of groups) {
        if (cards.length === 1) {
          out.push(`- "${name}" -> ${lang === 'en' ? 'AVAILABLE' : lang === 'es' ? 'DISPONIBLE' : 'DISPONIBILE'}: ${formatCardLine(cards[0])}`)
        } else {
          out.push(`- "${name}" -> ${lang === 'en' ? `AVAILABLE in ${cards.length} active versions` : lang === 'es' ? `DISPONIBLE en ${cards.length} versiones activas` : `DISPONIBILE in ${cards.length} versioni attive`}:`)
          for (const c of cards.slice(0, 6)) {
            out.push(`  * ${formatCardLine(c)}`)
          }
          if (cards.length > 6) out.push(`  * (+${cards.length - 6} ${lang === 'en' ? 'more' : lang === 'es' ? 'más' : 'altre'})`)
        }
      }
    }
    ...
    if (uncoveredTokens.length > 0) {
      if (lang === 'en') {
        ...
        out.push(`- NO EN PACKS ACTIVOS: ${uncoveredTokens.join(', ')} -> ninguna coincidencia en las releases activas de Card Advisor. El cliente no puede comprar estas cartas ahora.`)
      } else {
...
- Card NOT in active packs: be clear and direct with the customer that this card is NOT currently buyable. Do not hedge with "maybe later" or "wait for it". Suggest focusing on the cards currently available. Do not produce a buy/skip verdict on it. If the player is in their roster, switch to tactical advice on how to use them.`)
      ...
      out.push(
`REGLAS OPERATIVAS (SEGUIR SIEMPRE):
- Carta DISPONIBLE en CA activa: dile al cliente el nombre exacto de la release e invítalo a abrir "Analisi Card" para el veredicto preciso comprar/descartar. Puedes añadir una breve nota táctica sobre cómo la carta encajaría en la plantilla. No des tu propio veredicto comprar/descartar.
- Varias versiones activas del mismo jugador: enuméralas con el nombre de la release y dile al cliente que las compare en Analisi Card.
- Mismo apellido que corresponde a varios jugadores: pide al cliente que especifique cuál antes de indicar.
- Carta NO en packs activos: sé claro y directo con el cliente, esta carta NO se puede comprar ahora. Nada de "quizás más adelante" o "espera a que vuelva". Sugiere centrarse en las cartas actualmente disponibles. No produzcas un veredicto comprar/descartar sobre esa carta. Si el jugador ya está en su plantilla, pasa al consejo táctico sobre cómo usarlo.`)
    } else {
      out.push(
