...
export function computePlayerFieldOverall({
  ...
  const playStats = applyInGameEffects(afterProgression, { player })
  const fieldStats = applyFieldEffects(afterProgression, { player, coach, teamStyle })
  ...
  return {
    ...
    afterOverall: clampOverallRating(
      Math.max(playProfileOverall, coach || teamStyle ? fieldOverallRaw : playProfileOverall),
      cap
    )
  }
}
