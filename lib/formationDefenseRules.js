...
const ROLE_EQUIVALENCE_GROUPS = [
  ['PT', 'GK', 'GOALKEEPER', 'PORTIERE'],
  ['P', 'CF'],
  ['SP', 'SS'],
  ['TRQ', 'AMF'],
  ['CC', 'CMF'],
...
function equivalentRoleSet(role) {
  const normalized = normalizeRole(role)
  if (!normalized) return new Set()
  const group = ROLE_EQUIVALENCE_GROUPS.find((roles) => roles.includes(normalized))
  return new Set(group || [normalized])
}

export function rolesAreEquivalent(a, b) {
  const aSet = equivalentRoleSet(a)
  const bNorm = normalizeRole(b)
  return bNorm ? aSet.has(bNorm) : false
}
...
function canPlayAnyRole(player, roles) {
  ...
  return roles.some((role) => {
    const accepted = equivalentRoleSet(role)
    return [...playerRoles].some((playerRole) => accepted.has(playerRole) || rolesAreEquivalent(playerRole, role))
  })
}

export function playerCanPlaySlotRole(player, slotRole) {
  const normalizedSlot = normalizeRole(slotRole)
  if (!normalizedSlot) return false
  return canPlayAnyRole(player, [normalizedSlot])
}
 ...
 */
export function countDefenseRolesFromTitolari(titolari, getSlotRole = null) {
  const counts = { dc: 0, td: 0, ts: 0 }
  for (const p of titolari || []) {
    const pos = normalizeRole(getSlotRole?.(p) || p?.position)
 ...
 */
export function countDefenseAfterSwap(titolari, replacePlayerId, incomingRole, getSlotRole = null) {
  const counts = { dc: 0, td: 0, ts: 0 }
  let slotRole = incomingRole ? normalizeRole(incomingRole) : ''

  for (const p of titolari || []) {
    let pos = normalizeRole(getSlotRole?.(p) || p?.position)
 ...
 */
export function validateStartingXISwap(titolari, reserve, replacePlayerId, options = {}) {
  const replaced = (titolari || []).find((p) => p.id === replacePlayerId)
  if (!replaced) {
    return { valid: false, errors: ['replace_not_in_xi'], slotRole: '', reserveCardRole: '' }
  }

  const getSlotRole = typeof options?.getSlotRole === 'function' ? options.getSlotRole : null
  const slotRole = normalizeRole(getSlotRole?.(replaced) || replaced.position)
  const reserveCardRole = normalizeRole(reserve?.position)
  const before = countDefenseRolesFromTitolari(titolari, getSlotRole)
  const after = countDefenseAfterSwap(titolari, replacePlayerId, slotRole, getSlotRole)
  const errors = getDefenseRuleViolations(after)
  const reserveCanPlaySlot = playerCanPlaySlotRole(reserve, slotRole)

  if (!reserveCanPlaySlot) {
    errors.push('incompatible_slot_role')
  }
...
export function formatDefenseLineupSummary(titolari, getSlotRole = null) {
  const c = countDefenseRolesFromTitolari(titolari, getSlotRole)
