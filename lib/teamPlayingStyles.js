...
export const GAMEPLAY_NOT_MENU_TEAM_STYLES = [
  'Pressing Alto',
  'Pressing Selettivo',
  'Pressing Costante',
  'Gegenpressing',
...
export function getConfigurableTeamStyleNames(lang = 'it') {
  return TEAM_PLAYING_STYLES.map((style) => getTeamPlayingStyleLabel(style.id, lang))
}
...
export function normalizePlayingStyleCompetence(raw) {
  const out = {}
  if (!raw || typeof raw !== 'object') return out
  for (const [key, value] of Object.entries(raw)) {
    const id = canonicalizeTeamPlayingStyleId(key)
    if (!id) continue
...
export function isConfigurableTeamStyleText(text) {
  return Boolean(resolveTeamPlayingStyle(text))
}
