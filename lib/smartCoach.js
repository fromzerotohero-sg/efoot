...
export const SMART_CHAT_LIMIT = 1
export const SMART_COUNTERMEASURE_LIMIT = 2
...
export function getSmartUiQuotaState(context) {
  const chatUsed = !!context?.chat_used
  const countermeasuresUsed = Number(context?.countermeasures_used) || 0

  return {
    remainingChat: chatUsed ? 0 : SMART_CHAT_LIMIT,
    remainingCountermeasures: Math.max(0, SMART_COUNTERMEASURE_LIMIT - countermeasuresUsed)
  }
}
