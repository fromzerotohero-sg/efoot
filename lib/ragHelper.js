...
export function getRelevantSections(userMessage, maxChars = DEFAULT_MAX_CHARS) {
  ...
  const scored = sections
    .map(s => ({
      ...s,
      score: scoreSection(s.title, messageNorm) + (isMeccanicheSectionTitle(s.title) && isGameplayQuestion ? 3 : 0)
    }))
...
const EFOOTBALL_TERMS = [
  ...
  'ruolo', 'ruoli', 'meccanica', 'meccaniche', 'difesa', 'attacco', 'build', 'build meta', 'meta build', 'progressione', 'punti sviluppo', 'slider', 'overall', 'rating',
