...
import { expandQueryWithTeamStyleAliases } from './teamPlayingStyles.js'
...
const SECTION_KEYWORDS = {
  ...
  'CONTESTO VIDEOGIOCO (FONDAMENTALE)': [
    'contesto videogioco', 'card digitali', 'fisso vs modificabile', 'terminologia ufficiale efootball',
    'progressione', 'skill training', 'additional skills', 'regola oro', 'formazione fluida'
  ],
  ...
  '2. STILI DI GIOCO DELLA CARD': [
    'stile giocatore', 'playing style giocatore', 'stile attacco', 'stile difesa', 'opportunista', 'senza palla', "rapace d'area", 'fulcro',
    ...
    'box-to-box', 'hole player', 'orchestrator', 'anchor man', 'terzino offensivo', 'terzino difensivo', 'terzino mattatore',
    'portiere offensivo', 'portiere difensivo', 'pressione in attacco', 'ruolo di copertura', 'difensore instancabile',
    'funambolo', 'serpentina', 'treno in corsa',
  ...
  '3. MODULI TATTICI': [
    ...
    'formazione fluida', 'fluid formation', 'schema attacco', 'schema difesa', 'forma in attacco', 'forma in difesa'
  ],
  '4. STILI DI GIOCO DI SQUADRA': [
    ...
    'pressing totale', 'pressione totale', 'overload', 'sovraccarico', 'superioridad',
    'presion total', 'total pressing',
    'out wide', 'contropiede', 'gioco diretto'
  ],
  '5. ISTRUZIONI INDIVIDUALI': [
    'istruzioni individuali', 'difensivo', 'ancoraggio', 'anchoring',
    'linea alta', 'linea bassa', 'deep line', 'marcatura stretta', 'marcatura uomo', 'contropiede',
    'slot possesso', 'slot difesa', 'attacking', 'offensivo', 'deep line instruction'
  ],
  '6. CALCI PIAZZATI': [
  ...
  '7. MECCANICHE DI GIOCO AVANZATE': [
    ...
    'volee dinamica', 'volée dinamica', 'dynamic volley', 'match-up', 'match up', 'chiama pressing', 'call for pressure'
  ],
  '8. ABILITÀ GIOCATORE, ABILITÀ SPECIALI E STILI COM/IA': [
    ...
    'attivatore d attacco', 'attack trigger', 'skill training', 'additional skills'
  ],
  ...
  '11. PROVENIENZA CATALOGO': [
    'provenienza catalogo', 'source version', 'source_version', 'versione carta', 'patch',
    'v5.4', 'v6.0', 'efootball 2027', 'hub', 'catalogo attuale', 'carta attuale', 'card attuale'
  ]
...
function isStiliSquadraSectionTitle(title) {
  const normalized = normalizeText(title)
  return normalized.includes('stili squadra') || normalized.includes('stili di gioco di squadra')
}

const TEAM_STYLE_HINT = [
  'stile squadra', 'stili squadra', 'team playstyle', 'team playing style',
  'pressing totale', 'pressione totale', 'overload', 'sovraccarico', 'superioridad',
  'possesso palla', 'contropiede veloce', 'contrattacco', 'passaggio lungo', 'vie laterali',
...
const GAMEPLAY_HINT = [
  ...
  'compattezza', 'possesso', 'costruzione',
  'super cancel', 'kick cancel', 'kick feint', 'shot cancel', 'pass cancel',
  'tess cancel', 'croqueta', 'croqueta interrotta', 'double touch cancel'
]
...
function isAbilitaSectionTitle(title) {
  const normalized = normalizeText(title)
  return normalized.includes('abilita giocatori') || normalized.includes('abilita giocatore')
}
...
export function getRelevantSections(userMessage, maxChars = DEFAULT_MAX_CHARS) {
  ...
  const messageNorm = expandQueryWithTeamStyleAliases(normalizeText(userMessage).replace(/'/g, ' '))
  ...
  const isTeamStyleQuestion = TEAM_STYLE_HINT.some(hint => messageNorm.includes(hint))
  const scored = sections
    .map(s => ({
      ...s,
      score:
        scoreSection(s.title, messageNorm) +
        (isMeccanicheSectionTitle(s.title) && isGameplayQuestion && !isTeamStyleQuestion ? 3 : 0) +
        (isStiliSquadraSectionTitle(s.title) && isTeamStyleQuestion ? 6 : 0) +
        (isAbilitaSectionTitle(s.title) && isAbilityAdviceQuestion ? 6 : 0)
    }))

  scored.sort((a, b) => b.score - a.score)

  if (isAbilityAdviceQuestion || isTapTrickQuestion) {
  ...
  for (const s of scored) {
    ...
    if (total + addLen > maxChars) {
      if (selected.length === 0 && remainingBudget > 1200) {
        selected.push({
          ...s,
          content: s.content.slice(0, remainingBudget)
        })
  ...
  const STILI_TITLE = '2. STILI DI GIOCO DELLA CARD'
  return selected
    .map(s => {
...
export function getRelevantSectionsForContext(contextType, maxChars = 12000) {
  ...
  for (const title of titles) {
    ...
    if (total + s.content.length > maxChars) {
      ...
      if (selected.length === 0 && remaining > 1200) {
        selected.push({
          ...s,
...
const EFOOTBALL_TERMS = [
  ...
  'match-up', 'match up', 'pressing', 'pressing totale', 'overload', 'sovraccarico', 'manual defending', 'shadow marking', 'contrasto spalla', 'anticipazione',
