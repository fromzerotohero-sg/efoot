...
const SECTION_KEYWORDS = {
  ...
  '2. STILI GIOCATORE - Caratteristica card (FISSI)': [
    ...
    'pressione in attacco', 'frontline pressure',
...
export function getRelevantSections(userMessage, maxChars = DEFAULT_MAX_CHARS) {
  ...
  if (isAbilityAdviceQuestion || isTapTrickQuestion || isAttackingSurgeQuestion) {
    ...
    if (abilitaSection && !selected.some(x => x.title === abilitaSection.title)) {
      ...
      } else if (isAttackingSurgeQuestion) {
        const subsection87 = extractSubsection(abilitaSection.content, '8.7')
        if (subsection87) {
          const distinction =
            'Attacking Surge = Sprint in attacco. Abilità distinta da Attack Trigger / Attivatore d\'attacco, Acceleration Burst / Scatto bruciante e Incisive Run / Inserimento incisivo. Nei dati può esistere anche un pack chiamato Attacking Surge: non attribuire al pack l\'effetto dell\'abilità.'
          const condensed = `### 8.7 Abilità Speciali e Leadership\n\n${subsection87.replace(/^###\s+8\.7[^\n]*\n?/i, '').trim()}\n\n${distinction}`
          selected.push({
            title: abilitaSection.title,
            content: condensed
          })
...
const EFOOTBALL_TERMS = [
  ...
  'ancoraggio', 'anchoring', 'linea bassa', 'linea alta', 'deep line', 'pressing totale', 'overload', 'formazione fluida', 'fluid formation', 'pressione in attacco', 'frontline pressure', 'dynamic volley', 'volée dinamica', 'marcatura stretta', 'marcatura uomo',
