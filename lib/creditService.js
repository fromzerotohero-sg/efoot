 ...
 */
async function accreditMetalGateWallet(metalgateUserId, amount, description, referenceId = null) {
  if (!metalgateUserId || !process.env.NEXT_PUBLIC_METALGATE_API_URL || !process.env.METALGATE_API_KEY) {
  ...
  try {
    const metalgateUrl = process.env.NEXT_PUBLIC_METALGATE_API_URL.replace(/\/+$/, '')
    const res = await fetchWithRetry(`${metalgateUrl}/internal/accredit`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Internal-API-Key': process.env.METALGATE_API_KEY
    ...
    if (!res.ok) {
      ...
      console.error('[creditService] MetalGate accredit failed:', res.status, errData)
...
export async function logCreditError(admin, payload) {
  ...
  try {
    ...
    if (error) {
      console.error('[creditService] logCreditError insert error:', error.message)
    }
  } catch (err) {
    console.error('[creditService] logCreditError exception:', err?.message || err)
  }
}
...
export function classifyCreditError(error, opts = {}) {
  ...
  const userFaultByStatus = Number.isFinite(statusCode) && [400, 401, 403, 404, 409, 410, 422].includes(statusCode)
  const userFaultByType = ['validation_error', 'auth_error', 'user_error', 'client_error', 'insufficient_credits'].includes(errorType)
  ...
  const infraByStatus = Number.isFinite(statusCode) && (statusCode >= 500 || statusCode === 429 || statusCode === 408)
  const infraByType = ['timeout', 'network_error', 'server_error', 'rate_limit', 'provider_error', 'openai_error'].includes(errorType)
  ...
  const refundable = !userFaultByStatus && !userFaultByType && !userFaultByMessage && (infraByStatus || infraByType || infraByMessage || !errorCode)
 ...
 * @returns {Promise<{success: boolean, error?: string}>}
 */
export async function refundCredits(admin, userId, cost, operationType = 'ai-call', metalgateUserId = null) {
  ...
  if (mgId && process.env.NEXT_PUBLIC_METALGATE_API_URL && process.env.METALGATE_API_KEY) {
    try {
      ...
      if (!res.ok) {
        const errData = await res.json().catch(() => ({}))
        console.error('[creditService] MetalGate refund failed:', res.status, errData)
        return { success: false, error: errData.error || 'MetalGate refund failed' }
      }

      await recordTransaction(admin, userId, cost, 'purchase', `Refund ${operationType}`, null)
      return { success: true }
    } catch (err) {
...
export async function handleCreditOperationError(admin, payload) {
  ...
  return { refunded: refund.success, refundable: true, refundError: refund.error }
}
 ...
 * @returns {Promise<{ ok: boolean, idempotent?: boolean, error?: string }>}
 */
export async function accreditBonus(admin, userId, creditsAmount, referenceId, description = 'Bonus HP', periodKey = null) {
  ...
  try {
    ...
    if (usesMetalGateWallet) {
      ...
      if (mgResult.ok) {
        await recordTransaction(admin, userId, amount, 'purchase', description, referenceId)
        return { ok: true }
      }
      // Non bloccare la ruota: fallback al wallet locale se MetalGate risponde errore/transiente.
      console.error('[creditService] accreditBonus MetalGate fallback to local wallet:', mgResult.error)
    }
