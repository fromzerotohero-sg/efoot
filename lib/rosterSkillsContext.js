...
import { getEffectiveCardType, getNonProgressionReason } from './buildCoachServerUtils.js'
import {
...
export function getPlayerComAiPlaystyles(player) {
  ...
  ]).filter((item) => isComAiPlaystyle(item) || !isKnownPlayerSkill(item))
}
...
export function getNativePlayerSkills(player) {
  const fromNative = uniqueCanonSkills(readMetaArray(player, 'native_skills'))
  if (fromNative.length) return fromNative.filter(isPlayerSkillToken)
  return uniqueCanonSkills(player?.skills).filter((item) => isPlayerSkillToken(item) && isFixedInnateCardSkill(item))
}

export function getAdditionalOrUnclassifiedPlayerSkills(player) {
  const additional = uniqueCanonSkills(readMetaArray(player, 'additional_skills'))
  if (additional.length) return additional.filter(isPlayerSkillToken)
  // Fallback: skills = mixed; treat non-innate as additional candidates (origin unclassified).
  if (typeof console !== 'undefined' && process.env.NODE_ENV !== 'production' && Array.isArray(player?.skills) && player.skills.length) {
    console.warn('[rosterSkillsContext] additional_skills missing; treating non-innate skills as unclassified additional')
  }
  return uniqueCanonSkills(player?.skills).filter((item) => isPlayerSkillToken(item) && !isFixedInnateCardSkill(item))
}
...
export function formatPlayerSkillContext(player, lang = 'it', { maxListed = MAX_SKILLS_LISTED } = {}) {
  ...
  const additionalKnown = Array.isArray(readMetaArray(player, 'additional_skills')) && readMetaArray(player, 'additional_skills').length > 0
  ...
  const programsLabel = nonProg.blocked
    ...
    : cardType
      ? ((lang === 'en' || lang === 'es') ? `Programs: yes if under ${MAX_ADDITIONAL_SKILLS} additional` : `Programmi: sì se sotto ${MAX_ADDITIONAL_SKILLS} additional`)
...
export function buildRosterSkillAdvisorySection(roster, gameAnalysisRow, lang = 'it') {
  ...
  const lines = ordered.map((p) => {
    ...
    const additionalKnown = Array.isArray(readMetaArray(p, 'additional_skills')) && readMetaArray(p, 'additional_skills').length > 0

    let tail = ''
    if (nonProg.blocked) {
      tail = (lang === 'en' || lang === 'es') ? 'No skill Programs on this card.' : 'Programmi abilità non disponibili su questa card.'
    } else if (additionalKnown && additional.length >= MAX_ADDITIONAL_SKILLS) {
