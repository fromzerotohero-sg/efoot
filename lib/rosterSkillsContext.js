...
export function getNativePlayerSkills(player) {
  const fromNative = uniqueCanonSkills(player?.native_skills)
...
export function getAdditionalOrUnclassifiedPlayerSkills(player) {
  const additional = uniqueCanonSkills(player?.additional_skills)
...
export function getMergedPlayerSkills(player) {
  return uniqueCanonSkills([
    ...
    ...(Array.isArray(player?.native_skills) ? player.native_skills : []),
    ...(Array.isArray(player?.additional_skills) ? player.additional_skills : [])
...
export function formatPlayerSkillContext(player, lang = 'it', { maxListed = MAX_SKILLS_LISTED } = {}) {
  ...
  const additionalKnown = Array.isArray(player?.additional_skills) && player.additional_skills.length > 0
...
export function buildRosterSkillAdvisorySection(roster, gameAnalysisRow, lang = 'it') {
  ...
  const lines = ordered.map((p) => {
    ...
    const additionalKnown = Array.isArray(p?.additional_skills) && p.additional_skills.length > 0
