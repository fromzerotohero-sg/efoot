...
const ROLE_SKILL_PRIORITY = {
  ...
  DC: ['Intercettazione', 'Marcatura', 'Superiorità aerea', 'Spirito combattivo'],
  TD: ['Intercettazione', 'Rientro difensivo', 'Cross preciso'],
  TS: ['Intercettazione', 'Rientro difensivo', 'Cross preciso'],
  ...
  TRQ: ['Passaggio di prima', 'Passaggio filtrante', 'Passaggio dosato', 'Spirito combattivo'],
  ...
  ALA: ['Cross preciso', 'Doppio tocco', 'Rientro difensivo'],
  EDA: ['Cross preciso', 'Doppio tocco', 'Passaggio di prima'],
  ESA: ['Cross preciso', 'Doppio tocco', 'Passaggio di prima']
}
...
function skillTokenMatches(haveToken, expectedToken) {
  if (!haveToken || !expectedToken) return false
  return haveToken.includes(expectedToken) || expectedToken.includes(haveToken)
}

function playerHasSkill(player, expectedLabel) {
  const expected = normalizeSkillToken(expectedLabel)
  return getMergedPlayerSkills(player).some(s => skillTokenMatches(normalizeSkillToken(s), expected))
}
...
export function formatPlayerSkillContext(player, lang = 'it', { maxListed = MAX_SKILLS_LISTED } = {}) {
  ...
  const listed = skills.slice(0, maxListed)
  const countLabel = lang === 'en' ? 'skills' : 'abilità'
  const skillsPart = listed.length > 0 ? listed.join(', ') : (lang === 'en' ? 'none listed' : 'nessuna elencata')
  ...
  const programsLabel = nonProg.blocked
    ? (lang === 'en' ? 'Programs: no (Trending/max)' : 'Programmi: no (Trending/maxi)')
    : cardType
      ? (lang === 'en' ? 'Programs: yes if slots free' : 'Programmi: sì se slot liberi')
      : (lang === 'en' ? 'Programs: check card type' : 'Programmi: verifica tipo card')

  const cardPart = cardType ? `card ${cardType}` : (lang === 'en' ? 'card type unknown' : 'tipo card non indicato')
...
export function buildRosterSkillAdvisorySection(roster, gameAnalysisRow, lang = 'it') {
  ...
  const lines = ordered.map(p => {
    ...
    let tail = ''
    if (nonProg.blocked) {
      tail = lang === 'en' ? 'No skill Programs on this card.' : 'Programmi abilità non disponibili su questa card.'
    } else if (skills.length >= MAX_SKILLS_LISTED) {
      tail = lang === 'en' ? 'Skill slots full (6/6).' : 'Slot abilità pieni (6/6).'
      ...
      tail = lang === 'en' ? 'Core role skills present in list.' : 'Abilità core ruolo già presenti in elenco.'
    }
  ...
  if (analysisHint) {
    parts.push(lang === 'en' ? `Game-analysis cross-check: ${analysisHint}` : `Incrocio Analisi eFootball: ${analysisHint}`)
  }
  return parts.join('\n')
}
