...
export function getBuildSourceStats(player = {}, catalogCard = null) {
  if (catalogCard?.players_payload?.base_stats && typeof catalogCard.players_payload.base_stats === 'object') {
    if (Object.keys(catalogCard.players_payload.base_stats).length > 0) {
      return catalogCard.players_payload.base_stats
    }
  }
  if (catalogCard?.base_stats && typeof catalogCard.base_stats === 'object' && Object.keys(catalogCard.base_stats).length > 0) {
...
function getPlayerBoosters(player = {}) {
  ...
  if (!activeName || list.length === 0) return list
  const matched = list.filter((entry) => normalizeBoosterName(entry?.name) === activeName)
  if (matched.length > 0) return matched
  return list.slice(0, 1)
}

export function applyInGameEffects(stats, { player, coach, teamStyle }) {
  const withPlayerBoosters = applyBoosters(stats, getPlayerBoosters(player))
  return applyCoachEffects(withPlayerBoosters, coach, teamStyle)
}
...
/**
 * Anteprima statistiche e OVR da una distribuzione macro (solo progressione PT, senza booster/coach).
 */
export function previewGameplayBuildFromSliders({
  ...
  const finalInGameStats = applyInGameEffects(finalEfhubStats, { player, coach, teamStyle })
  const boostersConsidered = getPlayerBoosters(player).length > 0
  const coachConsidered = hasCoachEffects(coach)
  const progressionOverall = computeOverallRating({ position: targetPosition, height, weakFootAccuracy, stats: finalEfhubStats })
  const inGameOverall = computeOverallRating({ position: targetPosition, height, weakFootAccuracy, stats: finalInGameStats })
  const capped = capBuildOveralls({
    progressionOverall,
    inGameOverall,
    progressionOverallDecimal: progressionOverall,
    inGameOverallDecimal: inGameOverall,
    catalogCard,
    player
  })
...
export function calculateGameplayBuild({ player, roster = [], teamStyle = null, slotPosition = null, catalogCard = null, coach = null } = {}) {
  ...
  const finalInGameStats = applyInGameEffects(finalStats, { player, coach, teamStyle })
  const boostersConsidered = getPlayerBoosters(player).length > 0
  const coachConsidered = hasCoachEffects(coach)
  const progressionOverall = computeOverallRating({ position: targetPosition, height, weakFootAccuracy, stats: finalStats })
