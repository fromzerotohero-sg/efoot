...
/** Carte con profilo stats che possono ricevere PT (esclusi solo POTW/Trending/OTW ecc.). */
...
export function needsEfhubLevelCapEnrichment(player = {}, catalogCard = null) {
  ...
  if (catalogShowsProgression(catalogCard)) return true
  return catalogCardAllowsPtBuild(catalogCard, player)
}
...
function playProfileOverallFromSliders(sliders, baseStats, player, targetPosition, height, weakFootAccuracy) {
  const progressed = applyProgression(baseStats, sliders)
...
function spendLeftoverProgressionForOverall({
  ...
  for (let guard = 0; used < pointsAvailable && guard < maxSteps; guard++) {
    const currentOverall = playProfileOverallFromSliders(out, baseStats, player, targetPosition, height, weakFootAccuracy)
    let best = null
    ...
    for (const key of PROGRESSION_KEYS) {
      ...
      const trialOverall = playProfileOverallFromSliders(trial, baseStats, player, targetPosition, height, weakFootAccuracy)
...
export function applyFieldEffects(stats, { player, coach = null, teamStyle = null, statCaps = null, catalogCard = null } = {}) {
  const caps = statCaps || statCapsForBuild(catalogCard)
  const withBoosters = applyBoosters(stats, getFieldBoosters(player), caps)
  return applyCoachEffects(withBoosters, coach, teamStyle, caps)
}
...
export function previewGameplayBuildFromSliders({
  ...
  const finalEfhubStats = applyProgression(baseStats, safeSliders, statCaps)
  ...
  const finalFieldStats =
    coach || teamStyle || getFieldBoosters(player).length > 0
      ? applyFieldEffects(finalEfhubStats, { player, coach, teamStyle, statCaps, catalogCard })
      : finalInGameStats
...
export function calculateGameplayBuild({ player, roster = [], teamStyle = null, slotPosition = null, catalogCard = null, coach = null } = {}) {
  ...
  for (let step = 0; step < maxSteps && spent < pointsAvailable; step++) {
    ...
    sliders[bestKey] += 1
    spent += cost
    currentStats = applyProgression(baseStats, sliders, statCaps)
  }
  ...
  const finalStats = applyProgression(baseStats, sliders, statCaps)
  ...
  const fieldBoostersConsidered = getFieldBoosters(player).length > 0
  const finalFieldStats =
    coach || teamStyle || fieldBoostersConsidered
      ? applyFieldEffects(finalStats, { player, coach, teamStyle, statCaps, catalogCard })
      : finalInGameStats
  const boostersConsidered = getPlayerBoosters(player).length > 0
  const coachConsidered = hasCoachEffects(coach)
