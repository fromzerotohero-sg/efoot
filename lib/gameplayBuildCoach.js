...
export function applyFieldEffects(stats, { player, coach = null, teamStyle = null, statCaps = null, catalogCard = null } = {}) {
  const caps = statCaps || statCapsForBuild(catalogCard)
  const withManagerSkill = coach
    ? applyManagerSkill(stats, resolveCoachSkillValue(coach, teamStyle), caps)
    : stats
  return applyBoosters(withManagerSkill, getFieldBoosters(player), caps)
}
