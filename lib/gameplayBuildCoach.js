...
export function calculateGameplayBuild({ player, roster = [], teamStyle = null, slotPosition = null, catalogCard = null, activeCoach = null } = {}) {
  ...
  const progressionStats = applyProgression(baseStats, sliders)
  const boostedStats = applyBoosters(progressionStats, player.available_boosters || player.boosters || [])
  const finalStats = applyCoachEffects(boostedStats, activeCoach, teamStyle)
  ...
  return {
    ...
    sourceBaseStats,
    baseStats,
    progressionEfhubStats: progressionStats,
    boostedEfhubStats: boostedStats,
    ...
    boostersConsidered: Array.isArray(player.available_boosters || player.boosters) && (player.available_boosters || player.boosters || []).length > 0,
    coachStatBoostsConsidered: Boolean(activeCoach),
