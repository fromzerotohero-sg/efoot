...
function getPlayerBoosters(player = {}) {
  if (Array.isArray(player.available_boosters)) return player.available_boosters
  if (Array.isArray(player.boosters)) return player.boosters
  return []
}

function applyInGameEffects(stats, { player, coach, teamStyle }) {
...
export function previewGameplayBuildFromSliders({
  ...
  const afterOverall = Math.max(progressionOverall, inGameOverall)
...
export function calculateGameplayBuild({ player, roster = [], teamStyle = null, slotPosition = null, catalogCard = null, coach = null } = {}) {
  ...
  const afterOverall = Math.max(progressionOverall, inGameOverall)
  const afterOverallDecimal = Math.max(progressionOverallDecimal, inGameOverallDecimal)
  const safeSliders = sanitizeSliders(sliders)
