...
function progressionStatGainFromTick(baseStats, sliders, key) {
  const before = applyProgression(baseStats, sliders)
  const after = applyProgression(baseStats, { ...sliders, [key]: (sliders[key] || 0) + 1 })
  let gain = 0
...
function spendLeftoverProgressionForOverall({
  ...
  for (let guard = 0; used < pointsAvailable && guard < maxSteps; guard++) {
    ...
    for (const key of PROGRESSION_KEYS) {
      ...
      const statGain = ovrGain > 0 ? 0 : progressionStatGainFromTick(baseStats, out, key)
