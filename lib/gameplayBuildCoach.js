...
const SKILL_MODIFIERS = [
  { match: /long[- ]?range|rising shot|dipping shot|first[- ]?time|acrobatic|knuckle|chip shot|phenomenal finishing/i, weights: { shooting: 1.2, lowerBodyStrength: 0.35 } },
  { match: /curler|outside curler|set piece|penalty/i, weights: { shooting: 0.8, passing: 0.25 } },
  { match: /double touch|sole control|flip flap|marseille|cut behind|scissors feint|magnetic feet/i, weights: { dribbling: 1.25, dexterity: 0.7 } },
  { match: /through passing|one[- ]?touch pass|weighted pass|pinpoint crossing|low lofted|phenomenal passing/i, weights: { passing: 1.4, dexterity: 0.35 } },
...
function buildReasons({ group, targetPosition, teamStyle, sliders, player }) {
  ...
  if (top.includes('dribbling') || top.includes('dexterity')) reasons.push('Controllo, equilibrio e accelerazione rinforzati per migliorare efficacia nel ruolo.')
