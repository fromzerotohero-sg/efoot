...
import { hasNonProgressionCardType } from './cardProgressionEligibility.js'
...
const SKILL_MODIFIERS = [
  ...
  { match: /double touch|tap trick|sole control|flip flap|marseille|cut behind|scissors feint|magnetic feet|momentum dribbling|trickster|mazing run/i, weights: { dribbling: 1.45, dexterity: 0.85 } },
...
/** Carte che possono ricevere PT (Konami: no solo Trending / classe POTW-OTW). */
export function catalogCardAllowsPtBuild(catalogCard, player = null) {
  if (hasNonProgressionCardType(player, catalogCard)) return false
...
export function resolveProgressionLevelCap(player = {}, catalogCard = null) {
  // Konami: Trending (POTW/OTW) non hanno Player development — non inventare un level cap.
  if (hasNonProgressionCardType(player, catalogCard)) return 1
...
/** Penalità per impilare troppe tacche sulla stessa macro (prima era 0.08, troppo debole). */
const MACRO_STACK_PENALTY = 0.12
const MACRO_STACK_PENALTY_PHASE2 = 0.16
/** Limite di qualità per le build consigliate: il gioco permette di più, ma oltre 12 il consiglio diventa troppo concentrato. */
const RECOMMENDED_MAX_TACCE_PER_MACRO = 12

/** I 3 pilastri per peso ruolo/stile/skill — allineamento leggero alle build multi-macro dei guide. */
function getTopPillarKeys(weights, group, count = 3) {
  return PROGRESSION_KEYS.filter(key => isMacroAllowedForGroup(group, key))
...
/**
 * Piccolo bonus se un pilastro è ancora a 0 o resta indietro rispetto agli altri pilastri.
 * Non forza equilibrio totale: solo evita Tiro 14 / Passaggio 0 su profili da attaccante creativo.
 */
function pillarSpreadBonus(key, sliders, pillarKeys) {
  if (!pillarKeys.length || !pillarKeys.includes(key)) return 0
  const ticks = sliders[key] || 0
  const pillarTicks = pillarKeys.map(k => sliders[k] || 0)
  const maxPillar = Math.max(...pillarTicks)
  const activePillars = pillarTicks.filter(t => t > 0).length

  if (ticks === 0 && activePillars > 0) return 0.65
  if (maxPillar >= 3 && maxPillar - ticks >= 2 && ticks < maxPillar) return 0.3
  return 0
}
...
function spendLeftoverProgressionForOverall({
  ...
  const out = { ...sliders }
  let used = spent
  const maxSteps = PROGRESSION_KEYS.length * RECOMMENDED_MAX_TACCE_PER_MACRO
  ...
  for (let guard = 0; used < pointsAvailable && guard < maxSteps; guard++) {
    ...
    for (const key of PROGRESSION_KEYS) {
      if (out[key] >= RECOMMENDED_MAX_TACCE_PER_MACRO) continue
...
export function calculateGameplayBuild({ player, roster = [], teamStyle = null, slotPosition = null, catalogCard = null, coach = null } = {}) {
  ...
  const pillarKeys = getTopPillarKeys(weights, group)
  ...
  for (let step = 0; step < maxSteps && spent < pointsAvailable; step++) {
    ...
    for (const key of PROGRESSION_KEYS) {
      if (sliders[key] >= RECOMMENDED_MAX_TACCE_PER_MACRO) continue
      ...
      const score = (
        ...
        sliders[key] * MACRO_STACK_PENALTY +
        pillarSpreadBonus(key, sliders, pillarKeys)
      ) / cost
      if (score > bestScore) {
        bestScore = score
        bestKey = key
  ...
  for (let guard = 0; spent < pointsAvailable && guard < maxSteps; guard++) {
    const candidates = PROGRESSION_KEYS
      .filter((key) => sliders[key] < RECOMMENDED_MAX_TACCE_PER_MACRO)
      .filter((key) => isMacroAllowedForGroup(group, key))
      .map((key) => ({
        key,
        cost: nextTickCost(sliders, key),
        value:
          (weights[key] || 0) +
          hardBlockScore(group, key) -
          sliders[key] * MACRO_STACK_PENALTY_PHASE2 +
          pillarSpreadBonus(key, sliders, pillarKeys)
      }))
...
/**
	 * @param {{ it?: string[], en?: string[], es?: string[] } | string[] | null | undefined} bundle
	 * @param {'it'|'en'|'es'} lang
	 * @returns {string[]}
	 */
