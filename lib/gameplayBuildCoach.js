...
/**
 * Azzera macro illegali per ruolo (come in eFootball / EFHub):
 * - fuori dal gruppo portiere: nessun punto su gk1/gk2/gk3 (le tre "PT" portiere);
 * - portiere: nessun punto su tiro/dribbling/difesa (macro da campo).
 */
export function coerceBuildSlidersForRole(player, sliders, slotPosition = null, catalogCard = null) {
  const safe = sanitizeSliders(sliders)
  ...
  return sanitizeSliders(next)
}

/**
 * Macro non assegnabile per ruolo (es. PT per non portieri).
 */
...
export function tryApplyBuildSliderDelta({
  ...
  const base = coerceBuildSlidersForRole(player, sanitizeSliders(sliders), slotPosition, catalogCard)
...
export function previewGameplayBuildFromSliders({
  ...
  const pointsAvailable = availableProgressionPoints(levelCap)
  const safeSliders = coerceBuildSlidersForRole(player, sliders, slotPosition, catalogCard)
  ...
  const afterOverall = computeOverallRating({ position: targetPosition, height, weakFootAccuracy, stats: finalEfhubStats })
...
export function calculateGameplayBuild({ player, roster = [], teamStyle = null, slotPosition = null, catalogCard = null } = {}) {
  ...
  const safeSliders = coerceBuildSlidersForRole(player, sliders, slotPosition, catalogCard)
  const finalStats = applyProgression(baseStats, safeSliders)
