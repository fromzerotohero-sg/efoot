...
/**
 * Spende i PT rimasti sulla macro che fa salire di più l'OVR (miglior guadagno / costo PT).
 */
function spendLeftoverProgressionForOverall({
  ...
  for (let guard = 0; used < pointsAvailable && guard < maxSteps; guard++) {
    ...
    for (const key of PROGRESSION_KEYS) {
      ...
      if (ovrGain <= 0) continue

      const efficiency = ovrGain / cost
      if (
        !best ||
        efficiency > best.efficiency ||
        (efficiency === best.efficiency && ovrGain > best.ovrGain)
      ) {
        best = { key, cost, ovrGain, efficiency }
      }
    }
