import {
  ...
  clampSlidersToProgressionBudget,
  clampOverallRating,
...
/** Cap livello da catalogo / eFHUB (initialLevelCap, max_level). */
export function resolveCatalogProgressionLevelCap(catalogCard = null) {
  const catalogCaps = [
    ...
    catalogCard?.metadata?.efhub_db_level_cap
  ]
    .map((value) => Number(value))
    .filter((value) => Number.isFinite(value) && value > 1)

  if (catalogCaps.length > 0) return Math.floor(Math.min(...catalogCaps))
...
/**
 * Cap effettivo per PT in build (priorità: level_cap giocatore impostato in gioco, poi catalogo).
 * Non usare metadata.max_level qui: altrimenti 32 da catalogo sovrascrive 31 impostato dall'utente.
 */
function inferLevelCapFromBuildCoachSliders(player = {}) {
  const sliders =
  ...
  return inferLevelCapFromPointsUsed(totalPointsUsed(sliders))
}

/** Cap Konami/eFHUB UI: il DB espone spesso initialLevelCap+1 rispetto al cap riducibile in gioco. */
export function resolveInGameProgressionLevelCap(catalogCap) {
  const cap = Number(catalogCap)
  if (!Number.isFinite(cap) || cap <= 1) return null
  if (cap <= 2) return Math.floor(cap)
  return Math.floor(cap) - 1
}

export function resolveProgressionLevelCap(player = {}, catalogCard = null) {
  const playerCap = readExplicitPlayerLevelCap(player)
  if (playerCap != null) return playerCap

  const fromBuild = inferLevelCapFromBuildCoachSliders(player)
  if (fromBuild != null) return fromBuild

  const catalogCap = resolveCatalogProgressionLevelCap(catalogCard)
  if (catalogCap != null && catalogCap > 1) return resolveInGameProgressionLevelCap(catalogCap)

  const playerCapOne = [player.level_cap, player.metadata?.max_level]
    .map((value) => Number(value))
    .some((value) => value === 1)
...
function readExplicitPlayerLevelCap(player = {}) {
  const explicitCaps = [
    player.level_cap,
    player.levelCap,
    player.extracted_data?.level_cap,
    ...
    player.metadata?.build_coach?.level_cap
  ]
    .map((value) => Number(value))
    .filter((value) => Number.isFinite(value) && value > 1)
  return explicitCaps.length > 0 ? Math.floor(explicitCaps[0]) : null
}

/** Contesto cap per UI: cap gioco vs catalogo eFHUB (come dbLevelCap / levelCap su eFHUB). */
export function getProgressionCapInfo(player = {}, catalogCard = null) {
  const catalogLevelCap = resolveCatalogProgressionLevelCap(catalogCard)
  const playerLevelCap = readExplicitPlayerLevelCap(player)
  const levelCap = playerLevelCap ?? catalogLevelCap
  const pointsAvailable = availableProgressionPoints(levelCap)
  const catalogPointsAvailable =
    catalogLevelCap != null ? availableProgressionPoints(catalogLevelCap) : null

  return {
    levelCap,
    playerLevelCap,
    catalogLevelCap,
    capSource: playerLevelCap != null ? 'player' : catalogLevelCap != null ? 'catalog' : null,
    pointsAvailable,
    catalogPointsAvailable,
    catalogExceedsEffective:
      Number.isFinite(catalogLevelCap) &&
      Number.isFinite(playerLevelCap) &&
      catalogLevelCap > playerLevelCap,
    ptFormula: '(levelCap - 1) * 2'
  }
}

/** Allinea cap e slider al budget copiabile in Konami (es. 60 PT, non 62). */
function alignProgressionToInGameBudget(levelCap, sliders, catalogLevelCap = null) {
  const dbCap = Number(catalogLevelCap ?? levelCap)
  if (!Number.isFinite(dbCap) || dbCap <= 2) {
    return { levelCap, sliders: sanitizeSliders(sliders), adjusted: false }
  }
  if (Number(levelCap) < dbCap) {
    return { levelCap, sliders: sanitizeSliders(sliders), adjusted: false }
  }
  const spent = totalPointsUsed(sliders)
  const catalogBudget = availableProgressionPoints(dbCap)
  if (spent < catalogBudget) {
    return { levelCap, sliders: sanitizeSliders(sliders), adjusted: false }
  }
  const inGameCap = resolveInGameProgressionLevelCap(dbCap)
  const inGameBudget = availableProgressionPoints(inGameCap)
  if (!inGameBudget || spent <= inGameBudget) {
    return { levelCap: inGameCap, sliders: sanitizeSliders(sliders), adjusted: inGameCap !== catalogCap }
  }
  return {
    levelCap: inGameCap,
    sliders: clampSlidersToProgressionBudget(sliders, inGameCap),
    adjusted: true
  }
}
...
/** Booster in rosa / in campo — tutti quelli assegnati alla carta (es. Difesa + Pilastro). */
export function getFieldBoosters(player = {}) {
  const list = Array.isArray(player.available_boosters)
    ...
    : Array.isArray(player.boosters)
      ? player.boosters
      : []
  return list.filter((entry) => String(entry?.name || '').trim())
}
...
/**
 * Statistiche/OVR in campo: PT + tutti i booster in rosa + allenatore e stile squadra.
 */
export function applyFieldEffects(stats, { player, coach = null, teamStyle = null } = {}) {
  const withBoosters = applyBoosters(stats, getFieldBoosters(player))
  return applyCoachEffects(withBoosters, coach, teamStyle)
}
...
export function previewGameplayBuildFromSliders({
  ...
  const capInfo = getProgressionCapInfo(player, catalogCard)
  const pointsAvailable = availableProgressionPoints(levelCap)
  ...
  const finalFieldStats =
    coach || teamStyle || getFieldBoosters(player).length > 0
      ? applyFieldEffects(finalEfhubStats, { player, coach, teamStyle })
      : finalInGameStats
  ...
  return {
    ...
    targetPosition,
    levelCap,
    catalogLevelCap: capInfo.catalogLevelCap,
    capSource: capInfo.capSource,
    pointsAvailable,
    catalogPointsAvailable: capInfo.catalogPointsAvailable,
    pointsUsed,
    sliders: safeSliders,
    capWarnings: { it: capWarningsIt, en: capWarningsEn },
    baseStats,
    finalEfhubStats,
    finalBaseStats,
    finalInGameStats,
...
export function calculateGameplayBuild({ player, roster = [], teamStyle = null, slotPosition = null, catalogCard = null, coach = null } = {}) {
  ...
  const capInfo = getProgressionCapInfo(player, catalogCard)
  const aligned = alignProgressionToInGameBudget(levelCap, sliders, capInfo.catalogLevelCap)
  const effectiveLevelCap = aligned.levelCap
  const safeSliders = aligned.sliders
  const pointsUsed = totalPointsUsed(safeSliders)
  const effectivePointsAvailable = availableProgressionPoints(effectiveLevelCap)
  if (aligned.adjusted) estimatedFields.push('level_cap')
  const confidence = Math.max(0.55, Math.min(0.96, 0.96 - estimatedFields.length * 0.12))
  const warningIt = []
  const warningEn = []
  if (estimatedFields.length > 0) {
    warningIt.push('Alcuni dati sono stati stimati o recuperati da fallback.')
    warningEn.push('Some values were estimated or recovered from fallbacks.')
  }

  const finalStatsClamped = applyProgression(baseStats, safeSliders)
  const finalInGameStatsClamped = applyInGameEffects(finalStatsClamped, { player })
  const cappedClamped = capBuildOveralls({
    progressionOverall: computePlayProfileOverallRating({
      position: targetPosition,
      height,
      weakFootAccuracy,
      stats: finalStatsClamped
    }),
    inGameOverall: computePlayProfileOverallRating({
      position: targetPosition,
      height,
      weakFootAccuracy,
      stats: finalInGameStatsClamped
    }),
    progressionOverallDecimal: computeOverallRatingDecimal({
      position: targetPosition,
      height,
      weakFootAccuracy,
      stats: finalStatsClamped
    }),
    inGameOverallDecimal: computeOverallRatingDecimal({
      position: targetPosition,
      height,
      weakFootAccuracy,
      stats: finalInGameStatsClamped
  ...
  return {
    ...
    catalogLevelCap: capInfo.catalogLevelCap,
    pointsAvailable: effectivePointsAvailable,
    catalogPointsAvailable: capInfo.catalogPointsAvailable,
    pointsUsed,
    overBudget: pointsUsed > effectivePointsAvailable,
    sliders: safeSliders,
    sourceBaseStats,
    baseStats,
    finalEfhubStats: finalStatsClamped,
    finalInGameStats: finalInGameStatsClamped,
    finalBaseStats: efhubStatsToPlayerBaseStats(finalStatsClamped),
    beforeOverall,
    overallCap: cappedClamped.overallCap,
    afterOverall: cappedClamped.afterOverall,
    afterOverallDecimal: cappedClamped.afterOverallDecimal,
    progressionOverall: cappedClamped.progressionOverall,
    progressionOverallDecimal: cappedClamped.progressionOverallDecimal,
    inGameOverall: cappedClamped.inGameOverall,
    inGameOverallDecimal: cappedClamped.inGameOverallDecimal,
    boostersConsidered,
    coachConsidered,
    estimatedFields,
    confidence,
    reasons: buildReasonsBilingual({ group, teamStyle, sliders: safeSliders, player }),
    warnings: { it: warningIt, en: warningEn }
  }
}
