...
export async function loadThreadMessages(admin, userId, { surface = SURFACE, limit = 50 } = {}) {
  ...
  return { thread, messages: orderNewestFirstThenChronological(data || [], cap) }
}
...
export async function appendMessages(admin, userId, messages, { surface = SURFACE, threadId = null } = {}) {
  ...
  const rows = (messages || [])
    // I messaggi system sono indicatori transitori della UI, non cronologia.
    // Non salvarli: altrimenti un "Sto leggendo..." può riapparire bloccato
    // dopo un reload o una nuova domanda.
    .filter((m) => m && m.role !== 'system' && (m.content || m.payload))
    .map((m) => ({
      thread_id: thread.id,
      user_id: userId,
      role: m.role === 'user' ? 'user' : 'hero',
...
export function dbMessageToClient(row) {
  ...
  return {
    id: row.id,
    role: row.role === 'user' ? 'user' : row.role === 'system' ? 'system' : 'hero',
    content: row.content || '',
