...
const SKILL_DEFINITIONS = [
  ...
  { en: 'Shadow Hunt', it: 'Caccia all\'ombra', camel: ['shadowHunt'], aliasIt: ['Shadow Hunt'] },
...
const FIXED_INNATE_SKILL_KEYS = (() => {
  const keys = new Set()
  for (const itLabel of SKILL_CATEGORIES.Speciali) {
    keys.add(normalizeSkillKey(itLabel))
  }
  for (const row of SKILL_DEFINITIONS) {
    if (!SKILL_CATEGORIES.Speciali.includes(row.it)) continue
