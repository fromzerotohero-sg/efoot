...
import { formatPhaseMatchupForPrompt } from './efootballV6TacticalModel'
import {
  TEAM_PLAYSTYLES,
  TEAM_PLAYSTYLE_ALIASES,
  getTeamPlaystyleLabel,
  resolveTeamStyleId
} from './efootballV6Rules'
...
/**
 * Nome display ufficiale di uno stile squadra v6 da ID o alias libero.
 * Autorità unica: lib/efootballV6Rules.js (6 stili, Overload/Pressing totale incluso).
 */
function teamStyleDisplayName(styleKey) {
  return getTeamPlaystyleLabel(resolveTeamStyleId(styleKey) || styleKey, 'it')
}
...
function buildMatchupEngineText({ opponentFormation, opponentPlayers, clientStarters, extractedProfile }) {
  ...
  return `\nMATCHUP_ENGINE (uso interno, non mostrare all'utente):\n- ${hints.join('\n- ')}\n- Regola anti-ripetizione: scegli la leva primaria in base a questi segnali; non proporre sempre compattezza/marcatura se il rischio principale e fascia, ampiezza o TRQ.\n`
}
...
export async function generateCountermeasuresPrompt(
  ...
  if (opponentCoach && opponentCoach.coach_name) {
    ...
    if (opponentCoach.playing_style_competence && typeof opponentCoach.playing_style_competence === 'object') {
      ...
      Object.entries(opponentCoach.playing_style_competence).forEach(([style, value]) => {
        const styleName = teamStyleDisplayName(style)
  ...
  const v6PhaseText = formatPhaseMatchupForPrompt({
    ...(playerPerformance?.v6Context || {}),
    clientStarters: titolari,
    stylesLookup
  })
  ...
  if (metaInfo.isMeta) {
    ...
    if (opponentFormation.playing_style?.toLowerCase().includes('quick counter') || 
      ...
      metaCountermeasures += `- Contro contropiede veloce / ripartenze: proteggi la profondità con struttura, copertura centrale e timing del pressing; Linea Bassa è legacy v6 e non va proposta\n`
      metaCountermeasures += `- Come piano in partita: evita pressing continuo e proteggi prima la zona centrale\n`
      metaCountermeasures += `- Se suggerisci prudenza difensiva, descrivi un comportamento pratico osservabile e non inventare una voce di menu\n`
    }
  }
...
LINGUA DI RISPOSTA: DEVI TASSATIVAMENTE RISPONDERE IN ${lang === 'en' ? 'INGLESE' : lang === 'es' ? 'ESPAÑOL' : 'ITALIANO'}. Qualsiasi testo, spiegazione o motivazione deve essere in ${lang === 'en' ? 'inglese' : lang === 'es' ? 'español' : 'italiano'}.
...
${opponentText}${rosterText}${formationText}${tacticalText}${coachText}${cardUsageText}${attilaMemoryAnalysis}${historyText}${similarFormationAnalysis}${playerPerformanceAnalysis}${tacticalHabitsAnalysis}${patternsText}${userExperienceText}${matchupEngineText}${v6PhaseText}${metaCountermeasures}
...
6. **ADEGUAMENTI TATTICI PRE-PARTITA (terminologia ufficiale + azione applicabile):**
   ...
   - **Stile squadra configurabile v6:** usa SOLO questi 6 nomi: Possesso palla, Contropiede veloce, Contrattacco, Passaggio lungo, Vie laterali, Pressing totale (Overload). NON Pressing Alto, Gegenpressing, Tiki-Taka, Catenaccio, Contenimento.
   - **Istruzioni individuali correnti v6:** Difensivo, Ancoraggio, Marcatura stretta, Marcatura uomo, Obiettivo Contropiede. Se proponi una di queste, preferisci l'array individual_instructions con player_id reale. Offensivo e Linea Bassa/Deep Line sono legacy: possono esistere nei dati salvati ma NON vanno proposte.
   - **Compatibilità v6:** se nei dati cliente compare Offensivo o Linea Bassa, trattali come storico legacy; non trasformarli in una raccomandazione corrente. Per prudenza/profondità usa un match_plan pratico o una istruzione corrente realmente applicabile.
...
8. **ISTRUZIONI INDIVIDUALI:**
   - Suggerisci SOLO questi ID tecnici correnti nel campo "instruction": difensivo, ancoraggio, marcatura_stretta, marcatura_uomo, contropiede. Non usare label con spazi nel JSON. NON generare offensivo o linea_bassa: sono legacy v6.
   - Etichette umane correnti: Difensivo, Ancoraggio, Marcatura stretta, Marcatura uomo, Contropiede. NO passaggi corti/cross.
   - Offensivo e Linea bassa: non più selezionabili in v6; se presenti nel profilo sono solo legacy. Contropiede (slot difesa): usa solo una assegnazione compatibile con il giocatore reale.
...
FOCUS OUTPUT:
- Massimo 6 azioni operative totali tra formation_adjustments + tactical_adjustments + player_suggestions + individual_instructions.
- Distribuzione ideale: 2-3 tactical_adjustments, 1-2 player_suggestions, 1-2 individual_instructions. Non riempire categorie se non hai azioni forti.
- Deve esserci almeno 1 tactical_adjustment valido quando esiste una leva tattica applicabile. Non lasciare l'utente solo con cambi/istruzioni se serve anche un piano tattico.
- Se hai un cambio giocatore valido, quello è una decisione principale: "Cambia X con Y".
- Se hai una istruzione individuale ufficiale valida, usa quella invece di una frase tattica generica.
- Non riempire categorie per forza. Meglio 2 azioni forti che 6 deboli.
- Ordine impatto: (1) sostituzione valida X→Y, (2) istruzione individuale ufficiale su titolare, (3) stile squadra ufficiale, (4) piano in partita, (5) cambio modulo solo ultima risorsa.
COME GIOCARLA:
- Aggiungi sempre "play_summary": mini guida pratica distinta dalle contromisure operative.
- Non è una voce menu: deve spiegare come interpretare la partita con il gameplay attuale.
- 5 campi obbligatori, max 1 frase ciascuno: match_key, base_plan, attacking, defending, avoid.
- match_key è la frase WOW: scegli UNA sola lettura dominante tra questi archetipi e adattala ai dati reali: manipola il blocco, rompi la prima pressione, togli uomo tra le linee, attacca terzino isolato, congela ritmo, colpisci seconda palla, proteggi primo passaggio verticale, attira mediano fuori posizione.
- match_key deve citare almeno una leva concreta: zona, reparto o giocatore reale della rosa. Vietate frasi generiche tipo "gioca con pazienza", "chiudi il centro", "usa ampiezza" senza dire come.
- Usa tono diretto e pratico. Vietato "se vuoi", "valuta", "considera".
COERENZA PIANO (OBBLIGATORIA — leggi prima di scrivere qualsiasi suggerimento):
- Ordine di generazione: scegli PRIMA match_key, POI genera tactical_adjustments, individual_instructions, player_suggestions e il resto di play_summary in funzione di quella chiave.
- match_key è il piano dominante: nessuna contromisura può spingere nella direzione opposta. Se la chiave dice corsie/lato debole, nessun piano può dire di costruire stabilmente per vie centrali; il centro può essere solo scarico temporaneo. Vale anche al contrario.
- Se servono sia centro che fascia, esprimili SEMPRE come sequenza esplicita: "prima..., poi...", "quando..., allora...", "solo dopo...". Mai due frasi assolute opposte.
- Non assegnare allo stesso giocatore due funzioni opposte in suggerimenti diversi (es. "ricevitore tra le linee" in una frase e "togli ricezione tra le linee" in un'altra).
- Vietate frasi in negativo ambigue tipo "togli ricezione tra le linee": scrivi cosa FARE, non cosa smettere di fare. L'unico campo in negativo è avoid.
- Prima di rispondere rileggi tutte le frasi insieme: se due suggerimenti si contraddicono, elimina o riscrivi quello più debole. Un output contraddittorio è un output sbagliato.

{
  "analysis": {
    ...
    "why_weaknesses": "Il loro attacco vs la nostra difesa (o la formazione unica se l'avversario non è fluido)"
  },
  "countermeasures": {
    ...
    "tactical_adjustments": [
      {
        ...
        "application_hint": "Game Plan → Tattica → Stile squadra. Usalo solo se il tuo allenatore ha competenza >= 70.",
        "reason": "Motivazione breve e applicabile (1 riga)",
        "priority": "medium"
      },
      {
        "type": "game_plan_adjustment",
        "suggestion": "In partita: proteggi prima la profondità e chiudi il corridoio centrale con il tuo MED/CC",
        "application_hint": "",
        "reason": "Motivazione breve e applicabile (1 riga)",
        "priority": "high"
      },
      {
        "type": "match_plan",
        "suggestion": "In partita: pressa solo a scatti e chiudi prima il centro con il tuo MED/CC",
        "application_hint": "",
        "reason": "Motivazione breve e applicabile (1 riga)",
        "priority": "high"
      },
      {
        "type": "match_plan",
        "suggestion": "In partita: esci dal pressing con cambio lato verso il giocatore libero indicato dalla rosa",
        "application_hint": "",
        "reason": "Motivazione breve e applicabile (1 riga)",
        "priority": "medium"
      }
    ...
    "player_suggestions": [
      {
        ...
        "replace_player_name": "OBBLIGATORIO se action=add_to_starting_xi: nome del titolare da sostituire (es: 'Marcelo')",
        "replace_position": "OBBLIGATORIO se action=add_to_starting_xi: ruolo in campo del titolare (es: 'CLS', 'TD') — stesso slot",
        "reason": "Motivazione breve 1 riga; se card riserva ≠ replace_position, indica la transizione",
        "priority": "high"
      }
    ],
    "individual_instructions": [
      {
        "slot": "attacco_1",
        "player_id": "uuid del TITOLARE dalla lista",
  ...
  "play_summary": {
    "match_key": "Chiave della partita: attira il loro MED fuori posizione e colpisci lo spazio che lascia alle spalle.",
    "base_plan": "Piano base: attira pressione, non forzare subito il centro e prepara cambio lato.",
    "attacking": "Quando attacchi: cerca ampiezza e verticalizza solo quando il TRQ/CC è libero.",
    "defending": "Quando difendi: non uscire aggressivo con i DC; scherma il centro e proteggi la profondità.",
    "avoid": "Errore da evitare: pressing continuo alto se salta il primo contrasto."
  },
  "confidence": 85,
  "data_quality": "high",
  ...
  "fluid_formation_recommendation": {
    ...
    "title": "titolo breve solo se formazione fluida è rilevante",
    "reason": "motivo decisivo",
    "attack_action": "azione concreta in attacco",
    "defense_action": "azione concreta in difesa"
  },
  "link_up_recommendations": [
    {"name":"nome Collegamento esistente","decision":"use|do_not_use|not_activatable|insufficient_data","focal_player":"titolare reale o vuoto","key_man_player":"titolare reale o vuoto","reason":"perché"}
  ]
}
...
- Priorità: SOLO "high" o "medium" (mai "low")
- player_suggestions: usa SOLO add_to_starting_xi per riserve e DEVI sempre indicare replace_player_id + replace_player_name (il titolare da sostituire). remove_from_starting_xi isolato è vietato e verrà scartato.
- tactical_adjustments: type consentiti = team_playing_style, game_plan_adjustment, match_plan. Evita defensive_line/pressing/possession_strategy salvo retrocompatibilita; se li usi, la suggestion deve comunque essere frase completa e applicabile.
- tactical_adjustments: aggiungi application_hint solo per team_playing_style o game_plan_adjustment. Per match_plan lascia application_hint vuoto: il cliente deve vedere solo consiglio e motivazione.
- individual_instructions: il campo instruction deve usare SOLO ID tecnici correnti v6: difensivo, ancoraggio, marcatura_stretta, marcatura_uomo, contropiede. NON generare offensivo o linea_bassa/Deep Line: sono valori legacy. Per passaggio alto, filtrante, cambio lato o cross, usa tactical_adjustments type "match_plan".
- Se dati insufficienti, imposta data_quality a "low" e aggiungi warnings
- play_summary: oggetto obbligatorio con match_key, base_plan, attacking, defending, avoid. Ogni valore deve essere una frase pratica "in partita", non impostazione di menu.
- base_plan, attacking e defending devono sviluppare la match_key, non introdurre un piano diverso. avoid deve indicare l'errore che rompe quella chiave.
- analysis.opponent_formation_analysis deve coprire il nostro ATTACCO vs la loro DIFESA (o la formazione unica se l'avversario non è fluido).
- analysis.why_weaknesses deve coprire il loro ATTACCO vs la nostra DIFESA.
- fluid_formation_recommendation e link_up_recommendations sono OPZIONALI. Ometti fluid_formation_recommendation se formazione fluida cliente = NO e l'avversario non ha una fase difesa. Ometti link_up_recommendations se non ci sono Collegamenti. Non inventare una seconda disposizione avversaria.
- Per ogni Collegamento usa ESATTAMENTE lo stato del contesto: attivabile / non attivabile con questi titolari / dati insufficienti per verificarlo. Dati insufficienti NON significa incompatibile.
- ENTERPRISE: Focus su robustezza. Se non sei sicuro di una voce menu, dillo come "In partita:" e non spacciarla per impostazione configurabile. Mai generico secco.
- Le contromisure operative sono modifiche configurabili PRIMA della partita (formazione, stile, giocatori, istruzioni). play_summary invece è guida pratica "in partita" e non va presentata come voce menu.
...
// Autorità unica: lib/efootballV6Rules.js (6 stili v6, Overload/Pressing totale incluso).
// Una voce per alias; alias più lunghi prima, così "long ball counter" vince su "long ball".
const OFFICIAL_TEAM_STYLES = TEAM_PLAYSTYLES
  .flatMap((style) =>
    [...new Set([style.id, style.it, style.en, ...(TEAM_PLAYSTYLE_ALIASES[style.id] || [])])]
      .map((alias) => ({
        id: style.id,
        label: getTeamPlaystyleLabel(style.id, 'it'),
        alias,
        pattern: new RegExp(`\\b${escapeRegExp(alias).replace(/[-_ ]+/g, '[-_ ]+')}\\b`, 'i')
      }))
  )
...
function extractOfficialTeamStyle(value) {
  ...
  for (const style of OFFICIAL_TEAM_STYLES) {
    if (style.pattern.test(text)) return style.label
  }
  return null
}
  ...
  for (const adj of tactical) {
    ...
    if (type === 'playing_style_change') {
      ...
      if (isTooVagueTacticalText(suggestion)) {
        adj.type = 'match_plan'
        adj.suggestion = 'In partita: proteggi la profondità con copertura centrale e pressing a scatti'
        adj.application_hint = ''
      }
    ...
    if (adj.type === 'team_playing_style') {
      ...
      if (officialStyle) {
        ...
        adj.application_hint = 'Non è uno dei 6 stili squadra correnti: trattalo come piano pratico in partita, non come impostazione.'
      }
    }
...
export function focusCountermeasuresOutput(output) {
  ...
  cm.player_suggestions = limitArray(cm.player_suggestions, 2)
  cm.individual_instructions = limitArray(cm.individual_instructions, 2)
  cm.tactical_adjustments = limitArray(cm.tactical_adjustments, 3)

  const selected = []
  const addGroup = (key, weight) => {
    for (const item of cm[key] || []) selected.push({ key, item, weight })
  }

  addGroup('tactical_adjustments', 0)
  addGroup('player_suggestions', 1)
  addGroup('individual_instructions', 2)
  addGroup('formation_adjustments', 3)

  selected.sort((a, b) => byPriority(a.item, b.item) || a.weight - b.weight)
  const keep = selected.slice(0, 6)
  const topTactical = selected.find(({ key }) => key === 'tactical_adjustments')
  if (topTactical && !keep.some(({ key }) => key === 'tactical_adjustments')) {
    // Mantieni sempre almeno una leva tattica: senza questa card l'output sembra solo rosa/istruzioni.
    keep[Math.max(0, keep.length - 1)] = topTactical
  }
  const keepByKey = {
    formation_adjustments: new Set(),
    tactical_adjustments: new Set(),
    player_suggestions: new Set(),
    individual_instructions: new Set()
  }
  keep.forEach(({ key, item }) => keepByKey[key].add(item))

  for (const key of Object.keys(keepByKey)) {
    cm[key] = (cm[key] || []).filter((item) => keepByKey[key].has(item))
  }

  if (output.analysis) {
    if (Array.isArray(output.analysis.strengths)) output.analysis.strengths = output.analysis.strengths.slice(0, 2)
    if (Array.isArray(output.analysis.weaknesses)) output.analysis.weaknesses = output.analysis.weaknesses.slice(0, 2)
  }
