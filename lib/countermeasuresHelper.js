...
export async function generateCountermeasuresPrompt(
  ...
  try {
    // Budget più ampio: evita troncamenti eccessivi del RAG in contromisure pre-partita.
    const attilaMemoryContent = getRelevantSectionsForContext('countermeasures', 26000)
  ...
  const defenseSubstitutionRules = hasTitolariRiserve && titolari.length > 0
   ...
   - In titolari: massimo 3 DC, massimo 1 TD, massimo 1 TS; **almeno 1 terzino (TD o TS)** — mai 4 DC e mai difesa senza terzino
   - Stato attuale: ${formatDefenseLineupSummary(titolari)}
   - La riserva **entra nello stesso slot/ruolo** del titolare uscente (replace_position = ruolo del titolare in lista TITOLARI, es. CLS, TD)
   - "position" nel JSON = ruolo **card** della riserva (es. DC); **replace_position** = ruolo **in campo** del titolare che esce
   - Se la card è DC ma lo slot è CLS/MED/CC: scrivi "Metti [riserva] al posto di [titolare] (CLS)" — NON dire "metti Thuram come DC" se sostituisci un centrocampista
   - Vietato: sostituire l'unico TD/TS con un DC se hai già 3 DC; vietato qualsiasi swap che porti a 4 DC o a 0 terzini
   - Nel "reason", se ruolo card ≠ ruolo slot, aggiungi una frase breve sulla transizione (es. "Card DC, entra nello slot CLS di Marcelo")\n`
    : ''
