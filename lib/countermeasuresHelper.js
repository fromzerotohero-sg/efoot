...
import { formatDefenseLineupSummary } from './formationDefenseRules'
...
export async function generateCountermeasuresPrompt(
  ...
  try {
    // Budget ampio: le contromisure devono vedere bene regole RAG e vincoli rosa.
    const attilaMemoryContent = getRelevantSectionsForContext('countermeasures', 50000)
  ...
  const defenseSubstitutionRules = hasTitolariRiserve && titolari.length > 0
   ...
   - Puoi avere **fino a 3 DC** (è valido e comune, es. 3-5-2). Regola chiave: se vuoi un **4° difensore** mentre hai già 3 DC, quel quarto deve essere **TD o TS**, mai DC
...
- individual_instructions: il campo instruction deve usare SOLO ID tecnici con underscore quando necessario: marcatura_stretta, marcatura_uomo, linea_bassa. Se vuoi parlare di passaggio alto, filtrante, cambio lato o cross, usa tactical_adjustments.
