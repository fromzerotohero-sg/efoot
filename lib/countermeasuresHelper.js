...
6. **ADEGUAMENTI TATTICI PRE-PARTITA (terminologia ufficiale + azione applicabile):**
   ...
   - **Stile squadra configurabile:** usa SOLO questi 5 nomi: Possesso palla, Contropiede veloce, Contrattacco, Passaggio lungo, Vie laterali. NON Pressing Alto, Gegenpressing, Tiki-Taka, Catenaccio, Contenimento.
   - **Istruzioni individuali ufficiali:** Offensivo, Difensivo, Ancoraggio, Marcatura stretta, Marcatura uomo, Obiettivo Contropiede, Linea Bassa. Se proponi una di queste, preferisci l'array individual_instructions con player_id reale.
   - **Distinzione obbligatoria:** "Linea Bassa" = istruzione individuale difensiva su un giocatore valido; NON scrivere "abbassa la difesa" o "linea difensiva bassa" senza spiegare se è una istruzione individuale o un comportamento pratico.
...
8. **ISTRUZIONI INDIVIDUALI:**
   - Suggerisci SOLO questi ID tecnici nel campo "instruction": offensivo, difensivo, ancoraggio, marcatura_stretta, marcatura_uomo, contropiede, linea_bassa. Non usare label con spazi nel JSON.
   - Etichette umane corrispondenti: Offensivo, Difensivo, Ancoraggio, Marcatura stretta, Marcatura uomo, Contropiede, Linea bassa. NO passaggi corti/cross.
  ...
  "countermeasures": {
    ...
    "tactical_adjustments": [
      ...
      },
      {
        ...
        "suggestion": "Istruzione individuale: Linea Bassa su un MED/CC titolare valido",
        "application_hint": "Per coprire la profondità usa individual_instructions con instruction=linea_bassa e player_id reale; non scrivere solo 'abbassa la difesa'.",
        "reason": "Motivazione breve e applicabile (1 riga)",
        "priority": "high"
      },
      {
...
- individual_instructions: il campo instruction deve usare SOLO ID tecnici con underscore quando necessario: marcatura_stretta, marcatura_uomo, linea_bassa. Per passaggio alto, filtrante, cambio lato o cross, usa tactical_adjustments type "match_plan".
  ...
  for (const adj of tactical) {
    ...
    if (adj.type === 'team_playing_style') {
      ...
      if (officialStyle) {
        ...
        adj.application_hint = 'Non è uno dei 5 stili squadra ufficiali: trattalo come piano pratico in partita, non come impostazione.'
      }
    }

    if (adj.type === 'team_playing_style') {
      ensureTextField(adj, 'application_hint', 'Game Plan → Tattica → Stile squadra. Usa solo Possesso palla, Contropiede veloce, Contrattacco, Passaggio lungo o Vie laterali.')
