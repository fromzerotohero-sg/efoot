...
FOCUS OUTPUT:
- Massimo 5 azioni operative totali tra formation_adjustments + tactical_adjustments + player_suggestions + individual_instructions.
- Distribuzione ideale: 1-2 tactical_adjustments, 1-2 player_suggestions, 1-2 individual_instructions. Non riempire categorie se non hai azioni forti.
...
- 4 campi obbligatori, max 1 frase ciascuno: base_plan, attacking, defending, avoid.
  ...
  "countermeasures": {
    ...
    "tactical_adjustments": [
      ...
      },
      {
        ...
        "application_hint": "Non è una voce menu: è il comportamento da usare quando l'avversario riceve tra le linee.",
        "reason": "Motivazione breve e applicabile (1 riga)",
        "priority": "high"
      },
      {
        ...
        "application_hint": "Non è una voce menu: è il piano pratico con i giocatori disponibili.",
        "reason": "Motivazione breve e applicabile (1 riga)",
        "priority": "medium"
      }
    ],
...
- tactical_adjustments: aggiungi application_hint quando serve chiarire dove applicare o quando non è una voce menu.
...
- play_summary: oggetto obbligatorio con base_plan, attacking, defending, avoid. Ogni valore deve essere una frase pratica "in partita", non impostazione di menu.
...
function toMatchPlanSuggestion(value) {
  ...
  if (text && !startsWithMatchPlan(text)) return `In partita: ${text.charAt(0).toLowerCase()}${text.slice(1)}`
  return text || 'In partita: applica questa indicazione come comportamento pratico, non come voce menu'
}
...
export function normalizeCountermeasureTerminology(output) {
  ...
  for (const adj of tactical) {
    ...
    if (type === 'playing_style_change') {
      ...
      ensureTextField(adj, 'application_hint', 'Non è una voce menu: è un piano pratico da applicare durante la partita.')
    }
    ...
    if (adj.type === 'game_plan_adjustment') {
      ...
      ensureTextField(adj, 'application_hint', 'Non è una voce menu: è il comportamento pratico da usare in partita.')
    }
  }
