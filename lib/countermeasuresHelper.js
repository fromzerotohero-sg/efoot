...
import { getRelevantSectionsForContext } from './ragHelper.js'
import { getPlayerStyleDisplayName, playingStylesMatch } from './playingStyleResolve.js'
import { formatDefenseLineupSummary, rolesAreEquivalent } from './formationDefenseRules.js'
import { getTruthLayerPromptBlock } from './efootballTruthLayer.js'
import { getSnapshotPlayerRole, startersForSnapshotPhase } from './clientFormationSnapshot.js'
import { buildHeroFluidPromptBlock } from './efootballV6TacticalModel.js'
import { getPlacementWarningLines, getPlacementWarningTitle } from './playerFieldPlacement.js'
import { buildMatchZonePromptBlock } from './matchAttackZones.js'
import {
  formatCoachTeamStylePromptBlock,
  getTeamPlayingStyleLabel,
  resolveTeamPlayingStyle
} from './teamPlayingStyles.js'
...
function strongestOpponentInRoles(players, roles) {
  const candidates = (Array.isArray(players) ? players : [])
    .filter((player) => roles.includes(normalizeRole(player?.position)))
    .sort((a, b) => (Number(b?.overall_rating) || 0) - (Number(a?.overall_rating) || 0))
  const player = candidates[0]
  if (!player) return null
  return {
    name: sanitizeForPrompt(player.player_name, 50) || null,
    role: normalizeRole(player.position) || null,
    overall: Number(player.overall_rating) || null
  }
}

export function buildOpponentTacticalSignature(opponentFormation, opponentPlayers, extractedProfile) {
  const shape = inferOpponentShape(
    opponentPlayers,
    opponentFormation?.formation_name,
    extractedProfile
  )
  const creator = strongestOpponentInRoles(opponentPlayers, ['TRQ', 'AMF'])
  const wideThreat = strongestOpponentInRoles(opponentPlayers, ['CLD', 'CLS', 'EDA', 'ESA', 'RWF', 'LWF', 'RMF', 'LMF'])
  const striker = strongestOpponentInRoles(opponentPlayers, ['P', 'CF', 'SP', 'SS'])
  const primaryThreat = creator || (shape.widthProfile === 'wide' ? wideThreat : null) || striker || wideThreat

  return {
    formation: sanitizeForPrompt(opponentFormation?.formation_name, 24) || 'N/A',
    shape,
    creator,
    wideThreat,
    striker,
...
export function assessOpponentFormationEvidence(opponentFormation, opponentPlayers, extractedProfile = null) {
  const players = Array.isArray(opponentPlayers) ? opponentPlayers : []
  const expected = formationNumbers(opponentFormation?.formation_name)
  const actual = countByRoleLine(players, '')
  const expectedDefense = expected[0] || 0
  const expectedForwards = expected.length ? expected[expected.length - 1] : 0
  const expectedMidfield = expected.length > 2
    ? expected.slice(1, -1).reduce((sum, value) => sum + value, 0)
    : (expected[1] || 0)
  const comparable = players.length >= 9 && expectedDefense > 0
  const conflicts = []

  if (comparable && Math.abs(actual.def - expectedDefense) > 1) {
    conflicts.push(`etichetta ${opponentFormation?.formation_name}: ${expectedDefense} difensori, ruoli letti: ${actual.def}`)
  }
  if (comparable && Math.abs(actual.mid - expectedMidfield) > 1) {
    conflicts.push(`etichetta ${opponentFormation?.formation_name}: ${expectedMidfield} centrocampisti, ruoli letti: ${actual.mid}`)
  }
  if (comparable && Math.abs(actual.fwd - expectedForwards) > 1) {
    conflicts.push(`etichetta ${opponentFormation?.formation_name}: ${expectedForwards} attaccanti, ruoli letti: ${actual.fwd}`)
  }
  const namedPlayers = players.filter((player) => sanitizeForPrompt(player?.player_name, 50)).length
  const geometricPlayers = players.filter((player) => (
    Number.isFinite(Number(player?.x_percent ?? player?.x)) &&
    Number.isFinite(Number(player?.y_percent ?? player?.y))
  )).length

  return {
    reliable: players.length >= 10 && namedPlayers >= 8 && conflicts.length === 0,
    conflicts,
    players: players.length,
    namedPlayers,
    geometricPlayers,
    confidence: Number(extractedProfile?.shape_confidence || extractedProfile?.formation_confidence || 0) || null
  }
}
...
function formatOpponentRosterText(players, formationName, extractedProfile) {
  ...
  sorted.forEach((p) => {
    ...
    const x = Number(p?.x_percent ?? p?.x)
    const y = Number(p?.y_percent ?? p?.y)
    const geometry = Number.isFinite(x) && Number.isFinite(y) ? `, coordinate (${Math.round(x)},${Math.round(y)})` : ''
    lines.push(`- Slot ${slot}: ${pos} - ${name}${ovr}${geometry}`)
  })
...
function buildMatchupEngineText({ opponentFormation, opponentPlayers, clientStarters, clientSnapshot, extractedProfile }) {
  const signature = buildOpponentTacticalSignature(opponentFormation, opponentPlayers, extractedProfile)
  const opponentShape = signature.shape
  const defensiveStarters = clientSnapshot
    ? startersForSnapshotPhase(clientSnapshot, 'defense')
    : (clientStarters || [])
  const clientShape = countByRoleLine(defensiveStarters, '')
  const evidence = assessOpponentFormationEvidence(opponentFormation, opponentPlayers, extractedProfile)
  const hints = []

  if (opponentShape.centralDensity === 'high') {
    hints.push(signature.creator?.name
      ? `rischio_principale: densita_centrale e ${signature.creator.name} (${signature.creator.role}) tra le linee`
      : 'rischio_principale: densita_centrale')
    hints.push('leva_preferita: valuta per matchup tutti i 10 giocatori di movimento; scegli il titolare più vicino e adatto nella fase DIFESA, senza preferire automaticamente MED/CC')
  }
  ...
  if (opponentShape.widthProfile === 'wide') {
    hints.push(signature.wideThreat?.name
      ? `rischio_principale: corsie esterne, riferimento ${signature.wideThreat.name} (${signature.wideThreat.role})`
      : 'rischio_principale: corsie esterne e isolamento terzini')
    hints.push('leva_preferita: copertura fascia/terzino + marcatura su esterno solo se concreto')
  }
  if (signature.striker?.name) {
    hints.push(`riferimento_offensivo_avversario: ${signature.striker.name} (${signature.striker.role})`)
  }
  if (signature.primaryThreat?.name) {
    hints.push(`minaccia_foto_specifica: ${signature.primaryThreat.name} (${signature.primaryThreat.role})`)
  }
  if (opponentShape.sideBias === 'left') {
    hints.push('sovraccarico_lato: sinistra avversaria; proteggi il tuo lato destro e valuta il lato debole opposto')
  } else if (opponentShape.sideBias === 'right') {
    hints.push('sovraccarico_lato: destra avversaria; proteggi il tuo lato sinistro e valuta il lato debole opposto')
  }
  ...
  if (opponentShape.defensiveGaps.length > 0) {
    hints.push(`gap_difensivi_foto: ${opponentShape.defensiveGaps.map(z => sanitizeForPrompt(z, 30)).join(', ')}`)
  }
  if (evidence.conflicts.length > 0) {
    hints.push(`incoerenza_dati_avversario: ${evidence.conflicts.join('; ')}; privilegia coordinate/profilo visuale e presenta l'etichetta modulo come ipotesi confermata dall'utente, non come geometria`)
  }

  if (hints.length === 0) return ''

  const candidates = (clientSnapshot?.starters || clientStarters || [])
    .filter((player) => !['PT', 'GK'].includes(normalizeRole(
      getSnapshotPlayerRole(clientSnapshot, player, 'defense') || player?.position
    )))
    .map((player) => {
      const base = getSnapshotPlayerRole(clientSnapshot, player, 'base') || player?.position || '?'
      const attack = getSnapshotPlayerRole(clientSnapshot, player, 'attack') || base
      const defense = getSnapshotPlayerRole(clientSnapshot, player, 'defense') || base
      return `${sanitizeForPrompt(player?.player_name, 45)} [slot ${player?.slot_index}: base ${base}, attacco ${attack}, difesa ${defense}]`
    })

  return `\nMATCHUP_ENGINE (uso interno, non mostrare all'utente):\n- firma_avversario: ${signature.formation}|${opponentShape.centralDensity}|${opponentShape.widthProfile}|${opponentShape.strikerSetup}|${opponentShape.sideBias}\n- qualita_snapshot_avversario: ${evidence.reliable ? 'coerente' : 'parziale'}; giocatori=${evidence.players}; nomi=${evidence.namedPlayers}; coordinate=${evidence.geometricPlayers}\n- ${hints.join('\n- ')}\n- Candidati reali valutabili (tutti i giocatori di movimento, ruoli per fase): ${candidates.join(' | ') || 'non disponibili'}\n- Regola scelta giocatore: confronta minaccia, lato/altezza dello slot nella fase pertinente, competenza carta e istruzioni già attive. Non scegliere un giocatore per reputazione, Overall o perché è MED/CC.\n- Regola anti-ripetizione: scegli la leva primaria in base a questi segnali; non proporre sempre compattezza o marcatura centrale se il rischio principale e fascia, ampiezza, due punte o un gap specifico.\n- Specificita obbligatoria: opponent_read, diagnosis e almeno una contromisura devono dipendere da un segnale di questa firma. Se il nome della minaccia e leggibile, usalo nella lettura o nella marcatura; non ridurlo al solo ruolo generico.\n`
}
...
export async function generateCountermeasuresPrompt(
  ...
  if (opponentCoach && opponentCoach.coach_name) {
    ...
    if (opponentCoach.playing_style_competence && typeof opponentCoach.playing_style_competence === 'object') {
      ...
      Object.entries(opponentCoach.playing_style_competence).forEach(([style, value]) => {
        const styleName = getTeamPlayingStyleLabel(style, 'it') || style
  ...
  const clientSnapshot = playerPerformance?.clientFormationSnapshot || null
  ...
  if (hasTitolariRiserve) {
    ...
    titolari.forEach((p, idx) => {
      const currentPosition = getSnapshotPlayerRole(clientSnapshot, p, 'base') || p.position
      const attackPosition = getSnapshotPlayerRole(clientSnapshot, p, 'attack') || currentPosition
      const defensePosition = getSnapshotPlayerRole(clientSnapshot, p, 'defense') || currentPosition
      ...
      const phaseRoles = clientSnapshot?.enabled
        ? ` | attacco ${attackPosition || '?'} | difesa ${defensePosition || '?'}`
        : ''
      rosterText += `- [${p.id}] ${p.player_name || 'N/A'} - base ${currentPosition || 'N/A'}${phaseRoles} - Overall ${p.overall_rating || 'N/A'}${skillsPart}${slot}${verifiedMarker}\n`
      if (cardLevers) {
    ...
    const placementWarnings = getPlacementWarningLines(titolari, lang, clientSnapshot?.fluid || null)
    if (placementWarnings.length > 0) {
      rosterText += `\n${getPlacementWarningTitle(lang)}\n${placementWarnings.join('\n')}\n`
    }
  ...
  if (clientSnapshot && titolari.length > 0) {
    formationText += `${buildHeroFluidPromptBlock({
      fluid: clientSnapshot.fluid,
      starters: titolari,
      lang,
      evidence: {
        recurringIssues: tacticalPatterns?.recurring_issues,
        feedbackRows: playerPerformance?.coachFeedback,
        gameAnalysis: playerPerformance?.gameAnalysis
      }
    })}\n`
    formationText += `- Difesa nella fase DIFESA (DC/TD/TS): ${formatDefenseLineupSummary(
      titolari,
      (player) => getSnapshotPlayerRole(clientSnapshot, player, 'defense')
    )}\n`
    ...
    formationText += `- Disposizione in campo (RIFERIMENTO PRINCIPALE): ${positionsOrdered || 'N/A'}\n`
    formationText += `- Difesa attuale (DC/TD/TS in titolari): ${formatDefenseLineupSummary(titolari)}\n`
  } else if (clientFormation) {
  ...
  if (tacticalSettings) {
    tacticalText += `- Team Playing Style: ${getTeamPlayingStyleLabel(tacticalSettings.team_playing_style, 'it') || tacticalSettings.team_playing_style || 'N/A'}\n`
  ...
  if (activeCoach) {
    ...
    if (activeCoach.playing_style_competence && typeof activeCoach.playing_style_competence === 'object') {
      const currentTeamStyle = tacticalSettings?.team_playing_style || playerPerformance?.team_playing_style || null
      coachText += `\n${formatCoachTeamStylePromptBlock(activeCoach.playing_style_competence, currentTeamStyle, 'it')}\n`
    }
  ...
  try {
    ...
    const truthBlock = getTruthLayerPromptBlock('it')
    ...
    if (hasTitolariRiserve && (titolari.length > 0 || riserve.length > 0)) {
      ...
      if (truthBlock) {
        attilaMemoryAnalysis += `${truthBlock}\n\n`
      }
  ...
  const matchZonesText = buildMatchZonePromptBlock(matchHistory || [], lang)
  const tacticalZoneEvidence = tacticalPatterns && (
    tacticalPatterns.our_attack_areas_avg ||
    tacticalPatterns.opponent_attack_areas_avg ||
    tacticalPatterns.conceded_goal_zones_avg
  )
    ? `\nZONE AGGREGATE (fonti distinte; non confonderle):\n- Attacco cliente: ${JSON.stringify(tacticalPatterns.our_attack_areas_avg || tacticalPatterns.attack_areas_avg || {})}\n- Attacco avversario / pressione concessa: ${JSON.stringify(tacticalPatterns.opponent_attack_areas_avg || {})}\n- Zone gol subiti: ${JSON.stringify(tacticalPatterns.conceded_goal_zones_avg || {})}\n`
    : ''
  ...
  if (metaInfo.isMeta) {
    ...
    if (opponentFormation.playing_style?.toLowerCase().includes('quick counter') || 
      ...
      metaCountermeasures += `- Contro contropiede veloce / ripartenze: proteggi la profondità con disposizione, marcature e coperture compatibili; Linea Bassa è rimossa in v6.0.0 e non va suggerita\n`
...
- Aggiungi un solo comportamento semplice CON PALLA e uno SENZA PALLA, coerenti con il setup. Niente minutaggio, radiocronaca o coaching live.
...
- Formazione fluida: usa lo stato reale nel blocco cliente. Se ATTIVA valuta separatamente ATTACCO e DIFESA e non proporre di attivarla. Se NON ATTIVA, suggeriscila solo quando il blocco autorizza esplicitamente la valutazione.
- Selezione giocatori: valuta tutti i 10 giocatori di movimento in base al matchup e al ruolo della fase. VIETATO preferire automaticamente MED/CC, nomi famosi o Overall alti.
- Istruzioni già attive: non riproporle come cambiamento. Se una decisione le conferma, dichiarala come mantenimento; assegna nuove istruzioni solo a titolari reali.
- NON suggerire Offensivo o Linea bassa: rimossi in v6.0.0.

${opponentText}${rosterText}${formationText}${tacticalText}${coachText}${cardUsageText}${attilaMemoryAnalysis}${historyText}${matchZonesText ? `\n${matchZonesText}\n` : ''}${tacticalZoneEvidence}${similarFormationAnalysis}${playerPerformanceAnalysis}${tacticalHabitsAnalysis}${patternsText}${userExperienceText}${matchupEngineText}${metaCountermeasures}
...
8. Il micro-piano con/senza palla deve descrivere un'intenzione tattica semplice, non una sequenza tecnica di dribbling, tiri o comandi live.
...
4. **SUGGERIMENTI PERSONALIZZATI (CRITICO):**
   ...
   - Rispetta lo stile squadra già attivo se la competenza allenatore è >= 70. Non spingere Possesso palla per default e non scendere a uno stile più debole del coach.
   ${activeCoach && activeCoach.playing_style_competence ? `⚠️ VINCOLO ALLENATORE (OBBLIGATORIO):
   - Usa l'IDENTITÀ dell'allenatore (stili con competenza massima), non qualunque stile >= 70.
   - Se lo stile attuale è già un'identità o comunque più forte della proposta, MANTIENILO.
   - Esempio Lampard: Pressing totale 89, Passaggio lungo 89, Possesso palla 75, Vie laterali 69
     → ✅ MANTIENI Pressing totale se già attivo
     → ✅ Passaggio lungo solo se serve un cambio di pari identità
     → ❌ Possesso palla 75 è un downgrade, non un upgrade
     → ❌ Vie laterali 69 è vietato` : `- Se allenatore configurato, verifica competenze prima di suggerire stile.`}
...
5. **PRIORITÀ LEVE (ORDINE OBBLIGATORIO – il cliente non può cambiare modulo ogni partita):**
   ...
   - (3) **Stile squadra** – Solo per un upgrade o un cambio di pari identità. Mai un downgrade (es. Pressing totale 89 → Possesso 75).
...
6. **ADEGUAMENTI TATTICI PRE-PARTITA (terminologia ufficiale + azione applicabile):**
   ...
   - Nei match_plan non scrivere cambi ruolo ("come CLD", "come TRQ", "per cambiare ruolo"). Scrivi il comportamento pratico usando SOLO un nome reale letto nella rosa di questa richiesta.
...
8. **ISTRUZIONI INDIVIDUALI:**
   ...
   - Per marcatura_stretta o marcatura_uomo aggiungi sempre \`target_player_name\`: nome esatto di un avversario presente nello XI letto dalla foto. Non scrivere "una punta" se il nome è disponibile.
...
- Massimo 1 stile squadra ufficiale. Se lo stile attuale è già l'identità del coach, OMETTI il cambio o confermalo come mantenimento. Mai un downgrade verso Possesso o un altro stile più debole.
- Massimo 1 cambio formazione e solo se davvero necessario.
- with_ball e without_ball: array di 2 micro-azioni concrete ciascuno, con giocatori o zone reali. Le quattro azioni devono essere complementari, non parafrasi.
- plan_b: un solo trigger osservabile e una sola risposta. Deve cambiare la leva, non riscrivere il piano principale.
- starting_plan: mantienilo per compatibilità, ma usa solo le stesse indicazioni di with_ball/without_ball senza aggiungere duplicati.
...
- diagnosis, with_ball e without_ball NON devono ripetere la stessa frase con parole diverse.
- Qualità minima obbligatoria: se gli 11 nomi avversari sono leggibili, almeno una voce di without_ball deve nominare il bersaglio reale. Una marcatura senza "su [nome avversario]" è incompleta.
- Se dalla foto è leggibile una minaccia avversaria, nominala almeno una volta. Vietato ridurre tutto a "la punta", "il TRQ" o "la zona centrale".
...
- I valori tra parentesi angolari nel JSON seguente descrivono il contratto: sostituiscili sempre con dati reali di QUESTA richiesta. Non copiarli e non riusare nomi dagli esempi precedenti.

{
  "opponent_read": {
    "formation": "<modulo letto>",
    "trait": "<caratteristica dominante specifica della foto>",
    "assumption": "<unica ipotesi prudente, oppure stringa vuota>"
  },
  "diagnosis": "<una sola decisione centrale specifica>",
  "fit_proof": "<prova concreta usando un nome reale della rosa cliente>",
  "with_ball": [
    "<prima micro-azione con palla usando un titolare reale>",
    "<seconda micro-azione che indica lato, ricevitore o spazio>"
  ],
  "without_ball": [
    "<prima micro-azione con bersaglio avversario reale se leggibile>",
    "<seconda micro-azione di copertura complementare>"
  ],
  "plan_b": {
    "trigger": "<un cambiamento avversario osservabile>",
    "action": "<una risposta diversa e applicabile>"
  },
  "analysis": {
...
- with_ball e without_ball: esattamente 2 voci ciascuno quando la foto contiene 11 giocatori; specifici per questa foto, linguaggio umano, senza minutaggio.
- starting_plan: massimo 2 frasi, copia esattamente with_ball e without_ball per compatibilità.
...
const OFFICIAL_TEAM_STYLES = [
  { label: 'Possesso palla', patterns: [/possesso palla/i, /possession game/i, /possesso_palla/i] },
  { label: 'Contropiede veloce', patterns: [/contropiede veloce/i, /quick counter/i, /contropiede_veloce/i] },
  { label: 'Contrattacco', patterns: [/\bcontrattacco\b/i, /long ball counter/i] },
  { label: 'Passaggio lungo', patterns: [/passaggio lungo/i, /passaggio_lungo/i, /long ball(?! counter)/i] },
  { label: 'Vie laterali', patterns: [/vie laterali/i, /vie_laterali/i, /out wide/i] },
  { label: 'Pressing totale', patterns: [/pressing totale/i, /pressing_totale/i, /pressione totale/i, /\boverload\b/i, /sovraccarico/i, /superioridad/i, /presi[oó]n total/i] }
]
...
function extractOfficialTeamStyle(value) {
  const resolved = resolveTeamPlayingStyle(asText(value))
  if (resolved) return resolved.it
  ...
  for (const adj of tactical) {
    ...
    if (type === 'playing_style_change') {
      ...
      if (isTooVagueTacticalText(suggestion)) {
        adj.suggestion = 'Mantieni più corta la distanza tra centrocampo e difesa nella fase senza palla'
        adj.application_hint = 'Regola la disposizione difensiva nel Game Plan o nella fase DIFESA della Formazione fluida; non usare Linea bassa, rimossa in v6.0.0.'
      }
