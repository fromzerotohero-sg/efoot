...
function anchorComparePrefix(anchor, lang) {
  ...
  if (anchor.type === 'same_name' || anchor.type === 'same_field_role') {
    return isEn ? `Vs ${anchor.displayLabel}` : isEs ? `Vs ${anchor.displayLabel}` : `Rispetto a ${anchor.displayLabel}`
  }
  if (anchor.type === 'same_family') {
    return isEn
      ? `Vs ${anchor.displayLabel} (fallback — same line, no ${anchor.cardRole} on your layout)`
      : isEs
      ? `Vs ${anchor.displayLabel} (alternativa — misma línea, sin ${anchor.cardRole} en tu esquema)`
  ...
  if (anchor.type === 'same_wide_flank') {
    return isEn
      ...
      : isEs
      ? `Vs ${anchor.displayLabel} (rol de banda en tu plantilla)`
  ...
  if (anchor.type === 'natural_competence_only') {
    return isEn
      ...
      : isEs
      ? `Vs ${anchor.displayLabel} (solo comparación de habilidades — no es tu ${anchor.cardRole} titular)`
...
/**
 * Conteggio skill distinte carta vs titolare (per upgrade rosa, non solo numero totale skill).
 * @returns {{ onlyOnCard: number, shared: number, almostSame: boolean }}
 */
export function summarizeSkillDelta(card, catalogCard, starterPlayer) {
  const cardSkills = collectCardSkills(card, catalogCard)
  if (!starterPlayer || cardSkills.length === 0) {
    return { onlyOnCard: cardSkills.length, shared: 0, almostSame: false }
  }
  const rawStarterSkills = Array.isArray(starterPlayer?.skills)
    ? [
        ...(starterPlayer.skills || []),
        ...(Array.isArray(starterPlayer.com_skills) ? starterPlayer.com_skills : [])
      ]
    : getMergedPlayerSkills(starterPlayer)
  const starterSkills = uniqueSkillKeys(rawStarterSkills)
  const { shared, onlyOnCard } = diffSkillSets(cardSkills, starterSkills)
  const almostSame = onlyOnCard.length === 0 && shared.length >= 2
  return {
    onlyOnCard: onlyOnCard.length,
...
function formatSkillList(skills, lang, max = 2) {
  return skills
    .slice(0, max)
    .map((s) => getSkillDisplayLabel(s.canon, lang === 'en' ? 'en' : lang === 'es' ? 'es' : 'it'))
    .filter(Boolean)
    .join(lang === 'en' ? ' and ' : lang === 'es' ? ' y ' : ' e ')
}
...
function sharedPhrase(shared, lang) {
  ...
  if (defCount >= 3) {
    return lang === 'en' ? 'the same defensive core' : lang === 'es' ? 'la misma base defensiva' : 'la stessa base difensiva'
  }
  ...
  return lang === 'en' ? `shared ${labels}` : lang === 'es' ? `comparte ${labels}` : `condividi ${labels}`
}
...
export function buildSkillDeltaSentence({
  ...
  if (!starter || cardSkills.length === 0) {
    ...
    return isEn
      ...
      : isEs
      ? `Habilidades distintivas de la carta: ${labels}.`
  ...
  if (!cardOnlyLabels && shared.length >= 2) {
    const rotationNote = premium
      ? isEn
        ...
        : isEs
        ? ' Sigue siendo válida como rotación de élite en una línea concurrida — valora herramientas distintivas, físico, conexión y plan de partido; no es un descarte automático.'
      ...
      : ''
    return isEn
      ? `${comparePrefix}, skill profile is almost the same (${sharedPart}): new skills are not the main buy reason; judge the card from distinct tools, stats, traits and rotation.${rotationNote}`
      : isEs
      ? `${comparePrefix}, el perfil de habilidades es casi igual (${sharedPart}): las habilidades nuevas no son el motivo principal; valora la carta por herramientas distintivas, stats, rasgos y rotación.${rotationNote}`
      : `${comparePrefix}, il profilo abilità è quasi uguale (${sharedPart}): le skill nuove non sono il motivo principale; valuta la carta da tool distintivi, stats, tratti e rotazione.${rotationNote}`
  }
  ...
  if (!cardOnlyLabels) {
    ...
    if (lost) {
      return isEn
        ...
        : isEs
        ? `${comparePrefix}, ninguna habilidad nueva en esta carta; perderías ${lost} de tu titular.`
    ...
    return isEn
      ...
      : isEs
      ? `${comparePrefix}, las habilidades coinciden — cualquier mejora debe venir del uso de rol, perfil premium, stats o rasgos especiales, no de nuevas habilidades.`
  ...
  if (sharedPart) {
    return isEn
      ? `${comparePrefix}, you keep ${sharedPart}; the real gain is ${cardOnlyLabels}.`
      : isEs
      ? `${comparePrefix}, mantienes ${sharedPart}; la ganancia real es ${cardOnlyLabels}.`
      : `${comparePrefix}, ${sharedPart}; il salto vero è ${cardOnlyLabels}.`
  }

  return isEn
    ? `${comparePrefix}, the gain is ${cardOnlyLabels}.`
    : isEs
    ? `${comparePrefix}, el valor añadido es ${cardOnlyLabels}.`
    : `${comparePrefix}, il valore aggiunto è ${cardOnlyLabels}.`
}
