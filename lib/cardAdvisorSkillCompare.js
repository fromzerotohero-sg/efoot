...
function anchorComparePrefix(anchor, lang) {
  ...
  const name = String(anchor.player.player_name || '').trim()
  const isEn = lang === 'en'
  if (anchor.type === 'same_field_role') {
    return isEn ? `Vs ${anchor.displayLabel}` : `Rispetto a ${anchor.displayLabel}`
  }
  if (anchor.type === 'same_flank_zone') {
    const zone = isEn ? 'your current flank player' : 'chi copre la fascia oggi in campo'
    return isEn
      ? `Vs ${anchor.displayLabel} (${zone})`
      : `Rispetto a ${anchor.displayLabel} (${zone})`
  }
  ...
  return isEn ? `Vs ${name}` : `Rispetto a ${name}`
}
...
export function buildSkillDeltaSentence({
  ...
  const anchor = pickComparisonAnchor(players, card?.position)
  ...
  if (!cardOnlyLabels && shared.length >= 2) {
    return isEn
      ? `${comparePrefix}, skill profile is almost the same (${sharedPart}): you are not buying new abilities, only style or roster fit.`
      : `${comparePrefix}, il profilo abilità è quasi uguale (${sharedPart}): non compri per skill nuove, ma per stile o fit in rosa.`
  }
  ...
  if (!cardOnlyLabels) {
    ...
    return isEn
      ? `${comparePrefix}, abilities match closely — upgrade is about role use, not new skills.`
      : `${comparePrefix}, le abilità coincidono: l'upgrade è sul ruolo, non su skill diverse.`
  }
