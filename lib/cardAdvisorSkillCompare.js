...
function playerSupportsPosition(player, targetPosition) {
  ...
  if (String(player?.position || '').trim().toUpperCase() === target) return true
  ...
  return positions.some((entry) => {
    ...
    return (
      String(position || '').trim().toUpperCase() === target &&
      (!competence || competence.includes('alta') || competence.includes('high'))
    )
  })
}

function pickStarterForComparison(players, cardPosition) {
  const position = String(cardPosition || '').trim().toUpperCase()
  const candidates = (players || []).filter(
    (player) => player?.player_name && (player.position === position || playerSupportsPosition(player, position))
  )
  candidates.sort((a, b) => {
    ...
    if (b.position === position && a.position !== position) return 1
...
export function buildSkillDeltaSentence({
  ...
  const starter = pickStarterForComparison(players, card?.position)
  ...
  const starterName = String(starter.player_name || '').trim()
  ...
  if (!cardOnlyLabels && shared.length >= 2) {
    return isEn
      ? `Vs ${starterName}, skill profile is almost the same (${sharedPart}): you are not buying new abilities, only style or roster fit.`
      : `Rispetto a ${starterName}, il profilo abilità è quasi uguale (${sharedPart}): non compri per skill nuove, ma per stile o fit in rosa.`
  }
  ...
  if (!cardOnlyLabels) {
    ...
    if (lost) {
      return isEn
        ? `Vs ${starterName}, no new skills on this card; you would lose ${lost} from your starter.`
        : `Rispetto a ${starterName}, nessuna skill nuova sulla carta; perderesti ${lost} del titolare.`
    }
    return isEn
      ? `Vs ${starterName}, abilities match closely — upgrade is about role use, not new skills.`
      : `Rispetto a ${starterName}, le abilità coincidono: l'upgrade è sul ruolo, non su skill diverse.`
  }
  ...
  if (sharedPart) {
    return isEn
      ? `Vs ${starterName}, you keep ${sharedPart}; the real gain is ${cardOnlyLabels}${tail}${prem}.`
      : `Rispetto a ${starterName}, ${sharedPart}; il salto vero è ${cardOnlyLabels}${tail}${prem}.`
  }

  return isEn
    ? `Vs ${starterName}, the gain is ${cardOnlyLabels}${tail}${prem}.`
    : `Rispetto a ${starterName}, il valore aggiunto è ${cardOnlyLabels}${tail}${prem}.`
}
