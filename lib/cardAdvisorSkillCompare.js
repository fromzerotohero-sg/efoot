...
function anchorComparePrefix(anchor, lang) {
  ...
  if (anchor.type === 'same_family') {
    return isEn
      ? `Vs ${anchor.displayLabel} (same line in your squad)`
      : `Rispetto a ${anchor.displayLabel} (stesso reparto in rosa)`
  }
...
export function buildSkillDeltaSentence({
  ...
  const anchor = pickComparisonAnchor(players, card?.position, card?.name)
