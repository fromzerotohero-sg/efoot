...
export function buildSkillDeltaSentence({
  ...
  if (!cardOnlyLabels && shared.length >= 2) {
    return isEn
      ? `${comparePrefix}, skill profile is almost the same (${sharedPart}): new skills are not the reason to buy; judge premium value from style, stats, booster/showtime traits and roster fit.`
      : `${comparePrefix}, il profilo abilità è quasi uguale (${sharedPart}): le skill nuove non sono il motivo d'acquisto; valuta valore premium da stile, stats, booster/showtime e fit in rosa.`
  }
