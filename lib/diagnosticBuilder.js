import { getPlayerStyleDisplayName } from './playingStyleResolve.js'
import { formatBuildCoachSnippet, formatBuildProgressionSection } from './playerBuildCoachPrompt.js'
import { getPlayerDisplayStats } from './playerEffectiveStats.js'
import { buildRosterSkillAdvisorySection, formatPlayerSkillContext } from './rosterSkillsContext.js'
import { fieldPositionMatchesCardCompetences } from './playerSlotRoleMetadata.js'
import { collectCoachLinkUps, formatCoachLinkUpsForPrompt } from './efootballTruthLayer.js'
import {
  buildFluidFormationState,
  buildHeroFluidPromptBlock,
  startersForLinkUpVerification
} from './efootballV6TacticalModel.js'
import { formatStarterPlacementToken, getPlacementWarningLines, getPlacementWarningTitle } from './playerFieldPlacement.js'
import {
  formatCompactZonePair,
  formatZonePct,
  resolveMatchAttackZones,
  summarizeMatchAttackZones
} from './matchAttackZones.js'
...
function formatCompetenzeForPrompt(originalPositions, lang) {
  const empty = (lang === 'en' || lang === 'es') ? 'not set' : 'non impostate'
...
/** Match coach Connection Focal Point / Key Man against roster. Exported for chat fallback. */
export function matchConnectionToRoster(connection, roster, stylesLookup) {
...
function buildGameAnalysisText(gameAnalysisRow, lang) {
  ...
  fmt(stats.goal_types, (lang === 'en' || lang === 'es') ? 'Goal types' : 'Tipo gol')
  fmt(stats.shot_usage, (lang === 'en' || lang === 'es') ? 'Shot usage' : 'Tiro')
  fmt(stats.special_commands, (lang === 'en' || lang === 'es') ? 'Special commands' : 'Comandi speciali')
  fmt(stats.passing, (lang === 'en' || lang === 'es') ? 'Passing' : 'Passaggio')
  fmt(stats.dribbling, (lang === 'en' || lang === 'es') ? 'Dribbling' : 'Dribbling')
  fmt(stats.defense, (lang === 'en' || lang === 'es') ? 'Defense' : 'Difesa')
...
function buildMatchDerivedSection(matches, patternsRow, lang) {
  ...
  if (matchesWithData.length > 0) {
    ...
    const parts = []
    const zoneSummary = summarizeMatchAttackZones(matchesWithData)
    const oursPct = formatZonePct(zoneSummary.oursAvg, lang)
    const theirsPct = formatZonePct(zoneSummary.theirsAvg, lang)
    if (oursPct) {
      parts.push(
        (lang === 'en' || lang === 'es')
          ? `Your attack zones (avg, ${zoneSummary.oursCount} matches): ${oursPct}`
          : `Zone attacco tue (media, ${zoneSummary.oursCount} partite): ${oursPct}`
      )
    }
    if (theirsPct) {
      parts.push(
        (lang === 'en' || lang === 'es')
          ? `Opponent attack / pressure conceded (avg, ${zoneSummary.theirsCount} matches; NOT conceded-goal locations): ${theirsPct}`
          : `Attacco avversario / pressione concessa (media, ${zoneSummary.theirsCount} partite; NON sono zone dei gol subiti): ${theirsPct}`
      )
    }
    if (oursPct || theirsPct) {
      parts.push(
        (lang === 'en' || lang === 'es')
          ? 'Conceded-goal zones: not in database'
          : 'Zone dei gol subiti: non in database'
      )
    }
    if (withRatings > 0) parts.push(((lang === 'en' || lang === 'es') ? 'Player ratings present for' : 'Voti giocatori presenti per') + ` ${withRatings} ${(lang === 'en' || lang === 'es') ? 'matches' : 'partite'}`)
    if (parts.length) lines.push(parts.join('. '))
  }
  const patterns = patternsRow || {}
  if (patterns.our_attack_areas_avg && typeof patterns.our_attack_areas_avg === 'object' && Object.keys(patterns.our_attack_areas_avg).length > 0 && lines.every(l => !l.includes('Zone attacco tue') && !l.includes('Your attack zones'))) {
    const pct = formatZonePct(patterns.our_attack_areas_avg, lang)
    if (pct) lines.push(((lang === 'en' || lang === 'es') ? 'Your attack zones (pattern avg)' : 'Zone attacco tue (media pattern)') + ': ' + pct)
  }
  if (patterns.opponent_attack_areas_avg && typeof patterns.opponent_attack_areas_avg === 'object' && Object.keys(patterns.opponent_attack_areas_avg).length > 0 && lines.every(l => !l.includes('pressione concessa') && !l.includes('pressure conceded'))) {
    const pct = formatZonePct(patterns.opponent_attack_areas_avg, lang)
    if (pct) {
      lines.push(
        (lang === 'en' || lang === 'es')
          ? `Opponent attack / pressure conceded (pattern avg; NOT conceded-goal locations): ${pct}`
          : `Attacco avversario / pressione concessa (media pattern; NON zone gol subiti): ${pct}`
      )
    }
  }
  if (patterns.attack_areas_avg && typeof patterns.attack_areas_avg === 'object' && Object.keys(patterns.attack_areas_avg).length > 0 && lines.every(l => !l.includes('Zone attacco tue') && !l.includes('Your attack zones'))) {
    const entries = Object.entries(patterns.attack_areas_avg)
      .filter(([, v]) => v != null)
      .map(([k, v]) => `${sanitizeForPrompt(k, 15)}: ${v}`)
    if (entries.length) lines.push(((lang === 'en' || lang === 'es') ? 'Attack zones (pattern avg, unlabeled source)' : 'Zone attacco (media pattern, fonte non distinta)') + ': ' + entries.join(', '))
  }
  if (patterns.recovery_zones_avg && (Array.isArray(patterns.recovery_zones_avg) ? patterns.recovery_zones_avg.length > 0 : typeof patterns.recovery_zones_avg === 'object' && Object.keys(patterns.recovery_zones_avg).length > 0)) {
    ...
    lines.push(((lang === 'en' || lang === 'es') ? 'Recovery zones (avg)' : 'Zone recupero palla (media)') + ': ' + summary)
  }
  return lines.length ? lines.join('. ') : ''
}
...
function buildRecentLossesSection(matches, oppFormationsMap, lang) {
  ...
  for (const m of losses.slice(0, 4)) {
    ...
    const oursPct = formatZonePct(split.ours, lang)
    const theirsPct = formatZonePct(split.theirs, lang)
    if (oursPct) {
      parts.push((lang === 'en' || lang === 'es') ? `your attack: ${oursPct}` : `attacco tuo: ${oursPct}`)
    }
    if (theirsPct) {
      parts.push((lang === 'en' || lang === 'es') ? `opponent attack/pressure: ${theirsPct}` : `pressione avversaria: ${theirsPct}`)
    }
  ...
  const title = (lang === 'en' || lang === 'es') ? 'Recent losses (for "why did I lose?" analysis):' : 'Ultime sconfitte (per analisi "perché ho perso?"):'
  return `${title}\n${lines.join('\n')}`
}
...
function buildAiInfoLines(profile, lang) {
  ...
  add((lang === 'en' || lang === 'es') ? 'Name' : 'Nome', profile.first_name)
  add((lang === 'en' || lang === 'es') ? 'Call AI' : 'Chiamare l\'IA', profile.ai_name)
  add((lang === 'en' || lang === 'es') ? 'Division' : 'Divisione', profile.current_division)
  if (profile.hours_per_week != null && profile.hours_per_week !== '') add((lang === 'en' || lang === 'es') ? 'Hours/week' : 'Ore/settimana', String(profile.hours_per_week))
  addSelect((lang === 'en' || lang === 'es') ? 'Connection' : 'Connessione', 'connection_quality', profile.connection_quality)
  addSelect((lang === 'en' || lang === 'es') ? 'Slow opponent connection issues' : 'Difficoltà con avversari lenti', 'slow_opponent_connection_issues', profile.slow_opponent_connection_issues)
  addSelect((lang === 'en' || lang === 'es') ? 'Input delay' : 'Ritardo input', 'input_delay', profile.input_delay)
  addSelect((lang === 'en' || lang === 'es') ? 'Pass level' : 'Livello passaggi', 'pass_level', profile.pass_level)
  addSelect((lang === 'en' || lang === 'es') ? 'Smart assist' : 'Smart assist', 'smart_assist', profile.smart_assist)
  addSelect((lang === 'en' || lang === 'es') ? 'Platform' : 'Piattaforma', 'platform', profile.platform)
  add((lang === 'en' || lang === 'es') ? 'Favourite player' : 'Giocatore preferito', profile.favourite_player_name)
  const weak = profile.ai_weak_point
  if (weak != null && String(weak).trim() !== '') {
    const v = String(weak).trim().toLowerCase()
    lines.push(`${(lang === 'en' || lang === 'es') ? 'Weak point' : 'Punto debole'}: ${(labels.ai_weak_point && labels.ai_weak_point[v]) || sanitizeForPrompt(weak, 60)}`)
  }
  add((lang === 'en' || lang === 'es') ? 'Learn goals' : 'Cosa vuole imparare', profile.ai_learn_goals)
  add((lang === 'en' || lang === 'es') ? 'Notes for AI' : 'Note per l\'IA', profile.ai_notes)
  return lines
}
...
ISTRUZIONI (obbligatorie): Usa SOLO i dati sotto. Manca dato → non inventare. Dati=INDICATORI, vietato "X perché Y". Progression/Skill Training max 5 Additional Skills: non inventare slider. Rosa: solo nomi elencati. Stile squadra: 6 (Possesso, Contropiede veloce, Contrattacco, Passaggio lungo, Vie laterali, Pressing totale). Istruzioni: niente Offensivo/Linea bassa. Riassunto generale: non analisi partita singola.
...
INSTRUCTIONS (required): Use ONLY the data below. Missing data → don't invent. Data=INDICATORS, never "X because Y". Progression/Skill Training max 5 Additional Skills: do not invent sliders. Roster: only names listed. Team style: 6 (Possession, Quick Counter, Long Ball Counter, Long Ball, Out Wide, Overload). Instructions: never Attacking/Deep Line. General summary: not single match analysis.
...
export function buildDiagnostic(lang, data) {
  const L = (lang === 'en' || lang === 'es') ? LABELS_EN : LABELS_IT
  const sections = []

  // 0. Blocco ISTRUZIONI PER L'IA (sempre per primo; guida ragionamento chat)
  sections.push((lang === 'en' || lang === 'es') ? INSTRUCTIONS_BLOCK_EN : INSTRUCTIONS_BLOCK_IT)

  // 1. Profilo (sanitizza campi da DB)
  const profile = data.profile || {}
  const firstName = sanitizeForPrompt(profile.first_name, 80) || ((lang === 'en' || lang === 'es') ? 'User' : 'Utente')
  const teamName = sanitizeForPrompt(profile.team_name, 80) || ((lang === 'en' || lang === 'es') ? 'your team' : 'il tuo team')
  ...
  // 2. Rosa (sintesi)
  const formation = data.formation || ((lang === 'en' || lang === 'es') ? 'not set' : 'non impostata')
  ...
  const fluid = data.fluid || buildFluidFormationState(
    data.formationLayout || { formation, slot_positions: null },
    data.variantRows || []
  )
  const fluidBlock = buildHeroFluidPromptBlock({
    ...
    lang,
    evidence: {
      recurringIssues: data.patternsRow?.recurring_issues,
      gameAnalysis: data.gameAnalysisRow?.stats,
      attackAreas: (data.matches || []).map((m) => m.attack_areas).filter(Boolean).join(' ')
    }
  })
  ...
  if (placementWarningLines.length > 0) {
    starterLines.unshift(getPlacementWarningTitle(lang), ...placementWarningLines)
  } else {
  ...
  const matchLines = matches.slice(0, 10).map(m => {
    ...
    return `  ${d} vs ${sanitizeForPrompt(m.opponent_name, 40) || '?'} ${m.result || '-'} (${sanitizeForPrompt(m.formation_played, 20) || '-'}${vs})${votiStr}${zonesStr}`
  })
  ...
  const connMatch = coachRow?.connection ? matchConnectionToRoster(coachRow.connection, linkUpRoster.length ? linkUpRoster : allRoster, stylesLookup) : null
  ...
  let coachText = coachRow?.coach_name
    ...
    : L.noCoach
  if (collectCoachLinkUps(coachRow).length > 1) {
    const extraLinkUps = formatCoachLinkUpsForPrompt(coachRow, lang)
    if (extraLinkUps) coachText += ` ${extraLinkUps}`
  }
...
const LABELS_IT = {
  ...
  matchDerivedSection: text => `Dati dalle partite inserite (attacco tuo ≠ pressione avversaria ≠ gol subiti, voti, recupero): ${text}`,
  ...
  leveSection: (problems, recurring, formationUsage, matchesCount, lang) => {
    const leve = []
    if (problems.some(p => /difesa|defen|defens/i.test(String(p)))) leve.push('Difesa → leva: compattezza, Formazione fluida senza palla, Difensivo/Marcatura stretta/Marcatura uomo dove validi, rinforzo centrale o terzini.')
...
const LABELS_EN = {
  ...
  matchDerivedSection: text => `Data from entered matches (your attack ≠ opponent pressure ≠ conceded goals, ratings, recovery): ${text}`,
  rosterSection: (form, starters, reserves, totalReserves) =>
  ...
  leveSection: (problems, recurring, formationUsage, matchesCount, lang) => {
    const leve = []
    if (problems.some(p => /defen|defens/i.test(String(p)))) leve.push('Defence → lever: compactness, Fluid Formation without the ball, Defensive/Tight Marking/Man Marking where valid, reinforce centre or full-backs.')
...
/**
 * Minimal Sintesi / Sinergie / Leve subset for assistant-chat fallback (no full diagnostic).
 */
export function buildFallbackEnterpriseSubset({
  titolari = [],
  riserve = [],
  stylesLookup = {},
  formation = '',
  ...
  const L = (lang === 'en' || lang === 'es') ? LABELS_EN : LABELS_IT
  const parts = []
  parts.push(buildSintesiText(titolari, riserve, stylesLookup, formation, teamStyle, lang, L))
  const { advisable: coachAdvisable } = coachRow?.playing_style_competence
    ? coachAdvisableStyles(coachRow.playing_style_competence, lang)
    : { advisable: [] }
  const connMatch = coachRow?.connection
    ? matchConnectionToRoster(coachRow.connection, [...titolari, ...riserve], stylesLookup)
    : null
  let formVsUsage = null
  const formUsage = patternsRow?.formation_usage
  if (formation && formUsage && typeof formUsage === 'object') {
    const used = Object.entries(formUsage)
      .sort((a, b) => (b[1]?.matches || 0) - (a[1]?.matches || 0))
      .slice(0, 2)
      .map(([f]) => f)
    if (used.length && !used.some((f) => String(f).toLowerCase() === String(formation).toLowerCase())) {
      formVsUsage = { saved: formation, used }
    }
  }
  const synergies = L.synergiesSection(
    teamStyle,
    coachAdvisable,
    formVsUsage,
    connMatch,
  ...
  if (synergies) parts.push(synergies)
  const recurring = Array.isArray(patternsRow?.recurring_issues)
    ? patternsRow.recurring_issues.slice(0, 3).map((i) => i?.issue ?? i).filter(Boolean)
    : []
  const leve = L.leveSection(problems, recurring, formUsage, matchesCount, lang)
  if (leve) parts.push(leve)
