...
import { formatStarterPlacementToken, getPlacementWarningLines, getPlacementWarningTitle } from '@/lib/playerFieldPlacement'
import {
...
/** Competenze carta (allineato a buildPersonalContext in assistant-chat). */
function formatCompetenzeForPrompt(originalPositions, lang) {
  const empty = lang === 'en' ? 'not set' : lang === 'es' ? 'no configurado' : 'non impostate'
  if (!Array.isArray(originalPositions) || originalPositions.length === 0) return empty
  const s = originalPositions
    .map(p => (p?.position && p?.competence
      ? `${sanitizeForPrompt(p.position, 10)} ${sanitizeForPrompt(p.competence, 15)}`
      : null))
    .filter(Boolean)
    .join(', ')
  return s || empty
...
const AI_INFO_VALUE_LABELS = {
  ...
  es: {
    connection_quality: { good: 'buena', unstable: 'a veces inestable', lag: 'a menudo lag' },
    slow_opponent_connection_issues: { yes: 'sí', no: 'no', sometimes: 'a veces' },
    input_delay: { yes: 'sí', no: 'no', sometimes: 'a veces' },
    pass_level: { pa1: 'PA1', pa2: 'PA2', pa3: 'PA3' },
    smart_assist: { yes: 'sí', no: 'no' },
    platform: { console: 'Consola', pc: 'PC', mobile: 'Móvil', other: 'Otro' },
    ai_weak_point: { defence: 'defensa', attack: 'ataque', set_pieces: 'balón parado', transitions: 'transiciones', final_minutes: 'final del partido' }
  }
}
...
function buildGameAnalysisText(gameAnalysisRow, lang) {
  ...
  fmt(stats.goal_types, lang === 'en' ? 'Goal types' : lang === 'es' ? 'Tipos de gol' : 'Tipo gol')
  fmt(stats.shot_usage, lang === 'en' ? 'Shot usage' : lang === 'es' ? 'Uso de remate' : 'Tiro')
  fmt(stats.special_commands, lang === 'en' ? 'Special commands' : lang === 'es' ? 'Comandos especiales' : 'Comandi speciali')
  fmt(stats.passing, lang === 'en' ? 'Passing' : lang === 'es' ? 'Pase' : 'Passaggio')
  fmt(stats.dribbling, lang === 'en' ? 'Dribbling' : lang === 'es' ? 'Regate' : 'Dribbling')
  fmt(stats.defense, lang === 'en' ? 'Defense' : lang === 'es' ? 'Defensa' : 'Difesa')
...
function buildMatchDerivedSection(matches, patternsRow, lang) {
  ...
  if (matchesWithData.length > 0) {
    ...
    if (withAreas > 0) {
      ...
      if (Object.keys(sumAreas).length > 0 && areaMatchCount > 0) {
        ...
        if (pct) parts.push((lang === 'en' ? 'Attack zones (avg)' : lang === 'es' ? 'Zonas de ataque (media)' : 'Zone attacco (media)') + ': ' + pct)
      }
    }
    if (withRatings > 0) parts.push((lang === 'en' ? 'Player ratings present for' : lang === 'es' ? 'Calificaciones presentes en' : 'Voti giocatori presenti per') + ` ${withRatings} ${lang === 'en' ? 'matches' : lang === 'es' ? 'partidas' : 'partite'}`)
  ...
  if (patterns.attack_areas_avg && typeof patterns.attack_areas_avg === 'object' && Object.keys(patterns.attack_areas_avg).length > 0 && lines.every(l => !l.includes('Zone attacco') && !l.includes('Attack zones'))) {
    ...
    if (entries.length) lines.push((lang === 'en' ? 'Attack zones (pattern avg)' : lang === 'es' ? 'Zonas de ataque (media patrón)' : 'Zone attacco (media pattern)') + ': ' + entries.join(', '))
  }
  if (patterns.recovery_zones_avg && (Array.isArray(patterns.recovery_zones_avg) ? patterns.recovery_zones_avg.length > 0 : typeof patterns.recovery_zones_avg === 'object' && Object.keys(patterns.recovery_zones_avg).length > 0)) {
    ...
    lines.push((lang === 'en' ? 'Recovery zones (avg)' : lang === 'es' ? 'Zonas de recuperación (media)' : 'Zone recupero palla (media)') + ': ' + summary)
  }
  return lines.length ? lines.join('. ') : ''
}
...
function buildRecentLossesSection(matches, oppFormationsMap, lang) {
  ...
  const title = lang === 'en' ? 'Recent losses (for "why did I lose?" analysis):' : lang === 'es' ? 'Derrotas recientes (para análisis "¿por qué perdí?"):' : 'Ultime sconfitte (per analisi "perché ho perso?"):'
  return `${title}\n${lines.join('\n')}`
}
...
function buildCoachFeedbackSection(feedbackRows, lang) {
  ...
  if (isEn) {
    ...
    parts.push(`${count} sesiones de feedback en los últimos 30 días.`)
    if (weaknesses.length > 0) {
      parts.push(`EVITAR (el cliente reportó problemas): ${weaknesses.join('; ')}`)
    }
    if (strengths.length > 0) {
      parts.push(`REFORZAR (funcionó bien): ${strengths.join('; ')}`)
    }
    if (lessons.length > 0) {
      parts.push(`ADAPTAR (lecciones aprendidas): ${lessons.join('; ')}`)
    }
  } else {
...
function buildAiInfoLines(profile, lang) {
  ...
  add(lang === 'en' ? 'Name' : lang === 'es' ? 'Nombre' : 'Nome', profile.first_name)
  add(lang === 'en' ? 'Call AI' : lang === 'es' ? 'Llamar a la IA' : 'Chiamare l\'IA', profile.ai_name)
  add(lang === 'en' ? 'Division' : lang === 'es' ? 'División' : 'Divisione', profile.current_division)
  if (profile.hours_per_week != null && profile.hours_per_week !== '') add(lang === 'en' ? 'Hours/week' : lang === 'es' ? 'Horas/semana' : 'Ore/settimana', String(profile.hours_per_week))
  addSelect(lang === 'en' ? 'Connection' : lang === 'es' ? 'Conexión' : 'Connessione', 'connection_quality', profile.connection_quality)
  addSelect(lang === 'en' ? 'Slow opponent connection issues' : lang === 'es' ? 'Problemas con rivales lentos' : 'Difficoltà con avversari lenti', 'slow_opponent_connection_issues', profile.slow_opponent_connection_issues)
  addSelect(lang === 'en' ? 'Input delay' : lang === 'es' ? 'Retraso de entrada' : 'Ritardo input', 'input_delay', profile.input_delay)
  addSelect(lang === 'en' ? 'Pass level' : lang === 'es' ? 'Nivel de pases' : 'Livello passaggi', 'pass_level', profile.pass_level)
  addSelect(lang === 'en' ? 'Smart assist' : lang === 'es' ? 'Smart assist' : 'Smart assist', 'smart_assist', profile.smart_assist)
  addSelect(lang === 'en' ? 'Platform' : lang === 'es' ? 'Plataforma' : 'Piattaforma', 'platform', profile.platform)
  add(lang === 'en' ? 'Favourite player' : lang === 'es' ? 'Jugador favorito' : 'Giocatore preferito', profile.favourite_player_name)
  const weak = profile.ai_weak_point
  if (weak != null && String(weak).trim() !== '') {
    const v = String(weak).trim().toLowerCase()
    lines.push(`${lang === 'en' ? 'Weak point' : lang === 'es' ? 'Punto débil' : 'Punto debole'}: ${(labels.ai_weak_point && labels.ai_weak_point[v]) || sanitizeForPrompt(weak, 60)}`)
  }
  add(lang === 'en' ? 'Learn goals' : lang === 'es' ? 'Objetivos de aprendizaje' : 'Cosa vuole imparare', profile.ai_learn_goals)
  add(lang === 'en' ? 'Notes for AI' : lang === 'es' ? 'Notas para la IA' : 'Note per l\'IA', profile.ai_notes)
  return lines
}
...
ISTRUZIONI (obbligatorie): Usa SOLO i dati sotto. Manca dato → non inventare. §10.15: dati=INDICATORI, vietato "X perché Y". Statistiche/abilità FISSE: mai "migliorare/allenare". Rosa: solo nomi elencati. Stile squadra v6: solo 6 (Possesso, Contropiede veloce, Contrattacco, Passaggio lungo, Vie laterali, Pressing totale). Se manca competenza coach per Pressing totale, non inventarla. Riassunto generale: non analisi partita singola.
FORMAZIONE FLUIDA: se ATTIVA, riconoscila e valuta ATTACCO e DIFESA con i ruoli di fase (non player.position). Se NON ATTIVA, suggeriscila solo se i dati mostrano un bisogno diverso tra le due fasi, e motiva. Non cambiare formation_layout né player.position.
...
INSTRUCTIONS (required): Use ONLY the data below. Missing data → don't invent. §10.15: data=INDICATORS, never "X because Y". Stats/skills FIXED: never "improve/train". Roster: only names listed. Team style v6: only 6 (Possession, Quick Counter, Long Ball Counter, Long Ball, Out Wide, Overload). If Overload coach competence is missing, do not invent it. General summary: not single match analysis.
FLUID FORMATION: if ACTIVE, recognise it and evaluate ATTACK and DEFENCE with phase roles (not player.position). If OFF, suggest it only when data show a different need between the two phases, and motivate it. Do not change formation_layout or player.position.
...
const INSTRUCTIONS_BLOCK_ES = `---
INSTRUCCIONES (obligatorias): Usa SOLO los datos abajo. Falta dato → no inventes. §10.15: datos=INDICADORES, prohibido "X porque Y". Estadísticas/habilidades FIJAS: nunca "mejorar/entrenar". Plantilla: solo nombres listados. Estilo de equipo v6: solo 6 (Posesión, Contragolpe, Contraataque, Pase largo, Vías laterales, Pressing totale (Overload)). Si falta la competencia del entrenador para Pressing totale/Overload, no la inventes. Resumen general: no análisis de partido individual.
FORMACIÓN FLUIDA: si está ACTIVA, reconócela y evalúa ATAQUE y DEFENSA con los roles de fase (no player.position). Si NO está activa, sugiérela solo cuando los datos muestren una necesidad distinta entre las dos fases, y motívalo. No cambies formation_layout ni player.position.
EXPERIENCIA COACH (si está presente): Estos son feedbacks REALES del cliente sobre tus consejos anteriores. DEBES tenerlos en cuenta: NO repitas errores señalados (Debilidad), REFUERZA lo que funcionó (Fortaleza), ADAPTA los consejos según las lecciones aprendidas (Lección). Si el cliente señaló un problema con un jugador/formación, propón alternativas concretas de la plantilla.
---`

export function buildDiagnostic(lang, data) {
  const L = lang === 'en' ? LABELS_EN : lang === 'es' ? LABELS_ES : LABELS_IT
  const sections = []

  // 0. Blocco ISTRUZIONI PER L'IA (sempre per primo; guida ragionamento chat)
  sections.push(lang === 'en' ? INSTRUCTIONS_BLOCK_EN : lang === 'es' ? INSTRUCTIONS_BLOCK_ES : INSTRUCTIONS_BLOCK_IT)

  // 1. Profilo (sanitizza campi da DB)
  const profile = data.profile || {}
  const firstName = sanitizeForPrompt(profile.first_name, 80) || (lang === 'en' ? 'User' : lang === 'es' ? 'Usuario' : 'Utente')
  const teamName = sanitizeForPrompt(profile.team_name, 80) || (lang === 'en' ? 'your team' : lang === 'es' ? 'tu equipo' : 'il tuo team')
  ...
  // 2. Rosa (sintesi)
  const formation = data.formation || (lang === 'en' ? 'not set' : lang === 'es' ? 'no configurada' : 'non impostata')
  ...
  const fluid = data.fluid || buildFluidFormationState(
    data.formationLayout || { formation: data.formation, slot_positions: data.slot_positions || null },
    data.variantRows || []
  )
  const patternsEarly = data.patternsRow || {}
  const fluidBlock = buildHeroFluidPromptBlock({
    fluid,
    starters: titolari,
    lang,
    evidence: {
      recurringIssues: patternsEarly.recurring_issues,
      problems: Array.isArray(profile.common_problems) ? profile.common_problems : [],
      gameAnalysis: data.gameAnalysisRow?.stats,
      feedbackRows: data.feedbackRows,
      attackAreas: Array.isArray(data.matches) ? data.matches.map((m) => m.attack_areas).filter(Boolean).join(' ') : ''
    }
  })
  sections.push(fluidBlock)
  ...
  const skillsSnippet = (p) => {
    const ctx = sanitizeForPrompt(formatPlayerSkillContext(p, lang), 220)
    return ctx ? ` | ${ctx}` : ''
  }
  const starterLines = titolari.map(p => {
    const styleName = sanitizeForPrompt(getPlayerStyleDisplayName(p, stylesLookup), 30) || '-'
    const forma = formLabel(p.form)
    const stats = statsSnippet(getPlayerDisplayStats(p))
    const skills = skillsSnippet(p)
    const comp = formatCompetenzeForPrompt(p.original_positions, lang)
    const buildSnip = sanitizeForPrompt(formatBuildCoachSnippet(p, lang), 180)
    return `  ${sanitizeForPrompt(p.player_name, 50) || '?'} (${sanitizeForPrompt(formatStarterPlacementToken(p, fluid, lang), 60) || '?'}, ${styleName}, ${p.overall_rating ?? '-'}${forma}${stats}${skills}, competenze: ${comp}${buildSnip})`
  })
  const placementWarningLines = getPlacementWarningLines(titolari, lang, fluid)
  if (placementWarningLines.length > 0) {
    starterLines.unshift(getPlacementWarningTitle(lang), ...placementWarningLines.map((line) => `  ${line}`))
  }
  const reserveLines = riserve.slice(0, 12).map(p => {
    ...
    const comp = formatCompetenzeForPrompt(p.original_positions, lang)
    const buildSnip = sanitizeForPrompt(formatBuildCoachSnippet(p, lang), 180)
    return `  ${sanitizeForPrompt(p.player_name, 50) || '?'} (${sanitizeForPrompt(p.position, 20) || '?'}, ${styleName}, ${p.overall_rating ?? '-'}${forma}${stats}${skills}, competenze: ${comp}${buildSnip})`
  })
  sections.push(L.rosterSection(formation, starterLines, reserveLines, riserve.length))

  const buildProgressionText = formatBuildProgressionSection(roster, lang)
  if (buildProgressionText) sections.push(buildProgressionText)
  ...
  // I Link-up si attivano con i giocatori nel Game Plan iniziale, non con le riserve.
  const linkUpRoster = startersForLinkUpVerification(titolari, fluid)
  const connMatch = coachRow?.connection ? matchConnectionToRoster(coachRow.connection, linkUpRoster, stylesLookup) : null
  ...
  const linkUpBlock = formatCoachLinkUpsForHeroPrompt({
    coach: coachRow,
    starters: linkUpRoster,
    stylesLookup,
    lang
  })
  if (linkUpBlock) coachText = `${coachText}\n${linkUpBlock}`
  ...
  const skillAdvisory = buildRosterSkillAdvisorySection(roster, data.gameAnalysisRow, lang)
  if (skillAdvisory) sections.push(skillAdvisory)
...
function styleNameForPlayer(p, stylesLookup) {
  return sanitizeForPrompt(getPlayerStyleDisplayName(p, stylesLookup), 30) || ''
}
...
const LABELS_IT = {
  ...
  rosterSection: (form, starters, reserves, totalReserves) =>
    `Rosa (per ogni giocatore: position, stile giocatore card, rating, forma, fin/pas/tac, abilità, competenze=posizioni ideali dalla card es. CC Alta; se position ≠ competenze segnala il mismatch). Formazione: ${form}.\nTitolari:\n${starters.join('\n')}\nRiserve (${totalReserves}):\n${reserves.join('\n')}${totalReserves > 12 ? `\n... altri ${totalReserves - 12}` : ''}`,
  ...
  buildSection: (numStarters, numReserves, form, style) =>
    `Sintesi rosa: ${numStarters} titolari, ${numReserves} riserve. Formazione: ${form}. Stile squadra: ${style}.`,
  buildSectionRich: (numStarters, numReserves, form, style, uniqueStyles, defStyles, midStyles, atkStyles, lang) =>
    `Sintesi rosa (non progressione PT): ${numStarters} titolari, ${numReserves} riserve. Formazione: ${form}, stile squadra: ${style}. Stili in rosa: ${uniqueStyles}. Difesa: ${defStyles}. Centrocampo: ${midStyles}. Attacco: ${atkStyles}. Usa per fit movimenti, modulo e Collegamento.`,
  matches: 'partite',
  ...
  synergiesSection: (teamStyle, coachAdvisable, formVsUsage, connMatch, individualInstructions, competence, lang) => {
    ...
    if (connMatch && (!connMatch.focal.length || !connMatch.keyMan.length)) {
      parts.push('Collegamento allenatore non pienamente attivabile: manca Punto focale o Uomo chiave tra i titolari.')
    }
  ...
  leveSection: (problems, recurring, formationUsage, matchesCount, lang) => {
    const leve = []
    if (problems.some(p => /difesa|defen|defens/i.test(String(p)))) leve.push('Difesa → leva: compattezza, marcatura/copertura, rinforzo centrale o terzini. Non proporre Linea bassa: è legacy v6.')
...
const LABELS_EN = {
  ...
  rosterSection: (form, starters, reserves, totalReserves) =>
    `Roster (per player: position, card style, rating, form, fin/pas/tac, skills, competenze=ideal positions from card e.g. CM High; if position ≠ competenze flag the mismatch). Formation: ${form}.\nStarters:\n${starters.join('\n')}\nReserves (${totalReserves}):\n${reserves.join('\n')}${totalReserves > 12 ? `\n... ${totalReserves - 12} more` : ''}`,
  ...
  buildSection: (numStarters, numReserves, form, style) =>
    `Roster summary: ${numStarters} starters, ${numReserves} reserves. Formation: ${form}. Team style: ${style}.`,
  buildSectionRich: (numStarters, numReserves, form, style, uniqueStyles, defStyles, midStyles, atkStyles, lang) =>
    `Roster summary (not PT progression): ${numStarters} starters, ${numReserves} reserves. Formation: ${form}, team style: ${style}. Styles in roster: ${uniqueStyles}. Defence: ${defStyles}. Midfield: ${midStyles}. Attack: ${atkStyles}. Use for movement fit, formation and Link-up.`,
  matches: 'matches',
  ...
  synergiesSection: (teamStyle, coachAdvisable, formVsUsage, connMatch, individualInstructions, competence, lang) => {
    ...
    if (connMatch && (!connMatch.focal.length || !connMatch.keyMan.length)) {
      parts.push('Coach Link-up not fully activatable: missing Focal Point or Key Man among the starters.')
    }
    const instr = individualInstructions && typeof individualInstructions === 'object' ? individualInstructions : {}
    const hasContropiedeOnDef = Object.entries(instr).some(([slot, o]) => {
      const s = (slot || '').toLowerCase()
  ...
  leveSection: (problems, recurring, formationUsage, matchesCount, lang) => {
    const leve = []
    if (problems.some(p => /defen|defens/i.test(String(p)))) leve.push('Defence → lever: compactness, marking/cover, reinforce centre or full-backs. Do not recommend Deep Line: it is legacy in v6.')
...
const LABELS_ES = {
  profileSection: (name, team, problems) =>
    `Perfil: ${name}, equipo ${team}.${problems.length ? ` Problemas declarados: ${problems.join(', ')}.` : ''}`,
  aiInfoSection: lines => `Información para la IA: ${lines.join('. ')}`,
  coachFeedbackSection: text => `Experiencia Coach (feedback Gimnasio Coach): ${text}`,
  gameAnalysisSection: text => `Estadísticas de juego (Análisis eFootball, últimas 10 partidas): ${text}`,
  matchDerivedSection: text => `Datos de partidos ingresados (zonas de ataque, calificaciones, recuperación): ${text}`,
  rosterSection: (form, starters, reserves, totalReserves) =>
    `Plantilla (por jugador: posición, estilo de carta, calificación, forma, fin/pas/tac, habilidades, competencias=posiciones ideales de la carta ej. MC Alto; si posición ≠ competencias señalar el desajuste). Formación: ${form}.\nTitulares:\n${starters.join('\n')}\nSuplentes (${totalReserves}):\n${reserves.join('\n')}${totalReserves > 12 ? `\n... otros ${totalReserves - 12}` : ''}`,
  tacticsSection: (style, num) => `Táctica: estilo de equipo ${style}. Instrucciones individuales: ${num} activas.`,
  andamentoSection: (matchLines, patternText, noMatches) =>
    noMatches ? 'Forma: sin partidas cargadas.' : `Forma (últimas partidas):\n${matchLines.join('\n')}${patternText ? `\nPatrón: ${patternText}` : ''}`,
  difficoltaSection: (declared, recurring) =>
    `Dificultades:${declared.length ? ` declaradas: ${declared.join(', ')}.` : ''}${recurring.length ? ` Recurrentes: ${recurring.join(', ')}.` : ' Sin problemas recurrentes en BD.'}`,
  noCoach: 'Entrenador: sin entrenador activo configurado.',
  coachSection: (name, competence, connection, statBoosters) => {
    let t = `Entrenador: ${name}. Competencias de estilos: `
    if (competence && typeof competence === 'object') {
      const entries = Object.entries(competence)
        .map(([k, v]) => ({ k, v: parseInt(v, 10) || 0 }))
        .filter(({ v }) => !Number.isNaN(v))
        ...
        .slice(0, 5)
      t += entries.map(({ k, v }) => `${k}=${v}`).join(', ') + '.'
    } else t += 'no configuradas.'
    if (statBoosters && (Array.isArray(statBoosters) ? statBoosters.length : Object.keys(statBoosters || {}).length)) t += ' Stat boosters activos.'
    return t
  },
  coachSectionFull: (name, competence, connection, advisable, notAdvisable, connMatch, boosterLines, lang) => {
    let t = `Entrenador: ${name}. `
    if (competence && typeof competence === 'object') {
      const entries = Object.entries(competence)
        .map(([k, v]) => ({ k, v: parseInt(v, 10) || 0 }))
        .filter(({ v }) => !Number.isNaN(v))
        .sort((a, b) => b.v - a.v)
        .slice(0, 5)
      t += `Competencias: ${entries.map(({ k, v }) => `${k}=${v}`).join(', ')}. `
      if (advisable?.length) t += `Aconsejables (≥70): ${advisable.join(', ')}. `
      if (notAdvisable?.length) t += `No aconsejables (<70): ${notAdvisable.slice(0, 3).join(', ')}. `
    }
    if (boosterLines?.length) t += `Beneficiarios de stat boosters: ${boosterLines.join('; ')}. `
    return t.trim()
  },
  buildSection: (numStarters, numReserves, form, style) =>
    `Resumen de plantilla: ${numStarters} titulares, ${numReserves} suplentes. Formación: ${form}. Estilo de equipo: ${style}.`,
  buildSectionRich: (numStarters, numReserves, form, style, uniqueStyles, defStyles, midStyles, atkStyles, lang) =>
    `Resumen de plantilla (no progresión PT): ${numStarters} titulares, ${numReserves} suplentes. Formación: ${form}, estilo de equipo: ${style}. Estilos en plantilla: ${uniqueStyles}. Defensa: ${defStyles}. Centro del campo: ${midStyles}. Ataque: ${atkStyles}. Usar para ajuste de movimientos, módulo y Link-up.`,
  matches: 'partidas',
  skillsSection: arr => `Habilidades en plantilla: ${arr.slice(0, 12).join(', ')}.`,
  synergiesSection: (teamStyle, coachAdvisable, formVsUsage, connMatch, individualInstructions, competence, lang) => {
    const parts = []
    if (teamStyle === 'non impostato' || teamStyle === 'not set' || teamStyle === 'no configurado') {
      if (coachAdvisable?.length) parts.push(`Estilo de equipo no configurado → aconsejable configurar uno de: ${coachAdvisable.join(', ')} (coach fuerte en estos).`)
    } else if (coachAdvisable?.length && !coachAdvisable.some(s => teamStyle && String(teamStyle).toLowerCase().includes(s.toLowerCase()))) {
      const norm = String(teamStyle).toLowerCase().replace(/_/g, ' ')
      const match = coachAdvisable.find(s => norm.includes(s.toLowerCase()))
      if (!match) parts.push(`Estilo de equipo (${teamStyle}): verificar competencia del coach; aconsejables con este coach: ${coachAdvisable.join(', ')}.`)
    }
    if (formVsUsage) parts.push(`Desajuste: formación guardada ${formVsUsage.saved} vs módulos más usados en partido: ${formVsUsage.used.join(', ')}. Considerar módulos con mejor ratio de victorias.`)
    if (connMatch && (!connMatch.focal.length || !connMatch.keyMan.length)) {
      parts.push('Link-up del entrenador no completamente activable: falta Punto focal o Hombre clave entre los titulares.')
    }
    const instr = individualInstructions && typeof individualInstructions === 'object' ? individualInstructions : {}
    const hasContropiedeOnDef = Object.entries(instr).some(([slot, o]) => {
      const s = (slot || '').toLowerCase()
    ...
    if (hasContropiedeOnDef && competence) {
      const contropiedeVal = competence.contropiede_veloce ?? competence.contropiede
      const v = parseInt(contropiedeVal, 10)
      if (!Number.isNaN(v) && v < 70) parts.push('Nota: Contragolpe activo en defensores pero competencia coach Contragolpe <70 — para contragolpe sistemático preferir Contraataque/Pase largo o entrenador con Contragolpe ≥70.')
    }
    return parts.length ? `Sinergias y notas:\n${parts.map(p => `- ${p}`).join('\n')}` : ''
  },
  leveSection: (problems, recurring, formationUsage, matchesCount, lang) => {
    const leve = []
    if (problems.some(p => /defen|defens/i.test(String(p)))) leve.push('Defensa → palanca: compacidad, instrucciones (línea baja, marcaje, cobertura), refuerzo central o laterales.')
    if (problems.some(p => /attack|goal/i.test(String(p)))) leve.push('Ataque → palanca: estilo de equipo adecuado para verticalidad, delanteros de área, instrucciones en atacantes (anclaje/contragolpe donde permitido).')
    if (problems.some(p => /formation|modul/i.test(String(p)))) leve.push('Formación → palanca: probar módulos con mejor ratio de victorias de los datos de partidos; alinear formación guardada al uso real.')
    if (recurring.length) leve.push(`Problemas recurrentes (${recurring.slice(0, 2).join(', ')}) → anclar una palanca por vez: corregir FIT, instrucciones §5, o cambio de módulo/estilo.`)
    if (matchesCount > 0 && formationUsage && typeof formationUsage === 'object') {
      const top = Object.entries(formationUsage).sort((a, b) => (b[1]?.win_rate || 0) - (a[1]?.win_rate || 0)).slice(0, 1)
      if (top.length && top[0][1]?.win_rate >= 0.6) leve.push(`Módulo con buen ratio de victorias: ${top[0][0]} (${Math.round((top[0][1].win_rate || 0) * 100)}%). Considerar para próximas partidas.`)
    }
    return leve.length ? `Palancas posibles (síntoma → palanca):\n${leve.map(l => `- ${l}`).join('\n')}` : ''
  },
}
