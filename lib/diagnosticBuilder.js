import { getPlayerStyleDisplayName, playingStylesMatch } from '@/lib/playingStyleResolve'
...
function matchConnectionToRoster(connection, roster, stylesLookup) {
  ...
  const styleMatch = (p, styleName, pos) => {
    const pStyle = getPlayerStyleDisplayName(p, stylesLookup)
    const styleOk = !styleName || playingStylesMatch(pStyle, styleName)
...
export function buildDiagnostic(lang, data) {
  ...
  const competenzeSnippet = (p) => {
    const codes = getCardSlotCodesFromOriginalPositions(p.original_positions)
    if (codes.length === 0) return ''
    return ` | slot carta: ${codes.join('/')}`
  }
  const starterLines = titolari.map(p => {
    ...
    const cardSlots = competenzeSnippet(p)
    return `  ${sanitizeForPrompt(p.player_name, 50) || '?'} (${sanitizeForPrompt(p.position, 20) || '?'}, ${styleName}, ${p.overall_rating ?? '-'}${forma}${stats}${skills}${cardSlots})`
  })
