...
function getOutOfPositionStarterLines(titolari, lang) {
  const lines = []
  for (const p of titolari || []) {
    ...
    const comp = formatCompetenzeForPrompt(originals, lang)
    const slot = p.slot_index != null ? ` slot ${p.slot_index}` : ''
    lines.push(`  ${sanitizeForPrompt(p.player_name, 50) || '?'}${slot}: in campo ${sanitizeForPrompt(current, 10)}; competenze card ${comp}`)
  }
  return lines
}
...
export function buildDiagnostic(lang, data) {
  ...
  if (outOfPositionLines.length > 0) {
    ...
    starterLines.unshift(title, ...outOfPositionLines)
  }
  ...
  const linkUpRoster = startersForLinkUpVerification(allRoster, fluid)
...
const LABELS_IT = {
  ...
  buildSectionRich: (numStarters, numReserves, form, style, uniqueStyles, defStyles, midStyles, atkStyles, lang) =>
    `Sintesi rosa (non progressione PT): ${numStarters} titolari, ${numReserves} riserve. Formazione: ${form}, stile squadra: ${style}. Stili in rosa: ${uniqueStyles}. Difesa: ${defStyles}. Centrocampo: ${midStyles}. Attacco: ${atkStyles}. Usa per fit movimenti, modulo e connection.`,
  matches: 'partite',
...
const LABELS_EN = {
  ...
  buildSectionRich: (numStarters, numReserves, form, style, uniqueStyles, defStyles, midStyles, atkStyles, lang) =>
    `Roster summary (not PT progression): ${numStarters} starters, ${numReserves} reserves. Formation: ${form}, team style: ${style}. Styles in roster: ${uniqueStyles}. Defence: ${defStyles}. Midfield: ${midStyles}. Attack: ${atkStyles}. Use for movement fit, formation and connection.`,
  matches: 'matches',
...
const LABELS_ES = {
  ...
  coachSection: (name, competence, connection, statBoosters) => {
    ...
    if (connection) t += ` Conexión: ${typeof connection === 'string' ? connection : (connection.name || '')}.`
  ...
  coachSectionFull: (name, competence, connection, advisable, notAdvisable, connMatch, boosterLines, lang) => {
    ...
    if (connection?.name) {
      t += `Conexión: ${connection.name}. `
      if (connMatch) {
        const focalNames = connMatch.focal.map(p => sanitizeForPrompt(p.player_name, 20)).slice(0, 3)
        const keyNames = connMatch.keyMan.map(p => sanitizeForPrompt(p.player_name, 20)).slice(0, 3)
        if (focalNames.length) t += `Focal Point compatibles: ${focalNames.join(', ')}. `
        else if (connMatch.focalReq) t += `Focal Point requerido (${connMatch.focalReq}): sin coincidencia en plantilla. `
        if (keyNames.length) t += `Key Man compatibles: ${keyNames.join(', ')}. `
        else if (connMatch.keyManReq) t += `Key Man requerido (${connMatch.keyManReq}): sin coincidencia en plantilla. `
      }
    }
  ...
  buildSectionRich: (numStarters, numReserves, form, style, uniqueStyles, defStyles, midStyles, atkStyles, lang) =>
    `Resumen de plantilla (no progresión PT): ${numStarters} titulares, ${numReserves} suplentes. Formación: ${form}, estilo de equipo: ${style}. Estilos en plantilla: ${uniqueStyles}. Defensa: ${defStyles}. Centro del campo: ${midStyles}. Ataque: ${atkStyles}. Usar para ajuste de movimientos, módulo y conexión.`,
  matches: 'partidas',
  ...
  synergiesSection: (teamStyle, coachAdvisable, formVsUsage, connMatch, individualInstructions, competence, lang) => {
    ...
    if (connMatch && (!connMatch.focal.length || !connMatch.keyMan.length)) {
      parts.push('Conexión del entrenador no completamente activable: falta Focal Point o Key Man en plantilla. Para activarla se necesitan jugadores con estilo y posición requeridos.')
    }
    const instr = individualInstructions && typeof individualInstructions === 'object' ? individualInstructions : {}
    const hasContropiedeOnDef = Object.entries(instr).some(([slot, o]) => {
      const s = (slot || '').toLowerCase()
