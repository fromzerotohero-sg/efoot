...
function buildCountermeasureLines(cm, summary, lang) {
  const attack = []
  const defense = []
  const add = (target, row) => {
    const title = countermeasureText(row?.suggestion || row?.action || row, lang)
    if (!title || target.some((item) => item.title.toLowerCase() === title.toLowerCase())) return
    target.push({
      title,
      reason: countermeasureText(row?.reason, lang) || null
    })
  }
...
export function buildCustomerPrematchPlan(rawOutput, opts = {}) {
  ...
  const countermeasureLines = buildCountermeasureLines(cm, summary, lang)
