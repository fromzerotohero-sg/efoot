...
/** Cap minimo tale che i PT spesi da una build salvata rientrano nel budget (come eFHUB). */
export function inferLevelCapFromPointsUsed(pointsUsed) {
  const used = Math.ceil(Number(pointsUsed))
  if (!Number.isFinite(used) || used <= 0) return null
  for (let cap = 2; cap <= 99; cap++) {
    if (availableProgressionPoints(cap) >= used) return cap
  }
  return null
}

/**
 * Riduce le tacche finché i PT spesi rientrano nel budget (stessa tariffa eFHUB/Konami).
 */
export function clampSlidersToProgressionBudget(sliders = {}, levelCap) {
  const budget = availableProgressionPoints(levelCap)
  let out = { ...getDefaultSliderValues(), ...sliders }
  for (const key of PROGRESSION_KEYS) {
    out[key] = Math.max(0, Math.min(MAX_TACCE_PER_MACRO, Math.floor(Number(out[key] || 0))))
  }
  if (!budget) return out

  while (totalPointsUsed(out) > budget) {
    const candidates = PROGRESSION_KEYS
      .filter((key) => out[key] > 0)
      .map((key) => {
        const trial = { ...out, [key]: out[key] - 1 }
        return { key, nextTotal: totalPointsUsed(trial) }
      })
    if (!candidates.length) break

    const withinBudget = candidates
      .filter((entry) => entry.nextTotal <= budget)
      .sort((a, b) => b.nextTotal - a.nextTotal)
    const pick = withinBudget[0] || candidates.sort((a, b) => a.nextTotal - b.nextTotal)[0]
    out[pick.key] -= 1
  }
  return out
}
...
function applyStatDelta(stats = {}, delta = {}) {
  ...
  for (const [stat, amount] of Object.entries(delta || {})) {
    output[stat] = Math.min(99, Math.max(1, (Number(output[stat]) || 40) + (Number(amount) || 0)))
  }
  return output
}

export function applyBoosters(stats = {}, boosters = []) {
  let output = { ...normalizeStatsToEfhub(stats) }
  for (const booster of Array.isArray(boosters) ? boosters : []) {
    output = applyStatDelta(output, resolveBoosterStats(booster))
  }
  return output
}
...
export function applyManagerSkill(stats = {}, skillValue = 70) {
  ...
  for (const stat of MANAGER_SKILL_AFFECTED_STATS) {
    output[stat] = Math.min(99, output[stat] + Math.floor(output[stat] * (multiplier - 1)))
  }
  return output
}
...
export function applyCoachEffects(stats = {}, coach = null, teamStyle = null) {
  let output = applyManagerSkill(stats, resolveCoachSkillValue(coach, teamStyle))
  const statBoosters = Array.isArray(coach?.stat_boosters) ? coach.stat_boosters : []
  output = applyBoosters(output, statBoosters)
  return output
}
