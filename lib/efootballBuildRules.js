...
import { EFHUB_BOOSTER_STATS_BY_BASE_LEVEL } from './efhubBoosterStats.js'
...
function normalizeBoosterBaseKey(value) {
  return normalizeRuleKey(value).replace(/\s+\d+$/u, '').trim()
}
...
const EFHUB_BOOSTER_BASE_ALIASES = {
  tiro: 'shooting',
  'calci di punizione': 'free kick taking',
  'gioco aereo': 'aerial',
  passaggio: 'passing',
  'gestione del pallone': 'ball carrying',
  tecnica: 'technique',
  difesa: 'defending',
  duelli: 'duelling',
  agilita: 'agility',
  fisicita: 'physicality',
  portiere: 'goalkeeping',
  'istinto da attaccante': 'striker s instinct',
  'pilastro difensivo': 'defending',
  tuttocampo: 'hard worker',
  saracinesca: 'shutdown',
  crossatore: 'crossing',
  regista: 'regista',
  fondamenta: 'rebuilding',
  precisione: 'accuracy',
  'motore offensivo': 'offence creator',
  'proteggi il possesso': 'ball protection',
  equilibrio: 'balancer',
  contropiedista: 'counter',
  'blocco aereo': 'aerial block',
  spaccapartita: 'game changer',
  forza: 'strength',
  'movimento senza palla': 'off the ball',
  rubapalla: 'stealing'
}

function resolveEfhubBoosterStats(label, amount) {
  const efhubKey = EFHUB_BOOSTER_BASE_ALIASES[label] || label
  return EFHUB_BOOSTER_STATS_BY_BASE_LEVEL[efhubKey]?.[amount] || null
}
...
const EXACT_BOOSTER_STATS_BY_LABEL = {
  // Some eFootball player boosters are not flat four-stat categories.
  // Keep these keyed by booster name + level so the resolver works for every player.
  rubapalla: {
    2: {
      finishing: 1,
...
export function resolveBoosterStats(booster) {
  ...
  const boosterName = booster.name || booster.booster_name || booster.stat_name || booster.statName
  const label = normalizeBoosterBaseKey(boosterName)
  const rawEffect = booster.effect || booster.bonus || booster.value || booster.level || booster.amount
  const amount = parseBoostAmount(rawEffect ?? boosterName, Number(booster.bonus) || Number(booster.level) || 1)
  const exactStats = EXACT_BOOSTER_STATS_BY_LABEL[label]?.[amount]
  if (exactStats) return exactStats
  const efhubStats = resolveEfhubBoosterStats(label, amount)
  if (efhubStats) return efhubStats
...
export function applyCoachEffects(stats = {}, coach = null, teamStyle = null, statCaps = {}) {
  ...
  const statBoosters = Array.isArray(coach?.stat_boosters) ? coach.stat_boosters : []
