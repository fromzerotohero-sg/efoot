...
function normalizeExplicitStatDelta(stats = {}) {
  if (!stats || typeof stats !== 'object' || Array.isArray(stats)) return {}
  const normalized = {}
  for (const [key, value] of Object.entries(stats)) {
    const amount = Number(value)
    if (!Number.isFinite(amount) || amount === 0) continue
    const stat = SINGLE_STAT_BY_LABEL[normalizeRuleKey(key)] || key
    normalized[stat] = (normalized[stat] || 0) + amount
  }
  return normalized
}
...
export function resolveBoosterStats(booster) {
  ...
  const explicitStats = normalizeExplicitStatDelta(booster.stats)
