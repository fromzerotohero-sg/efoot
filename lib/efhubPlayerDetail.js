...
import { normalizePlayingStylesContract } from './playingStyleResolve.js'
...
export async function fetchEfhubCardDetail(card) {
  ...
  try {
    ...
    // v6 dual: EFHub espone playingStyleDefensive sulle carte con stile difensivo
    // documentato; i riferimenti flight ("$undefined", "$f:...") contano come assenti.
    const rawDefensive = player?.playingStyleDefensive || extractString(markup, 'playingStyleDefensive')
    const defensiveStyle = rawDefensive && !String(rawDefensive).startsWith('$') ? String(rawDefensive).trim() : null
    const attackStyle = player?.playingStyle || playingStyle || ''
    const playingStyles = normalizePlayingStylesContract({ attack: attackStyle, defense: defensiveStyle })
