...
function getReasonSectionTitle(id, lang) {
  const titles = {
    context: { it: 'Cosa abbiamo incrociato', en: 'What we cross-checked' },
    allocation: { it: 'Perché questi PT', en: 'Why these points' },
...
function buildMetaReasonSections({ lang, sliders, pointsUsed, card, enrichedCatalog, engineReasons }) {
  ...
  const skillsClause = formatSkillsClause(card, enrichedCatalog, lang)
  const contextItems = []

  if (role) {
    contextItems.push(
      lang === 'en'
        ? `Community reference for ${role} — no formation, coach, or your match stats.`
        : `Riferimento community per ${role}: niente modulo, coach né statistiche della tua rosa.`
    )
  }
  if (skillsClause) {
    contextItems.push(
      lang === 'en'
        ? `Skills on the card (${skillsClause}) still steer which progression areas the meta profile boosts.`
        : `Le abilità già in scheda (${skillsClause}) indicano quali macro la meta tende a potenziare su questo profilo.`
    )
  }
  if (playingStyle && playingStyle !== 'Profilo da analizzare') {
    contextItems.push(
      lang === 'en'
        ? `Playing style ${playingStyle}: generic pack follows how this profile is most often built online.`
        : `Stile carta ${playingStyle}: il pack generico replica come questo profilo viene giocato più spesso online.`
    )
  }
  ...
  if (topLine.length > 0) {
    allocItems.push(
      lang === 'en'
        ? `${pointsUsed} PT on ${topLine.join(', ')} — typical spread for role and style, not your XI.`
        : `${pointsUsed} PT su ${topLine.join(', ')}: distribuzione tipica per ruolo e stile, non calibrata sulla tua rosa.`
    )
  }
  for (const reason of engineReasons.slice(0, 2)) {
...
function buildRosterReasonSections({
  ...
  if (skillsClause) {
    contextItems.push(
      lang === 'en'
        ? `Given the skills on this card (${skillsClause}), we put points where those strengths show up in your matches — not on filler stats.`
        : `Viste le abilità in scheda (${skillsClause}), i PT vanno sulle macro che le valorizzano nel tuo modo di giocare, non su numeri “di comodo”.`
    )
  } else if (role) {
    contextItems.push(
      lang === 'en'
        ? `Profile for ${role}: progression follows how you use this slot in your XI, not a generic template.`
        : `Profilo ${role}: la progressione segue come usi questo slot in formazione, non un template generico.`
    )
  }
  ...
  if (playingStyle && playingStyle !== 'Profilo da analizzare') {
    contextItems.push(
      lang === 'en'
        ? `Card playing style ${playingStyle}: the build matches how this profile moves and creates danger with your settings.`
        : `Stile carta ${playingStyle}: la build segue come si muove e dove crea superiorità con le tue impostazioni.`
    )
  }

  const tacticalBits = []
  if (formation) tacticalBits.push(lang === 'en' ? `formation ${formation}` : `modulo ${formation}`)
  if (teamStyle) tacticalBits.push(lang === 'en' ? `team style ${teamStyle}` : `stile squadra ${teamStyle}`)
  if (coachName) tacticalBits.push(coachName)
  if (tacticalBits.length) {
    contextItems.push(
      lang === 'en'
        ? `With your ${tacticalBits.join(' · ')}, points target where this role actually decides phases in your system.`
        : `Con ${tacticalBits.join(' · ')} su rosa, puntiamo dove questo ruolo pesa davvero nelle fasi di gioco.`
    )
  }

  const slot = targetPosition || role
  if (slot) {
    const sameAsRole = role && String(slot).trim().toUpperCase() === String(role).trim().toUpperCase()
    contextItems.push(
      sameAsRole
        ? lang === 'en'
          ? `Slot ${slot} in your XI: PT follow the match impact you want from that position.`
          : `Slot ${slot} nella tua formazione: i PT seguono l’impatto che vuoi da quella posizione.`
        : lang === 'en'
          ? `You field it as ${slot}: allocation is tuned to that real slot, not only the printed role.`
          : `Lo schieri come ${slot}: la distribuzione è calibrata su quello slot, non solo sul ruolo in carta.`
    )
  }
  ...
  if (contextItems.length) {
    sections.push({
      id: 'context',
      title: getReasonSectionTitle('context', lang),
      items: contextItems.slice(0, 4)
    })
  }

  const allocItems = []
  const topSliders = getTopSliders(rosterSliders, 4)
  if (topSliders.length) {
    const topDetail = topSliders
      .map(({ key, val }) => `${getBuildSliderLabel(key, lang)} (${val} PT)`)
      .join(', ')
    allocItems.push(
      lang === 'en'
        ? `We invested ${pointsUsed} PT on ${topDetail} — consistent with the skills and tactical context above.`
        : `Abbiamo investito ${pointsUsed} PT su ${topDetail}: scelta coerente con abilità e contesto tattico che hai letto sopra.`
    )
  }
  for (const reason of engineReasons.slice(0, 2)) {
    if (reason) allocItems.push(reason)
  ...
  if (metaSliders) {
    ...
    if (deltas.length) {
      const vsMetaItems = deltas.map(({ key, delta }) => {
        const label = getBuildSliderLabel(key, lang)
        if (lang === 'en') {
          return delta > 0
            ? `${label}: +${delta} PT vs meta — your roster and habits need more weight here than the generic pack.`
            : `${label}: ${delta} PT vs meta — we trim here because your setup does not need the community surplus.`
        }
        return delta > 0
          ? `${label}: +${delta} PT rispetto al meta — la tua rosa e il tuo gioco chiedono più peso qui del pack generico.`
          : `${label}: ${delta} PT rispetto al meta — alleggeriamo qui perché il tuo sistema non ha bisogno del surplus community.`
      })
      sections.push({
        id: 'vsMeta',
...
function formatBuildResult(
  ...
  const maxReasons = variant === 'roster' ? 8 : 5
