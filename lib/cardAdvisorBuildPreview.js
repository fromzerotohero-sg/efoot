...
function buildVsMetaPositiveLines(deltas, lang, teamStyleLabel) {
  ...
  if (lines.length && lang === 'en') {
    lines.push('You can trust the Recommended build — the benchmark is only there to compare.')
  } else if (lines.length) {
    lines.push('Puoi applicare tranquillo la build Consigliata: il confronto serve solo a capire la differenza.')
  }
  return lines.slice(0, 3)
}
  ...
  if (lang === 'en') {
    const ovr =
      playOverall && cardOverall && playOverall !== cardOverall
        ? ` Estimated profile with these points: ${playOverall} (card at level 1: ${cardOverall}).`
        : ''
    return `This build fits how you play${setup ? ` (${setup})` : ''}${skillsClause ? ` with ${skillsClause}` : ''}: you will ${p1}${p2 ? ` and ${p2}` : ''}.${ovr}`
  }
  const ovr =
    playOverall && cardOverall && playOverall !== cardOverall
      ? ` Profilo stimato con questi PT: ${playOverall} (carta a liv. 1: ${cardOverall}).`
      : ''
  return `Build pensata per come giochi tu${setup ? ` (${setup})` : ''}${skillsClause ? `, con ${skillsClause}` : ''}: in partita ${p1}${p2 ? ` e ${p2}` : ''}.${ovr}`
}
...
function buildMetaWhyLead({ lang, pointsUsed, topSliders, skillsClause, role }) {
  ...
  if (lang === 'en') {
    return `How many players usually spread points on ${role || 'this role'}${skillsClause ? ` (${skillsClause})` : ''} — for comparison only. Typical focus: ${phrase}.`
  }
  return `Come spesso vengono distribuiti i punti su ${role || 'questo ruolo'}${skillsClause ? ` (${skillsClause})` : ''} — solo per confronto. Di solito punta a: ${phrase}.`
}
...
function buildMetaReasonSections({ lang, sliders, pointsUsed, card, enrichedCatalog, engineReasons }) {
  ...
  if (skillsClause && playingStyle && playingStyle !== 'Profilo da analizzare') {
    contextItems.push(
      lang === 'en'
        ? `${playingStyle} with ${skillsClause} — a common spread for this card type (not your squad).`
        : `${playingStyle} con ${skillsClause}: distribuzione tipica per questa carta (non la tua rosa).`
    )
  } else if (skillsClause) {
    contextItems.push(
      lang === 'en'
        ? `This card stands out for ${skillsClause}.`
        : `Punto di forza della carta: ${skillsClause}.`
    )
  } else if (role) {
    contextItems.push(
      lang === 'en'
        ? `Typical point spread for ${role} — without your formation or coach.`
        : `Distribuzione tipica per ${role} — senza il tuo modulo o allenatore.`
    )
  }
  ...
  if (top) {
    allocItems.push(
      lang === 'en'
        ? `Main focus: ${getBuildSliderLabel(top.key, lang)} (${top.val} PT) — you will ${macroGameplayPhrase(top.key, lang)}.`
        : `Punto principale: ${getBuildSliderLabel(top.key, lang)} (${top.val} PT) — così ${macroGameplayPhrase(top.key, lang)}.`
    )
  }
...
function buildRosterReasonSections({
  ...
  if (topSliders.length >= 2) {
    ...
    allocItems.push(
      lang === 'en'
        ? `With ${pointsUsed} points we focus on ${getBuildSliderLabel(a.key, lang)} (${a.val}) and ${getBuildSliderLabel(b.key, lang)} (${b.val}) — you will ${macroGameplayPhrase(a.key, lang)} and ${macroGameplayPhrase(b.key, lang)}.`
        : `Con ${pointsUsed} PT puntiamo su ${getBuildSliderLabel(a.key, lang)} (${a.val}) e ${getBuildSliderLabel(b.key, lang)} (${b.val}) — così ${macroGameplayPhrase(a.key, lang)} e ${macroGameplayPhrase(b.key, lang)}.`
    )
  } else if (topSliders.length === 1) {
    const t = topSliders[0]
    allocItems.push(
      lang === 'en'
        ? `With ${pointsUsed} points on ${getBuildSliderLabel(t.key, lang)} (${t.val}) you will ${macroGameplayPhrase(t.key, lang)}.`
        : `Con ${pointsUsed} PT su ${getBuildSliderLabel(t.key, lang)} (${t.val}) ${macroGameplayPhrase(t.key, lang)}.`
    )
  }
  for (const reason of engineReasons.slice(0, 1)) {
    if (reason) allocItems.push(reason)
