...
export const BUILD_SLIDER_LABELS = {
  shooting: { it: 'Tiro', en: 'Shooting', es: 'Remate' },
  ...
  lowerBodyStrength: { it: 'Forza arti inferiori', en: 'Lower body', es: 'Fuerza piernas' },
  aerialStrength: { it: 'Forza in aria', en: 'Aerial', es: 'Fuerza aérea' },
  defending: { it: 'Difesa', en: 'Defending', es: 'Defensa' },
  gk1: { it: 'PT 1', en: 'GK 1', es: 'POR 1' },
  gk2: { it: 'PT 2', en: 'GK 2', es: 'POR 2' },
  gk3: { it: 'PT 3', en: 'GK 3', es: 'POR 3' }
}
...
const TEAM_STYLE_LABELS = {
  contrattacco: { it: 'Contrattacco', en: 'Long Ball Counter', es: 'Contraataque' },
  contropiede_veloce: { it: 'Contropiede veloce', en: 'Quick Counter', es: 'Contraataque rápido' },
  possesso_palla: { it: 'Possesso palla', en: 'Possession Game', es: 'Posesión' },
  vie_laterali: { it: 'Vie laterali', en: 'Out Wide', es: 'Juego por bandas' },
  passaggio_lungo: { it: 'Passaggio lungo', en: 'Long Ball', es: 'Balón largo' },
  pressing_totale: { it: 'Pressing totale', en: 'Overload', es: 'Presión total' }
}
...
function inferAdvisorSlotPosition(card, rosterContext) {
  ...
  const starter = players.find(player => {
    return isStarterPlayer(player)
      && String(player.position || '').trim().toUpperCase() === target
  })
...
function simplifyEngineReason(text, lang) {
  ...
  const replacementsEs = [
    [/Passaggio valorizzato per creare linee di gioco e servire compagni\.?/i, 'Sirves mejor a los compañeros y llegas antes a las zonas peligrosas.'],
    [/Dribbling e controllo stretto rinforzati per sfruttare le sue qualità tecniche e conduzione\.?/i, 'Más seguridad con el balón cuando atacas.'],
    [/Dribbling e controllo stretto rinforzati per sfruttare skill tecniche e conduzione\.?/i, 'Más seguridad con el balón cuando atacas.'],
    [/Destrezza usata per comportamento offensivo, accelerazione ed equilibrio nel ruolo\.?/i, 'Más explosividad en las arrancadas: te desmarcas antes.'],
    [/Forza arti inferiori usata per velocita, potenza e resistenza nel sistema squadra\.?/i, 'Piernas más reactivas durante todo el partido y en las transiciones.'],
    [/Tiro potenziato per finalizzazione e minaccia offensiva coerente con le skill\.?/i, 'Más peligroso cuando te presentas frente a la portería.'],
    [/Difesa prioritaria per copertura, contrasti e protezione della squadra\.?/i, 'Más sólido en los duelos y cuando recuperas el balón.'],
    [/Forza in aria scelta per duelli fisici, salto e contatto\.?/i, 'Más fuerte en los duelos aéreos y en las segundas jugadas.'],
    [/Abilita native considerate per dare peso alle macro piu coerenti\.?/i, ''],
    [/Abilità native considerate per dare peso alle macro più coerenti\.?/i, ''],
    [/Build adattata allo stile squadra: ([^.]+)\.?/i, 'Pensada para cómo juegas con $1.'],
    [/Portiere ottimizzato sulle macro PT e presenza fisica\.?/i, 'Puntos elegidos para darte más seguridad bajo los palos.'],
    [/macro PT/gi, 'puntos'],
    [/non su numeri "di comodo"/gi, ''],
    [/non su statistiche "di comodo"/gi, ''],
    [/Passing was emphasized to open lanes and supply teammates\.?/i, 'Sirves mejor a los compañeros y llegas antes a las zonas peligrosas.'],
    [/Dribbling and tight possession were boosted to leverage technical skills and ball carrying\.?/i, 'Más seguridad con el balón cuando atacas.'],
    [/Dexterity was used for offensive movement, acceleration and balance in role\.?/i, 'Más explosividad en las arrancadas: te desmarcas antes.'],
    [/Lower-body strength was used for pace, shot power and stamina within the squad system\.?/i, 'Piernas más reactivas durante todo el partido y en las transiciones.'],
    [/Shooting was boosted for finishing and offensive threat aligned with skills\.?/i, 'Más peligroso cuando te presentas frente a la portería.'],
    [/Defending was prioritized for coverage, duels and team protection\.?/i, 'Más sólido en los duelos y cuando recuperas el balón.'],
    [/Aerial strength was chosen for physical duels, jumping and contact\.?/i, 'Más fuerte en los duelos aéreos y en las segundas jugadas.'],
    [/Player skills were considered to weight the most coherent progression macros\.?/i, ''],
    [/Build tuned for team style: ([^.]+)\.?/i, 'Pensada para cómo juegas con $1.'],
    [/Goalkeeper tuned on GK progression macros and physical presence\.?/i, 'Puntos elegidos para darte más seguridad bajo los palos.']
  ]
  let out = raw
  const replacements = lang === 'en' ? replacementsEn : lang === 'es' ? replacementsEs : replacementsIt
...
const MACRO_GAMEPLAY_ES = {
  shooting: 'rematas mejor las ocasiones',
  passing: 'construyes y sirves antes de que la defensa se reorganice',
  dribbling: 'ganas los duelos en banda y mantienes el balón bajo presión',
  dexterity: 'te desmarcas en las arrancadas',
  lowerBodyStrength: 'sostienes el ritmo y las transiciones durante noventa minutos',
  aerialStrength: 'ganas balones altos y segundas jugadas',
  defending: 'recuperas y cubres con más autoridad',
  gk1: 'dominas bajo los palos',
  gk2: 'dominas bajo los palos',
  gk3: 'dominas bajo los palos'
}
...
function getReasonSectionTitle(id, lang) {
  const titles = {
    context: { it: 'In pratica per te', en: 'What this means for you', es: 'En la práctica para ti' },
    skills: { it: 'Cosa senti in partita', en: 'What you feel in matches', es: 'Qué sientes en los partidos' },
    allocation: { it: 'Dove mettiamo i punti', en: 'Where the points go', es: 'Dónde ponemos los puntos' },
    vsMeta: { it: 'Perché questa è la scelta giusta', en: 'Why this fits you better', es: 'Por qué esta es la elección correcta' }
  }
  if (lang === 'en') return titles[id]?.en || ''
  if (lang === 'es') return titles[id]?.es || ''
  return titles[id]?.it || ''
}

function macroGameplayPhrase(key, lang) {
  const map = lang === 'en' ? MACRO_GAMEPLAY_EN : lang === 'es' ? MACRO_GAMEPLAY_ES : MACRO_GAMEPLAY_IT
  const fallback = lang === 'en' ? 'it fits your match plan' : lang === 'es' ? 'se adapta a tu estilo de juego' : 'serve al tuo modo di giocare'
      ...
      if (labels.length) {
        pushLine(
          ...
          lang === 'en'
            ...
            : lang === 'es'
            ? `Tu carta tiene ${joinLabels(labels, lang)}: los puntos en Remate te hacen ${outcome}.`
      ...
      if (labels.length) {
        pushLine(
          ...
          lang === 'en'
            ...
            : lang === 'es'
            ? `Con ${joinLabels(labels, lang)}, los puntos en Pase te hacen ${outcome}.`
            : `Con ${joinLabels(labels, lang)}, i punti in Passaggio ti fanno ${outcome}.`
        )
      }
    }
    if (macroKey === 'dribbling') {
      const matched = filterSkillsByPattern(allTokens, /double touch|tap trick|sole control|flip flap|chop turn|cut behind|scissors|marseille|momentum dribbling|trickster|mazing run|magnetic feet|acceleration burst/i)
      const labels = labelSkills(matched, lang)
      if (labels.length) {
        pushLine(
          macroKey,
          lang === 'en'
            ...
            : lang === 'es'
            ? `Tus habilidades (${joinLabels(labels, lang)}) cuentan aquí: los puntos en Regate te hacen ${outcome}.`
      ...
      if (labels.length) {
        pushLine(
          ...
          lang === 'en'
            ...
            : lang === 'es'
            ? `Con ${joinLabels(labels, lang)}, la Destreza te hace ${outcome}.`
      ...
      if (labels.length) {
        pushLine(
          ...
          lang === 'en'
            ...
            : lang === 'es'
            ? `Con ${joinLabels(labels, lang)}, las piernas aguantan así ${outcome}.`
      ...
      if (labels.length) {
        pushLine(
          ...
          lang === 'en'
            ...
            : lang === 'es'
            ? `En el aire (${joinLabels(labels, lang)}): ${outcome}.`
    ...
    if (delta > 0) {
      lines.push(
        lang === 'en'
          ...
          : lang === 'es'
          ? `Ponemos más puntos en ${label} que la referencia porque tu ${teamStyleLabel || 'estilo'} lo necesita — ${outcome}.`
      ...
      lines.push(
        lang === 'en'
          ...
          : lang === 'es'
          ? `Usamos menos puntos en ${label} que la referencia: los PT se quedan donde te hacen ganar los partidos.`
...
function buildRosterWhyLead({
  ...
  const setup = [formation, teamStyleLabel, slot ? (lang === 'en' ? `as ${slot}` : lang === 'es' ? `como ${slot}` : `da ${slot}`) : null]
    .filter(Boolean)
    .join(' · ')
  ...
  if (lang === 'es') {
    const ovr =
      playOverall && cardOverall && playOverall !== cardOverall
        ? ` OVR estimado con estos PT: ${playOverall} (carta nivel 1: ${cardOverall}).`
        : ''
    return `Para tu plantilla${setup ? ` (${setup})` : ''}: ${spread || 'consulta los sliders arriba'}. En partido ${p1}${p2 ? ` y ${p2}` : ''}.${ovr}`
  }
  const ovr =
    playOverall && cardOverall && playOverall !== cardOverall
...
function buildMetaWhyLead({ lang, sliders, role }) {
  ...
  if (lang === 'es') {
    return spread
      ? `Referencia (${role || 'rol'}): ${spread}. Perfil de paquete genérico, sin tu plantilla, formación o entrenador — en partido usa la build recomendada.`
      : `Referencia para ${role || 'este rol'}: perfil genérico. En partido usa la build recomendada.`
  }
  return spread
...
function buildRosterReasonSections({
  ...
  if (slot) tacticalBits.push(lang === 'en' ? `slot ${slot}` : lang === 'es' ? `rol ${slot}` : `ruolo ${slot}`)
  ...
  if (skillsClause) {
    contextItems.push(
      lang === 'en'
        ...
        : lang === 'es'
        ? `Tu carta: ${skillsClause}${playingStyle && playingStyle !== 'Profilo da analizzare' ? ` · ${playingStyle}` : ''}. Reforzamos lo que ya sabes hacer.`
  ...
  if (tacticalBits.length) {
    contextItems.push(
      lang === 'en'
        ...
        : lang === 'es'
        ? `Tu plantilla (${tacticalBits.join(' · ')}): los puntos se adaptan a tu estilo de juego, no a una build genérica.`
    ...
    contextItems.push(
      lang === 'en'
        ...
        : lang === 'es'
        ? `Rol ${role}: pensada para tu formación.`
