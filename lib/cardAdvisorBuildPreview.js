...
export function getBuildSliderLabel(key, lang = 'it') {
  ...
  if (!entry) return key
  return (lang === 'en' || lang === 'es') ? entry.en : entry.it
}
...
function simplifyEngineReason(text, lang) {
  ...
  let out = raw
  for (const [pattern, value] of (lang === 'en' || lang === 'es') ? replacementsEn : replacementsIt) {
...
function getReasonSectionTitle(id, lang) {
  ...
  return titles[id]?.[(lang === 'en' || lang === 'es') ? 'en' : 'it'] || ''
}

function macroGameplayPhrase(key, lang) {
  const map = (lang === 'en' || lang === 'es') ? MACRO_GAMEPLAY_EN : MACRO_GAMEPLAY_IT
  return map[key] || ((lang === 'en' || lang === 'es') ? 'it fits your match plan' : 'serve al tuo modo di giocare')
}
...
function buildRosterWhyLead({
  ...
  const spread = formatTopSlidersLine(rosterSliders, lang, 3).join((lang === 'en' || lang === 'es') ? ' · ' : ' · ')
  ...
  const setup = [formation, teamStyleLabel, slot ? ((lang === 'en' || lang === 'es') ? `as ${slot}` : `da ${slot}`) : null]
    .filter(Boolean)
    .join(' · ')
...
function buildMetaWhyLead({ lang, sliders, role }) {
  const spread = formatTopSlidersLine(sliders, lang, 4).join((lang === 'en' || lang === 'es') ? ' · ' : ' · ')
  if (lang === 'en') {
    return spread
...
function buildRosterReasonSections({
  ...
  if (slot) tacticalBits.push((lang === 'en' || lang === 'es') ? `slot ${slot}` : `ruolo ${slot}`)
