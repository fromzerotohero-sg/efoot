...
function formatBuildResult(build, lang) {
  if (!build?.ok) {
    return {
      ok: false,
  ...
  return {
    ...
    playOverall: build.inGameOverall ?? build.afterOverall,
    fieldOverall: build.fieldOverall ?? null,
    targetPosition: build.targetPosition,
    reasons: pickBilingualList(build.reasons, lang).slice(0, 3),
    warnings: pickBilingualList(build.warnings, lang).slice(0, 2),
...
export async function computeCardAdvisorBuildPreview({ card, catalogRow, rosterContext, lang = 'it', admin = null }) {
  ...
  let virtual = cardToVirtualPlayer(card, catalogCard)
  const enrichedCatalog =
    admin != null ? (await enrichCatalogCardForBuildCoach(virtual, catalogCard)) || catalogCard : catalogCard
  ...
  const meta = formatBuildResult(metaRaw, lang)
  const rosterBuild = rosterRaw ? formatBuildResult(rosterRaw, lang) : null
