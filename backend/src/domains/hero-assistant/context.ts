...
async function appendTacticalFeedback(client, userId, summary, lang) {
  try {
    ...
    const summaryAlreadyHasFeedback = /Esperienza Coach|Coach Experience|feedback Palestra|Coach Gym feedback/i.test(summary)
    return feedback && !summaryAlreadyHasFeedback ? trimContext(`${feedback}\n\n${summary}`) : summary
  } catch {
    return summary
  }
}
