...
function buildCardAdvisorRagQuery(card, catalogCard, tacticalSettings) {
  return [
    ...
    card?.position,
    card?.style || catalogCard?.playing_style,
...
export function buildDeepAnalysisRequest({
  ...
  const players = Array.isArray(rosterContext?.players) ? rosterContext.players : []
  const starters = players.filter((player) => player?.slot_index !== null && player?.slot_index !== undefined)
  const reserves = players.filter((player) => player?.slot_index === null).slice(0, 12)
  const diagnostic = rosterContext?.diagnostic
  const diagnostics = diagnostic && typeof diagnostic === 'object'
    ? { ...diagnostic, content: clean(diagnostic.content, 2000) }
    : diagnostic
  const ragKnowledge = getRelevantSections(
    buildCardAdvisorRagQuery(card, catalogCard, rosterContext?.tacticalSettings),
    5000
  )
  const context = {
    card,
    ...
    player_performance: Array.isArray(rosterContext?.performance)
      ? rosterContext.performance.slice(0, 8)
      : [],
...
RAG
${ragKnowledge || 'No relevant RAG available.'}`
    }],
