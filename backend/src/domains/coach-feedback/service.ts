...
import type { OpenAiProvider, OpenAiProviderError } from '../../openai.js'
import type { MockCreditProvider } from '../../types.js'
import type { createCoachFeedbackDb, ChatMatch, ChatProfile, OwnedMatch } from './db.js'
...
const PROFILE_WHITELIST: Record<string, readonly string[]> = {
...
const WEAK_POINT_TO_LABEL: Record<string, string> = {
  defence: 'Difesa',
  attack: 'Attacco',
  set_pieces: 'Piazzati',
...
export interface CoachFeedbackError extends Error {
  statusCode: number
  type: string
}
...
// TODO(ts): unify with MockCreditProvider / MetalGate credit contract (amount + userId on deduct/refund).
...
export interface AiKnowledgeLike {
  update(input: { token: string; userId: string }): Promise<unknown>
}

function serviceError(message: string, statusCode: number, type = 'validation'): CoachFeedbackError {
  const error = new Error(message) as CoachFeedbackError
  error.statusCode = statusCode
  error.type = type
  return error
}

function assertLive(config: Pick<BackendConfig, 'dormant'> | Record<string, unknown>) {
  if ((config as { dormant?: boolean })?.dormant) {
...
export function normalizeHistory(raw: unknown): ChatHistoryMessage[] {
  ...
  const output: ChatHistoryMessage[] = []
...
export function buildProfileContext(profile: ChatProfile | null | undefined): string {
  if (!profile) return ''
  const lines: string[] = []
...
export function buildMatchContext(match: ChatMatch | OwnedMatch | null | undefined): string {
  if (!match) return ''
  const lines: string[] = []
  ...
  if ('match_date' in match && match.match_date) {
    lines.push(`Data: ${new Date(match.match_date).toLocaleDateString('it-IT')}`)
  }
  return lines.join('\n')
}

export function buildSystemPrompt(lang: string | undefined, profileContext: string, matchContext: string): string {
  const isIt = lang === 'it'
  const rules = isIt
...
export function validateProfileUpdates(raw: unknown): Record<string, unknown> {
  ...
  if (source.ai_notes !== undefined && source.ai_notes !== null) {
    const value = String(source.ai_notes).trim()
    if (value && value.length <= MAX_NOTES) clean.ai_notes = value
  }
  if (source.hours_per_week !== undefined && source.hours_per_week !== null) {
    const value = Number.parseInt(String(source.hours_per_week), 10)
    if (Number.isFinite(value) && value >= 0 && value <= 168) clean.hours_per_week = value
  }
  return clean
}

export function validateInsights(raw: unknown): TacticalInsight[] {
  ...
  return raw
    .filter((item): item is { type: string; text: string } =>
      Boolean(item) &&
      typeof item === 'object' &&
      validTypes.has((item as { type?: string }).type as string) &&
      typeof (item as { text?: unknown }).text === 'string' &&
      Boolean((item as { text: string }).text.trim().length > 0))
    .slice(0, 10)
...
export function buildExtractionPrompt(
  ...
  matchInfo: OwnedMatch | ChatMatch | null
): string {
...
async function charge(
  credits: CreditsLike,
  capability: string,
  input: { userId: string; idempotencyKey?: unknown }
): Promise<ChargedCredits> {
  const charged = {
    capability,
    amount: AI_COST,
    userId: input.userId,
...
async function withRefund<T>(
  credits: CreditsLike,
  charged: ChargedCredits,
  action: () => Promise<T>,
  logger: { error?: (payload: unknown, message?: string) => void } | Console
): Promise<T> {
  try {
    return await action()
  } catch (error) {
...
async function completionText(openai: OpenAiProvider, body: unknown, operation: string): Promise<string | undefined> {
  const response = await openai.complete(body, operation)
  const data = await response.json() as { choices?: Array<{ message?: { content?: string } }> }
  return data?.choices?.[0]?.message?.content
}

async function chatCompletion(openai: OpenAiProvider, body: Record<string, unknown>): Promise<string | undefined> {
  try {
    return await completionText(openai, body, 'coach-feedback-chat')
  } catch (error) {
    const err = error as OpenAiProviderError
    if (err?.type !== 'model_not_found' || body.model === 'gpt-4o') throw error
    return completionText(openai, { ...body, model: 'gpt-4o' }, 'coach-feedback-chat-fallback')
  }
}

export interface CoachFeedbackChatInput {
  message?: unknown
  history?: unknown
  lang?: string
  token: string
  ...
  rateLimit?: { remaining?: number; resetAt?: unknown }
}

export interface CoachFeedbackSaveInput {
...
export function createCoachFeedbackService({
  ...
  credits: CreditsLike | MockCreditProvider
  db: ReturnType<typeof createCoachFeedbackDb>
  aiKnowledge: AiKnowledgeLike
  config?: Partial<BackendConfig> & { openAiModel?: string; dormant?: boolean }
  logger?: { error?: (payload: unknown, message?: string) => void } | Console
}) {
  ...
  return {
    async chat(input: CoachFeedbackChatInput) {
      assertLive(config)
      ...
      const charged = await charge(credits as CreditsLike, 'coach-feedback-chat', input)
      return withRefund(credits as CreditsLike, charged, async () => {
    ...
    async save(input: CoachFeedbackSaveInput) {
      assertLive(config)
      const conversation = Array.isArray(input.conversation)
        ? input.conversation.slice(-MAX_CONVERSATION_MESSAGES) as Array<{ role?: string; content?: string }>
        : []
      if (conversation.length === 0) {
        throw serviceError('Empty conversation', 400, 'conversation_required')
      }
      const sessionType = ['profile_setup', 'feedback', 'update'].includes(input.sessionType as string)
        ? String(input.sessionType)
        : 'feedback'
      ...
      const charged = await charge(credits as CreditsLike, 'save-coach-feedback', input)

      const result = await withRefund(credits as CreditsLike, charged, async () => {
        ...
        if (profileUpdates.ai_weak_point) {
          profileUpdates.common_problems = [
            WEAK_POINT_TO_LABEL[String(profileUpdates.ai_weak_point)] || profileUpdates.ai_weak_point
          ]
        }
        ...
        const summary = typeof extracted.conversation_summary === 'string'
          ...
          : ''
        const outcome = ['win', 'loss', 'draw'].includes(extracted.outcome as string)
          ? String(extracted.outcome)
          : null
