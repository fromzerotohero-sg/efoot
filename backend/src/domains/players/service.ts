...
export function createPlayerWriteService(writeProvider, options = {}) {
  ...
  const scheduleKnowledgeRefresh = (userId, token, source) => {
    try {
  ...
  return {
    async save({ token, userId, player, formationLayout }) {
      ...
      if (playerData.slot_index != null) {
        ...
        if (existing) {
          ...
          scheduleKnowledgeRefresh(userId, token, 'players.save.updated')
      ...
      scheduleKnowledgeRefresh(userId, token, 'players.save.created')
    ...
    async patchById({ token, userId, playerId, patch }) {
      ...
      scheduleKnowledgeRefresh(userId, token, 'players.patch')
      return { player: data }
    },
    ...
    async assignToSlot(input) {
      ...
      scheduleKnowledgeRefresh(input.userId, input.token, 'roster.assign-to-slot')
      return result
    },
    ...
    async removeFromSlot(input) {
      ...
      scheduleKnowledgeRefresh(input.userId, input.token, 'roster.remove-from-slot')
      return result
    },
    ...
    async deleteById({ token, userId, playerId }) {
      ...
      scheduleKnowledgeRefresh(userId, token, 'players.delete')
