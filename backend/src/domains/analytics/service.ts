...
import { calculateTacticalPatterns, type UsageMap } from '../matches/patterns.js'
import { domainError } from '../matches/payloads.js'

type TacticalPatternsInsert = Database['public']['Tables']['team_tactical_patterns']['Insert']
...
export interface AnalyticsReadService {
  getGameAnalysis(params: { token: string; userId: string }): Promise<GameAnalysisResult>
}

export interface RecalculatedPatterns {
  formation_usage: UsageMap
  playing_style_usage: UsageMap
  recurring_issues: Json[]
}

export interface AnalyticsWriteService {
  recalculatePatterns(params: {
    token: string
    userId: string
    requestedUserId?: string
    now?: Date
  }): Promise<RecalculatedPatterns | null>
  saveGameAnalysis(params: {
    token: string
    userId: string
    stats: unknown
    capturedAt?: Date | string
  }): Promise<{ success: true; captured_at: string }>
}

export function createAnalyticsReadService(readProvider: ReadOnlySupabaseProvider): AnalyticsReadService {
  return {
...
export function createAnalyticsWriteService(writeProvider: UserSupabaseWriteProvider): AnalyticsWriteService {
  return {
    async recalculatePatterns({ token, userId, requestedUserId, now = new Date() }) {
      ...
      const payload: TacticalPatternsInsert = {
        user_id: userId,
        ...patterns,
    ...
    async saveGameAnalysis({ token, userId, stats, capturedAt = new Date() }) {
      ...
      const result = await client
        .from('user_game_analysis')
        .upsert(
          { user_id: userId, stats: stats as Json, captured_at: timestamp },
          { onConflict: 'user_id' }
        )
...
export function registerAnalyticsRoutes(
  app: FastifyInstance,
  { identity, analyticsReads, analyticsWrites }: {
    identity: IdentityProvider
    analyticsReads: AnalyticsReadService
    analyticsWrites: AnalyticsWriteService
  }
): void {
  const fail = (reply: FastifyReply, error: any) =>
    reply.code(error.statusCode || 500).send({ error: error.message || 'Internal error' })

  app.post<{ Body: { user_id?: string } }>('/v1/analytics/recalculate-patterns', async (request, reply) => {
    try {
  ...
  app.put<{ Body: { stats?: unknown; captured_at?: string } }>('/v1/analytics/game-analysis', async (request, reply) => {
    try {
      const session = await identity.resolveUser(request)
