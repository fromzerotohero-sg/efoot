**Versione**: 9.0.2 ENTERPRISE | **Data**: 17 Agosto 2026 | **Ruleset**: eFootball v6.0.0 | **Lingua**: Italiano
...
| **Stile Squadra** | 🔧 MODIFICABILE | Possesso palla, Contropiede, Pressing totale, ecc. - Scelta utente |
...
**Sigle posizioni — bridge ES ufficiale Konami** (l'AI deve riconoscere anche il client spagnolo; il corpus resta italiano):

| ES (client spagnolo) | EN | Significato |
|---|---|---|
| DC | CF | Delantero centro / Prima punta |
| SD | SS | Segundo delantero / Seconda punta |
| ED | RWF | Extremo derecho / Esterno offensivo destro |
| EI | LWF | Extremo izquierdo / Esterno offensivo sinistro |
| MCO | AMF | Mediapunta / Trequartista |
| MC | CMF | Centrocampista central |
| MD | RMF | Mediocampista derecho |
| MI | LMF | Mediocampista izquierdo |
| MCD | DMF | Mediocentro defensivo / Mediano |
| LTD | RB | Lateral derecho / Terzino destro |
| LTI | LB | Lateral izquierdo / Terzino sinistro |
| DFC | CB | Defensa central / Difensore centrale |
| POR | GK | Portero / Portiere |
...
**REGOLA AI per uso multilingua**: il client italiano del gioco usa "Tra le linee" e "Onnipresente". Il catalogo inglese (PSD) usa "Orchestrator" e "Box-to-Box". Il client spagnolo usa "Orquestador" e "Box-to-Box★". Sono lo **stesso stile**: parlando con utenti italiani usa il nome IT del client (es. "Tra le linee"); parlando con utenti inglesi usa "Orchestrator"; parlando con utenti spagnoli usa "Orquestador". Se un giocatore in rosa appare con "Orchestratore", "Orchestrator", "Orquestador" o "Box-to-Box" salvato dal catalogo, NON dire "non esiste questo stile": è lo stesso di "Tra le linee" / "Onnipresente". Alias ES noti dello stesso catalogo già gestito (non sono stili extra): Oportunista = Opportunista; Cazagoles = Rapace d'area; Delantero de apoyo = Attaccante di rientro; Hombre objetivo = Fulcro di gioco; Ancla = Collante; Destructor = Incontrista; Construcción = Sviluppo.
...
**REGOLA AI nomi abilità (IT/ES/EN)**: i nomi italiani canonici in questa sezione seguono il client ufficiale eFootball (allineati a `playerSkillLabels.js`). In rosa, catalogo PSD o eFootball Lab possono comparire **alias EN/ES** o varianti community: usa la mappatura §8.11 e NON dire "abilità inesistente" se riconosci l'alias.
...
### 8.11 Showtime e abilità recenti (sinonimi IT/ES/EN)

**Regola per l'AI**: se in rosa compare un nome EN o ES (catalogo/EFHub/client spagnolo), usa l'effetto della riga IT corrispondente. Molte voci sono **sinonimi** di §8.1–8.4, non abilità extra da sommare due volte.
