...
export function translate(key, lang = 'it', params = {}) {
  ...
  const translation = serverTranslations[safeLang]?.[key] || serverTranslations.en?.[key] || key
...
export async function generateWeeklyTasksForUser(userId, supabaseUrl, serviceKey, week, lang = 'it') {
  ...
  try {
    ...
    if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] Fetching profile for user ${userId}`)
    const { data: profile, error: profileError } = await admin
      .from('user_profiles')
      .select('user_id, first_name, last_name, common_problems, current_division')
      .eq('user_id', userId)
      .maybeSingle()
    ...
    if (profileError) {
      ...
      if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] Profile found: ${profile ? 'yes' : 'no'}`)
    }

    // 2. Recupera ultime 10 partite per analisi
    if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] Fetching matches for user ${userId}`)
    const { data: matches, error: matchesError } = await admin
      .from('matches')
      .select('id, user_id, match_date, result, is_home, team_stats, data_completeness, recommended_formation_used, formation_played')
      .eq('user_id', userId)
    ...
    if (matchesError) {
      ...
      if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] Matches found: ${matches?.length || 0}`)
    }

    // 3. Recupera pattern tattici
    if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] Fetching patterns for user ${userId}`)
    const { data: patterns, error: patternsError } = await admin
      .from('team_tactical_patterns')
      .select('formation_usage, playing_style_usage, recurring_issues, last_50_matches_count')
      .eq('user_id', userId)
      .maybeSingle()
    ...
    if (patternsError) {
      ...
      if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] Patterns found: ${patterns ? 'yes' : 'no'}`)
    }
    ...
    if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] Generated ${tasks.length} tasks from data analysis`)
    ...
    if (!tasks || tasks.length === 0) {
      ...
      if (genericTasks && genericTasks.length > 0) {
        ...
        if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] Added ${genericTasks.length} generic tasks as fallback`)
      } else {
        ...
        if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] Added 3 hardcoded generic tasks as last fallback`)
      }
    }

    // 6. Verifica se ci sono già task per questa settimana (o giorni vicini per tolleranza)
    if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] Checking for existing tasks for user ${userId}, week ${week.start}`)
    ...
    if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] Existing tasks check result: ${existingTasks?.length || 0} tasks found in range ${rangeStartStr} - ${rangeEndStr}`)

    if (existingTasks && existingTasks.length > 0) {
      if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] Tasks already exist for week range (found ${existingTasks.length}), skipping generation`)
      // Restituisci i task esistenti invece di array vuoto
      const { data: existingTasksFull, error: fetchError } = await admin
        .from('weekly_goals')
        .select('id, user_id, goal_type, goal_description, target_value, current_value, difficulty, week_start_date, week_end_date, status, created_at, completed_at')
        .eq('user_id', userId)
        .gte('week_start_date', rangeStartStr)
        .lte('week_start_date', rangeEndStr)
      ...
      if (!fetchError && existingTasksFull) {
        if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] Returning ${existingTasksFull.length} existing tasks`)
        return existingTasksFull
      }
      return []
    }
    
    if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] No existing tasks found for week ${week.start}, proceeding with generation`)
    ...
    if (tasks.length > 0) {
      ...
      if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] Inserting ${tasksToInsert.length} tasks into database for user ${userId}, week ${week.start}`)
      ...
      const { data: insertedTasks, error: insertError } = await admin
        ...
        .select('id, user_id, goal_type, goal_description, target_value, current_value, difficulty, week_start_date, week_end_date, status, created_at')
      ...
      if (insertError) {
        ...
        if (insertError.code === '23505' || insertError.message?.includes('unique') || insertError.message?.includes('duplicate')) {
          if (process.env.NODE_ENV !== 'production') {
            console.warn('[TaskHelper] Duplicate task detected (race condition), recovering existing tasks...')
          }
          ...
          if (!fetchAfterError && existingTasksAfterError && existingTasksAfterError.length > 0) {
            if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] Recovered ${existingTasksAfterError.length} existing tasks after duplicate error`)
...
async function generateTasksBasedOnData(profile, matches, patterns, lang = 'it') {
  ...
  if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] generateTasksBasedOnData called with: matches=${matches?.length || 0}, profile=${!!profile}, patterns=${!!patterns}`)

  // Se non ci sono abbastanza dati, genera task generici (sempre almeno 3 task)
  if (!matches || matches.length < 3) {
    if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] User has ${matches?.length || 0} matches (< 3), generating generic tasks`)
    ...
    if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] Generated ${tasks.length} generic tasks`)
    return tasks // Restituisci sempre almeno 3 task generici
  }
  
  if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] User has ${matches.length} matches (>= 3), generating personalized tasks`)
...
export async function updateTasksProgressAfterMatch(userId, supabaseUrl, serviceKey, matchData) {
  ...
  try {
        ...
        if (progressChanged) {
          ...
          if (updateError) {
            console.error(`[TaskHelper] Error updating task ${task.id}:`, updateError)
             ...
             console.warn(`[TaskHelper] Update returned no data for task ${task.id} (user ${userId}). Verify task exists and permissions.`)
          } else {
    ...
    if (completedTasks.length > 0) {
      if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] ${completedTasks.length} task completati, avvio aggiornamento AI Knowledge...`)
      ...
      import('./aiKnowledgeHelper').then(async ({ updateAIKnowledgeScore }) => {
        try {
          ...
          if (process.env.NODE_ENV !== 'production') console.log(`[TaskHelper] AI Knowledge aggiornato: score=${result.score}, level=${result.level}`)
        } catch (err) {
