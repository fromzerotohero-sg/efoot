  ...
  React.useEffect(() => {
    const loadStatus = async () => {
      try {
        ...
        if (!res.ok)
          throw new Error(
            data?.error || "Impossibile caricare la ruota giornaliera",
          );
        setStatus(data);
        if (data?.claimed_today && data?.reward_amount != null) {
          setLastReward(Number(data.reward_amount));
          setMessage(
            "Hai già ritirato il premio di oggi. Torna domani per una nuova ruota.",
          );
        }
      } catch (err) {
    ...
    try {
      ...
      window.setTimeout(() => {
        ...
        window.dispatchEvent(
          new CustomEvent("credits-accredited", {
            detail: {
              amount: reward,
              source: "daily-spin",
    ...
    <div className="spin-lab-page">
      ...
      <main className="spin-shell">
        <section className="hero-card">
          ...
          <p>
            Ogni giorno puoi ritirare un bonus HP. Il premio appare nel saldo in
        ...
        <section className="game-card">
          <div className="wheel-stage" ref={wheelRef}>
            ...
            <div className="led-ring">
              {Array.from({ length: 36 }, (_, i) => (
                <i
                  key={i}
                  style={{ transform: `rotate(${i * 10}deg) translateY(-50%)` }}
                />
              ))}
            </div>
            ...
            >
              <svg
                ...
                className="wheel-svg"
                role="img"
                aria-label="Ruota premi Hero Points"
              >
                <defs>
                ...
                {segments.map((segment) => (
                  <g key={segment.reward}>
                    ...
                    >
                      <tspan x={segment.labelPoint.x} dy="-8">
                        {segment.reward}
                      </tspan>
                      <tspan x={segment.labelPoint.x} dy="25">
                        HP
                      </tspan>
                    </text>
                  </g>
          ...
          <div className="controls">
            <button
              ...
              onClick={spinWheel}
              disabled={isSpinning || loadingStatus || status?.claimed_today}
            >
              {loadingStatus ? (
                <>
      ...
      <style jsx>{`
        ...
        .stadium-lights {
          ...
          background: conic-gradient(
            from 180deg at 50% 0%,
            transparent 0deg,
            rgba(255, 255, 255, 0.12) 16deg,
            transparent 32deg,
            transparent 64deg,
            rgba(0, 212, 255, 0.12) 82deg,
            transparent 102deg
          );
