  ...
  return (
    <div className="neon-card" style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
        ...
        <h2 style={{ margin: 0, fontSize: '15px', fontWeight: 700, color: 'var(--text-main)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
...
function LineChartCard({ title, series, isPercent, t }) {
  ...
  return (
    <div className="neon-card" style={{
      ...
      <h3 style={{ margin: '0 0 12px', fontSize: 'clamp(14px, 3vw, 15px)', fontWeight: 700, color: 'var(--text-main)' }}>{title}</h3>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px 16px', marginTop: '8px', fontSize: 'clamp(10px, 2.5vw, 11px)', color: '#6b6b6b' }}>
        {series.map(({ label, tu, div1 }) => (
          <span key={String(label)} style={{ display: 'inline-flex', gap: '6px' }}>
            <strong style={{ color: 'var(--text-main)' }}>{t ? (t(String(label)) || String(label)) : String(label)}:</strong> Tu {formatVal(tu)} · Top {formatVal(div1)}
          </span>
        ))}
      </div>
    </div>
...
export default function GraficiComparazionePage() {
  ...
  return (
    <div className="ux-v2-light-tool" style={{ minHeight: '100%', overflowX: 'hidden', color: 'var(--text-main)' }}>
      <header style={{
        display: 'flex',
        alignItems: 'center',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <h1 style={{ margin: 0, fontSize: 'clamp(18px, 4vw, 22px)', fontWeight: 800, wordBreak: 'break-word', color: 'var(--text-main)' }}>{t('chartsAndComparisonTitle')}</h1>
          </div>
        </div>
      </header>
