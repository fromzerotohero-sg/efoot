...
export default function NewMatchPage() {
  ...
  React.useEffect(() => {
    ...
    const loadProgress = async () => {
      ...
      try {
        ...
        if (
          parsed.isHome !== undefined &&
          loadedStepData[HOME_AWAY_STEP_ID] === undefined
        ) {
        ...
        if (
          rawImages.player_ratings != null &&
          typeof rawImages.player_ratings === "string"
        ) {
        ...
        const firstEmptyStep = STEPS.findIndex(
          (step) =>
            loadedStepData[step.id] === undefined ||
            loadedStepData[step.id] === null,
        );
        if (firstEmptyStep >= 0) {
  ...
  const processSelectedImage = React.useCallback(
    async ({ section, slotIndex = null, file }) => {
      ...
      try {
        ...
        setStepImages((prev) => {
          if (
            ...
            const arr = Array.isArray(prev.player_ratings)
              ? [...prev.player_ratings]
              : [];
  ...
  const mergePlayerRatings = (listOfData) => {
    ...
    listOfData.forEach((d) => {
      ...
      if (d.cliente && typeof d.cliente === "object")
        Object.assign(cliente, d.cliente);
      if (d.avversario && typeof d.avversario === "object")
        Object.assign(avversario, d.avversario);
    });
    if (
      Object.keys(cliente).length === 0 &&
      Object.keys(avversario).length === 0
    )
      return listOfData[0] || null;
    return {
  ...
  const handleExtract = async (section) => {
    ...
    const images = isPlayerRatings
      ? (Array.isArray(stepImages.player_ratings)
          ? stepImages.player_ratings
          : stepImages.player_ratings
            ? [stepImages.player_ratings]
            : []
        ).filter(Boolean)
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        if (
          extractData.result &&
          typeof extractData.result === "string" &&
          extractData.result.trim()
        ) {
      ...
      const merged =
        isPlayerRatings && results.length > 0
          ? mergePlayerRatings(results)
          : results[0];
      ...
      setStepData((prev) => ({
        ...prev,
        [section]:
          isPlayerRatings && merged && typeof merged === "object"
            ? {
                cliente: merged.cliente || null,
                avversario: merged.avversario || null,
              }
            : merged,
      }));
  ...
  const photoSteps = React.useMemo(
    () => STEPS.filter((s) => s.id !== HOME_AWAY_STEP_ID),
    [STEPS],
  );
  const photosUploaded = React.useMemo(() => {
    return photoSteps.filter(
      (step) => stepData[step.id] && stepData[step.id] !== null,
    ).length;
  }, [stepData, photoSteps]);

  const photosMissing = React.useMemo(() => {
    return photoSteps
      .filter((step) => !stepData[step.id] || stepData[step.id] === null)
      .map((step) => step.label);
  }, [stepData, photoSteps]);
  ...
  const handleShowSummary = () => {
    const photosCount = photoSteps.filter(
      (step) => stepData[step.id] != null,
    ).length;
    if (photosCount < 3) {
  ...
  // Lock body scroll when summary modal is open (mobile-safe)
  React.useEffect(() => {
    if (showSummary) {
    ...
    try {
      ...
      const matchData = {
        ...
        playing_style_played:
          stepData.formation_style?.playing_style_played || null,
      ...
      try {
        ...
        throw new Error(
          mapErrorToUserMessage("500", t("saveMatchError"), lang).message,
        );
      }
  ...
  const playerRatingsImages =
    isPlayerRatingsStep && Array.isArray(stepImages.player_ratings)
      ? stepImages.player_ratings
      : stepImages.player_ratings
        ? [stepImages.player_ratings]
        : [];
  const currentImage = isPlayerRatingsStep
    ? playerRatingsImages[0] || null
    : stepImages[currentSection];
  const hasImageForExtract = isPlayerRatingsStep
    ? playerRatingsImages.filter(Boolean).length >= 1
    : !!stepImages[currentSection];
  ...
  const stepStatusLabel = (step) => {
    ...
    if (stepData[step.id] && stepData[step.id] !== null)
      return isItalian ? "Letto" : "Read";
    if (stepData[step.id] === null) return isItalian ? "Saltato" : "Skipped";
    if (step.id === "player_ratings") {
      const count = Array.isArray(stepImages.player_ratings)
        ? stepImages.player_ratings.filter(Boolean).length
        : stepImages.player_ratings
          ? 1
          : 0;
      return count > 0
  ...
  const primaryCtaStyle = {
    ...
    gap: "10px",
    boxShadow:
      "0 10px 30px rgba(61, 220, 151, 0.28), inset 0 1px 0 rgba(255,255,255,0.45)",
  };
    ...
    >
      ...
      >
        ...
        >
          <div>
            ...
            >
              {isItalian
                ? "Trasforma la partita in vantaggio competitivo."
                : "Turn your match into competitive edge."}
            </h1>
            <p
              style={{
                margin: "18px 0 0",
          ...
          >
            ...
            >
              {readableSections}/{photoSteps.length}{" "}
              {isItalian ? "screenshot letti" : "screenshots read"}
              {extractedResult
                ? ` · ${isItalian ? "Risultato" : "Score"} ${extractedResult}`
                : ""}
            </div>
          </div>
        </div>
      ...
      >
        {STEPS.map((step, index) => {
          ...
          const isCompleted =
            stepData[step.id] !== null && stepData[step.id] !== undefined;
          ...
          return (
            <button
              ...
              style={{
                ...
                borderRadius: "16px",
                background: isActive
                  ? "linear-gradient(145deg, rgba(61, 220, 151, 0.18), rgba(0, 161, 166, 0.10))"
                  : isCompleted
                    ? "rgba(34, 197, 94, 0.10)"
                    : isSkipped
            ...
            <div>
              ...
              >
                {currentSection === HOME_AWAY_STEP_ID
                  ? isItalian
                    ? "Prima cosa: dove hai giocato?"
                    : "First: where did you play?"
            ...
            <div style={{ marginTop: "8px" }}>
              ...
              >
                ...
                >
                  ...
                  >
                    {isItalian ? "Ho giocato in casa" : "I played home"}
                  </div>
                  <div
                    style={{
                      fontSize: "13px",
                  ...
                  >
                    {isItalian
                      ? "La tua squadra e a sinistra/casa negli screenshot."
                      : "Your team is left/home in screenshots."}
                  </div>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  >
                    {isItalian ? "Ho giocato fuori casa" : "I played away"}
                  </div>
                  <div
                    style={{
                      fontSize: "13px",
                  ...
                  >
                    {isItalian
                      ? "La tua squadra e a destra/fuori negli screenshot."
                      : "Your team is right/away in screenshots."}
                  </div>
                </button>
              </div>
              <div
            ...
            <>
              ...
              >
                ...
                >
                  ...
                  {hasImageForExtract
                    ? isItalian
                      ...
                      : "Screenshot uploaded"
                    : isItalian
                      ? "Carica lo screenshot giusto"
                      : "Upload the right screenshot"}
                </div>
                <div
                  style={{
                    fontSize: "13px",
                ...
                >
                  {hasImageForExtract
                    ? isItalian
                      ? "Ora fai leggere lo screenshot al Coach. Ogni lettura consuma 2 HP e aggiorna questa sezione."
                      : "Now let the Coach read it. Each read uses 2 HP and updates this section."
                    : isItalian
                      ? "Scegli dalla galleria o usa la fotocamera. Dopo il caricamento potrai farlo leggere al Coach."
                      : "Choose from gallery or use the camera. After upload, the Coach can read it."}
                </div>
              </div>
              {/* Image Preview(s) */}
              {isPlayerRatingsStep ? (
                  ...
                  {[0, 1].map((slot) => (
                    ...
                    >
                      <div style={{ fontSize: "13px", opacity: 0.9 }}>
                        {slot === 0
                          ? t("stepPlayerRatingsPhoto1")
                          : t("stepPlayerRatingsPhoto2")}
                      </div>
                      {playerRatingsImages[slot] ? (
                        <div style={compactPreviewFrameStyle}>
                          <img
                            src={playerRatingsImages[slot]}
                            alt=""
                      ...
                      >
                        <button
                          ...
                          onClick={() =>
                            document
                              .getElementById(`match-upload-${slot}`)
                              ?.click()
                          }
                          disabled={extracting || saving}
                          style={{
                            ...
                            gap: "8px",
                            cursor:
                              extracting || saving ? "not-allowed" : "pointer",
                            fontSize: "14px",
                            fontWeight: 700,
                            color: "var(--accent)",
                        ...
                        <button
                          ...
                          onClick={() =>
                            document
                              .getElementById(`match-camera-${slot}`)
                              ?.click()
                          }
                          disabled={extracting || saving}
                          style={{
                ...
                <>
                  ...
                  >
                    <button
                      ...
                      onClick={() =>
                        document
                          .getElementById(`match-upload-${currentSection}`)
                          ?.click()
                      }
                      disabled={extracting || saving}
                      style={{
                    ...
                    <button
                      ...
                      onClick={() =>
                        document
                          .getElementById(`match-camera-${currentSection}`)
                          ?.click()
                      }
                      disabled={extracting || saving}
                      style={{
                ...
                {hasImageForExtract && (
                  <button
                    ...
                    style={{
                      ...
                      gap: "8px",
                      fontWeight: 900,
                      boxShadow: currentData
                        ? "none"
                        : "0 10px 30px rgba(61, 220, 151, 0.24)",
                    }}
                  >
                    {extracting ? (
                      <>
              ...
              {currentData && (
                ...
                >
                  <strong>
                    {isItalian
                      ? "Perfetto: questa sezione ora aiuta il Coach a capire meglio la partita."
                      : "Perfect: this section now helps the Coach understand the match better."}
                  </strong>
                </div>
              )}
            </>
        ...
        >
          <div style={{ minWidth: "220px", flex: 1 }}>
            ...
            >
              {minimumReached
                ? isItalian
                  ? "Puoi salvare la partita e aggiornare dashboard, memoria AI e task."
                  : "You can save the match and update dashboard, AI memory and tasks."
                : isItalian
                  ? "Leggi almeno 3 screenshot per creare una partita utile."
                  : "Read at least 3 screenshots to create a useful match."}
            </p>
          </div>
          <button
            ...
            style={{
              ...
              boxShadow:
                saving || photosUploaded < 3
                  ? "none"
                  : "0 12px 32px rgba(34, 197, 94, 0.28)",
            }}
          >
            {saving ? (
              <>
        ...
        >
          <div
            style={{
              ...
              position: "relative",
              boxShadow:
                "0 24px 80px rgba(0,0,0,0.65), 0 0 50px rgba(61,220,151,0.18)",
            }}
          ...
          >
            ...
            >
              <div
                style={{
                  ...
                  justifyContent: "center",
                  background:
                    "linear-gradient(135deg, rgba(255,203,5,0.22), rgba(61,220,151,0.14))",
                ...
                <button
                  ...
                  style={{
                    ...
                    background: isHome
                      ? "rgba(61, 220, 151, 0.3)"
                      : "rgba(61, 220, 151, 0.1)",
                    border: `1px solid ${isHome ? "rgba(61, 220, 151, 0.6)" : "rgba(61, 220, 151, 0.3)"}`,
                    borderRadius: "8px",
                    color: "var(--accent)",
                    fontSize: "14px",
            ...
            <div style={{ marginBottom: "16px" }}>
              ...
              <input
                ...
                onFocus={(e) => {
                  e.target.style.borderColor = "rgba(61, 220, 151, 0.6)";
                  e.target.style.boxShadow = "0 0 10px rgba(61, 220, 151, 0.3)";
                }}
                onBlur={(e) => {
                  e.target.style.borderColor = "rgba(61, 220, 151, 0.3)";
      ...
      <style jsx>{`
        ...
        .match-choice-card:hover {
          ...
          box-shadow: 0 12px 32px rgba(61, 220, 151, 0.12);
        }
        .match-save-cta {
          position: relative;
          overflow: hidden;
