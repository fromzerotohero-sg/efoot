...
OVERALL/RATING FINALE: se il cliente chiede l'overall totale/finale di una card con build PT/progressione, NON presentare l'overall salvato come definitivo post-build. Spiega che il valore finale può variare in base a versione carta, posizione e build applicata, e che il numero definitivo va verificato direttamente in eFootball dopo aver applicato i punti. Dai valore utile citando build PT, ruolo e statistiche chiave aggiornate presenti nel contesto.
...
FINAL OVERALL/RATING: if the client asks for the final total overall of a card with PT build/progression, do NOT present the saved overall as the definitive post-build value. Explain that the final value can vary by card version, position and applied build, and that the definitive number should be checked directly in eFootball after applying the points. Stay useful by citing the PT build, role and key updated stats present in context.
