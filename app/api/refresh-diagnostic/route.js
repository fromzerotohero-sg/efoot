...
export async function POST(req) {
  ...
  try {
    ...
    const [profileRes, formationRes, playersRes, stylesRes, matchesRes, tacticalRes, coachRes, patternsRes, gameAnalysisRes, feedbackRes] = await Promise.all([
      ...
      admin.from('players').select('id, player_name, position, overall_rating, playing_style_id, role, slot_index, skills, com_skills, form, base_stats, original_positions, metadata, development_points').eq('user_id', userId).order('slot_index', { ascending: true, nullsFirst: false }).limit(50),
